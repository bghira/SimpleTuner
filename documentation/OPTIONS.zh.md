...
- **说明**：仅在启用 `--gradient_checkpointing` 时生效。`unsloth` 变体需要 CUDA。FFN-only 变体目前支持 Chroma、Flux、Krea 2、LTXVideo2、MageFlow、Wan 和 Z-Image；模型不支持该 scope 时会直接报错。实测 tradeoff 见 [Unsloth-style checkpointing](experimental/UNSLOTH_CHECKPOINTING.md)。
...
- **默认值**：`12`
- **内容**：activation offload 可使用的 pinned CPU tensor bucket 最大数量。
- **原因**：pinned memory 有利于 CPU/GPU 传输，但可变分辨率和文本长度会产生少见 tensor 形状。达到上限后，新的 bucket 形状会改用普通 CPU memory。
- **说明**：设为 `0` 可关闭 activation offload 的 pinned-memory pooling。

### `--gradient_checkpointing_offload_prefetch`

- **默认值**：`false`
- **内容**：学习 offloaded activations 的 backward restore 顺序，并把可能下一个需要的 tensor 预取回 GPU。
- **原因**：JIT H2D restore 通常很难重叠。顺序稳定后，prefetch 可把部分传输隐藏在 backward compute 后面。
- **说明**：实验性功能，并且只在启用 `--gradient_checkpointing_offload_attention` 时生效。
...
- `int4-torchao`、`int8-torchao` `fp8-torchao`、`fp8wo-torchao` 通过 Diffusers 使用 TorchAoConfig（CUDA）。需要 `torchao` 与较新的 diffusers/transformers。
...
- `fp8-native` and `fp8-torchao` 需要支持 FP8 scaled matmul 的 Ada Lovelace（RTX 40/L40S）、Hopper（H100/H200）或更新的加速器
- `fp8-transformerengine` 会把符合条件的 Linear 层替换为 TransformerEngine FP8 模块，并用 TE FP8 autocast 包装模型 forward。使用 `pip install 'simpletuner[transformerengine]'` 安装；此 preset 面向 Ada Lovelace、Hopper 或更新的 CUDA 加速器。
...
- `fp8-sdnq` - 使用 SDNQ 原生 FP8 matmul 的 FP8 权重，面向 H100/H200 级加速器
...
- `--sdnq_modules_to_not_convert`、`--sdnq_modules_to_not_use_matmul` - JSON 数组、文件路径或逗号分隔的 module pattern。
- `--sdnq_modules_dtype_dict`、`--sdnq_modules_quant_config` - 用于逐模块 dtype/quantization 覆盖的 JSON 对象或文件路径。
...
- **默认值**：`trained-stage`
- **内容**：选择 SDXL 验证只运行已训练阶段，还是运行拆分的 base/refiner 管线。
...
- **内容**：full-pipeline 验证使用已训练 stage 2 模型进行 refine 时加载的固定 SDXL stage 1/base 模型。
- **默认值**：从所选 SDXL 版本推断，通常是 `stabilityai/stable-diffusion-xl-base-1.0`
...
- **注意**：
  ...
  - 对于 Flux Kontext，这是选择验证数据集的正确方式；不要启用 `--validation_using_datasets`
...
Flux Kontext 的验证也始终走这条基于条件的路径。使用 `--eval_dataset_id` 选择用于验证的编辑数据集，并保持 `--validation_using_datasets` 为关闭状态。
...
**Flux Kontext 与 `--validation_using_datasets`**：不要启用此标志。Kontext 是纯编辑模型，验证会通过其常规的成对图像 + 条件数据集路径运行。请改用 `--eval_dataset_id` 选择编辑数据集。
...
- **内容**：训练数据加载器的全局默认批大小。主要数据集可以用自己的 `train_batch_size` 覆盖采样器微批大小；没有覆盖值的数据集使用此值。
- **原因**：影响内存消耗、收敛质量与训练速度。批越大结果通常越好，但过大可能导致过拟合或训练不稳定，并延长训练时间。需要实验，通常希望在不降低训练速度的前提下尽量用满显存。
- **静态配置**：训练器级学习率缩放，以及其他必须使用单个固定批大小的配置或报告，即使数据集覆盖了微批大小，也仍使用此全局值。
- **梯度累积**：当选中的数据集具有不同的解析后微批大小时，一次优化器更新的全局样本总数，是所有累积微步和所有数据并行 rank 上实际本地微批大小之和。被选中且没有覆盖值的数据集在对应 rank 上使用此全局默认值。常用的全局/有效批大小公式仍是配置参考值，而不是每次更新的准确样本数。梯度检查点不改变这一计算方式。
...
- **内容**：用于特征提取的预训练编码器。
- **原因**：论文使用 DINOv2-g（ViT-Giant）。更小的 DINO 变体占用更少内存，而 Qwen-VL 教师可提供更强的语义视觉特征。
- **默认**：`dinov2_vitg14`
- **选项**：`dinov2_vitg14`, `dinov2_vitb14`, `dinov2_vits14`, `qwen3-vl-4b`, `qwen2.5-vl-7b`，或 Hugging Face Qwen-VL 模型 ID。

### `--crepa_encoder_frames_batch_size`

- **内容**：外部特征编码器并行处理的帧数。0 或负数表示整批全部帧同时处理；若不能整除，剩余会作为更小批次处理。
- **原因**：外部视觉编码器可通过切片批处理帧来减少显存占用（速度降低）。
- **默认**：`-1`
...
- **内容**：选择 CREPA 的教师信号来源。
- **原因**：`encoder` 用于经典外部编码器路径，`backbone` 用于内部块到块对齐，`self_flow` 用于 Self-Flow 的更干净 EMA 教师视图。
- **选项**：`encoder`, `backbone`, `self_flow`
- **默认**：`encoder`

### `--crepa_self_flow`

- **内容**：启用 Self-Flow 模式的旧布尔别名。
- **原因**：为了兼容旧配置而保留，但新配置更推荐使用 `crepa_feature_source=self_flow`。
- **默认**：`false`
- **注意**：它与 `crepa_use_backbone_features` 冲突，也会与指向其他模式的 `crepa_feature_source` 冲突。

### `--crepa_self_flow_mask_ratio`

- **内容**：在 Self-Flow 中接收替代 timestep 的 token 比例。
- **原因**：控制更干净 token 与更高噪声 token 之间的信息不对称程度。较高数值会增强自监督信号，但也可能让训练不稳定。
- **默认**：`0.1`
- **范围**：`0.0` 到 `0.5`

### `--crepa_teacher_block_index`

- **内容**：使用主干特征或 Self-Flow 时的教师块索引。
- **原因**：无需外部编码器即可让早期学生块对齐到更深教师块。Self-Flow 明确要求它，以便 EMA 教师读取更深的语义层。
- **默认**：在 backbone 模式下未提供时使用 `crepa_block_index`；在 Self-Flow 模式下为必填。
...
crepa_model = "dinov2_vitg14"  # 或使用 "qwen3-vl-4b" 获取 Qwen-VL 视觉特征
...
- **内容**：检查点保存和 TorchInductor 图编译（包括重新编译）前所需的最小可用磁盘空间。
- **原因**：提前检测空间不足，并对检查点或编译器缓存所在的文件系统执行配置的操作。
- **格式**：大小字符串，如 `100G`、`50M`、`1T`、`500K`，或纯字节数。
- **默认**：无（功能禁用）
- **范围**：每个执行编译的进程都会检查 `TORCHINDUCTOR_CACHE_DIR`（或 PyTorch 默认目录），以及配置的独立 `TRITON_CACHE_DIR`。这只是预检查，不会预留空间：编译开始后仍可能耗尽空间，后端初始化也可能在检查前写入。请设置足够支持编译的阈值。磁盘检查不会恢复 CUDA 内存不足错误，也不会重试失败的编译。
...
options:
  ...
  --validation_prompt_library [VALIDATION_PROMPT_LIBRARY]
                        使用 SimpleTuner 内置 prompt 库。音乐/音频的
                        caption + lyrics prompt 可使用 "audio"。
  --user_prompt_library USER_PROMPT_LIBRARY
                        自定义 JSON prompt 库路径。条目可使用 prompt 或
                        caption，并可选提供 lyrics。
  --eval_dataset_id EVAL_DATASET_ID
