...
- **内容**: diffusion Transformer の block-wise training を有効にする JSON object または JSON file path。
- **必須キー**: `layers_per_block` は 1 noise range で実行する連続 denoiser layer の最大数です。
- **任意キー**: `overlap`（既定 `0.05`）、`blocks_to_train`、`block_paths`、`timestep_boundaries`。
...
- 組み込み検証が完了するたびに（WebUI からの手動実行、定期実行、ベースモデルのベンチマーク）、公開プロバイダーが未設定の場合もスクリプトが実行されます。この場合、`{local_checkpoint_path}` は `output_dir`（チェックポイントは不要）、`{remote_checkpoint_path}` は空になります。設定済みのプロバイダーでは、成功したアップロードごとに返されたパスでフックが実行され、アップロード失敗時にローカルフックは実行されません。スキップ・失敗・中止された検証や外部スクリプトの起動では、この完了フックは実行されません。
...
- **形式**: エントリ配列、または `runs` 配列を持つオブジェクト。各エントリは次を含められます:
  ...
  - `strength`: 任意のスカラー上書き。
  - `target_stage`: このアダプタを適用する任意の multi-stage 検証ターゲット。利用できる別名はモデル依存で、一般的な値は `one`、`two`、`stage1`、`stage2`、Wan の `high`/`low` です。
...
- **内容**: 検証用の入力画像パスとプロンプトの JSON リストです。例：`[{"path": "/data/val/frame.png", "prompt": "ゆっくり前進するショット"}]`
- **理由**: 検証データセットを作成せずに、画像コンディショニング付き検証を実行できます。
- **注意**:
  - 各エントリには `path` と `prompt` が必要です。`shortname` または `name` で出力ラベルを任意指定できます。
  - 検証プロンプトを準備する時点で、画像パスはローカルに存在している必要があります。
  - 設定すると、データセットから検証サンプルを探す代わりに、これらのエントリが検証入力画像として使用されます。
...
- **内容**: エポックカウンタが `--num_train_epochs` に達した時点で、`--max_train_steps` に未達でも学習を停止します。
- **理由**: 明示的な `--max_train_steps` 実行で、ステップ上限に達するまで追加のエポックパスを続けたい場合は無効にします。`--max_train_steps=0` の実行は常にエポック駆動で、厳密なエポック上限を使います。
...
- **内容**: 学習データローダのグローバルな既定バッチサイズ。主要データセットでは `train_batch_size` によりサンプラのマイクロバッチサイズを上書きでき、上書きのないデータセットはこの値を使います。
...
- **静的設定**: トレーナーレベルの学習率スケーリングや、単一の固定バッチサイズを必要とするその他の設定・レポートでは、データセットがマイクロバッチサイズを上書きしていても、このグローバル値を使います。
- **勾配蓄積**: 選択されたデータセットの解決後のマイクロバッチサイズが異なる場合、1 回のオプティマイザ更新におけるグローバルなサンプル総数は、すべての蓄積マイクロステップおよびデータ並列ランクにおける実際のローカルマイクロバッチサイズの合計です。上書きのないデータセットが選択された場合、そのランクではこのグローバル既定値を使います。通常のグローバル/有効バッチサイズ式は、更新ごとの正確なサンプル数ではなく設定上の参照値です。勾配チェックポイントはこの計算に影響しません。
...
- **内容**: アスペクトバケットがそのデータセットのバケットサイズ要件より小さい場合に `repeats` を自動調整します。
- **理由**: マルチ GPU 構成で最小要件を満たさないデータセットによる学習失敗を防ぎます。
- **仕組み**:
  - データセットごとのバケットサイズ要件を計算: `解決後のデータセット train_batch_size × num_gpus × gradient_accumulation_steps`
  - いずれかのアスペクトバケットがそのデータセットのバケットサイズ要件を下回る場合、`repeats` を自動的に増加
...
- **例**: 25 枚、8 GPU、`train_batch_size=4` の場合、そのデータセットのバケットサイズ要件は 32。自動で `repeats=1` にして 50 サンプル（25 × 2）を確保します。
...
- **理由**: 論文では DINOv2-g（ViT-Giant）を使用。小型 DINO はメモリ消費が少なく、Qwen-VL 教師はより強い意味的な視覚特徴を提供できます。
- **既定**: `dinov2_vitg14`
- **選択肢**: `dinov2_vitg14`, `dinov2_vitb14`, `dinov2_vits14`, `qwen3-vl-4b`, `qwen2.5-vl-7b`、または Hugging Face の Qwen-VL モデル ID。

### `--crepa_encoder_frames_batch_size`

- **内容**: 外部特徴エンコーダが並列処理するフレーム数。0 以下でバッチ全フレームを同時処理します。割り切れない場合、残りは小さなバッチとして処理されます。
- **理由**: 外部視覚エンコーダはフレームを分割バッチ処理でき、速度と引き換えに VRAM を抑えられます。
- **既定**: `-1`
...
crepa_model = "dinov2_vitg14"  # Qwen-VL 視覚特徴には "qwen3-vl-4b" も使用可能
...
- **内容**: チェックポイント保存および TorchInductor グラフコンパイル（再コンパイルを含む）の前に必要な最小空きディスク容量。
- **理由**: 容量不足を早期に検知し、チェックポイントまたはコンパイラキャッシュのファイルシステムに対して設定されたアクションを実行します。
- **形式**: `100G`、`50M`、`1T`、`500K` のようなサイズ文字列、またはバイト数。
- **デフォルト**: なし（機能無効）
- **対象**: コンパイルする各プロセスで `TORCHINDUCTOR_CACHE_DIR`（未設定なら PyTorch の既定値）と、設定されている場合は別の `TRITON_CACHE_DIR` を確認します。これは事前確認であり、容量の予約ではありません。コンパイル開始後に容量が不足する場合や、確認前にバックエンドの初期化が書き込む場合があります。コンパイルに十分な閾値を設定してください。CUDA メモリ不足からの復旧や失敗したコンパイルの再試行は行いません。
...
options:
  ...
  --validation_prompt_library [VALIDATION_PROMPT_LIBRARY]
                        SimpleTuner の組み込みプロンプトライブラリを使います。
                        音楽/音声の caption + lyrics プロンプトには
                        "audio" を使います。
  --user_prompt_library USER_PROMPT_LIBRARY
                        カスタム JSON プロンプトライブラリへのパス。
                        エントリは prompt または caption と、任意の lyrics を使えます。
  --eval_dataset_id EVAL_DATASET_ID
