...
### LTX-2 の intrinsic / reference conditioning

- **内容**: upstream 形式の clean-token conditioning を行う LTX-2 model wrapper 用オプション。
- **設定キー**:
  - `ltx2_intrinsic_conditioning`: condition object の配列。対応する `type` は `first_frame`, `prefix`, `suffix`, `spatial_crop`, `mask` です。各 object は `probability` を受け取ります。
  - `ltx2_prefix_conditioning_probability`, `ltx2_prefix_conditioning_frames`, `ltx2_suffix_conditioning_probability`, `ltx2_suffix_conditioning_frames`, `ltx2_first_frame_conditioning_probability`, `ltx2_mask_conditioning_probability`: intrinsic condition を追加する短縮キー。
  - `ltx2_reference_spatial_scale_factor`, `ltx2_reference_temporal_scale_factor`: IC-LoRA reference 座標スケーリングの任意 override。未指定時は reference/target latent サイズから spatial scale を推定します。
- **理由**: LTX-2 intrinsic condition は選択された target token を clean latent に置き換え、その token timestep を 0 にし、video loss から除外します。`mask` では値 `1` が clean conditioning / no loss を意味します。
- **注記**: これらの設定は JSON/TOML 設定フィールド、または対応する `--ltx2_*` CLI flag で指定できます。これらの LTX-2 mode 用 dataset template と WebUI control は、この model-level support とは別です。
