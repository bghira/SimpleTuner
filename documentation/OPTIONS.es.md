...
- `metal-flash-attention` usa el backend PyTorch custom-op de Universal Metal Flash Attention en Apple Silicon. Instala primero el paquete UMFA `examples/pytorch-custom-op-ffi`; las llamadas SDPA MPS FP16/FP32 elegibles se envian a `metal_sdpa_extension`. SimpleTuner ejecuta una comprobacion inicial de paridad contra PyTorch SDPA y rechaza builds UMFA que no coincidan numericamente.
