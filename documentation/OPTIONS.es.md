...
- **Qué**: Para `--minimax_music_train_component=language_model`, limita las ventanas `prefix` y `random` aisladas en frames de audio de 25Hz.
- **Predeterminado**: `0` (entrenar con pistas completas)
- **Notas**:
  - Un frame son 40ms; 7500 frames son cinco minutos.
  - El modo `continuation` usa sus propios ajustes de frames objetivo y duración descritos abajo.
...
- **Qué**: Elige cómo se construye la secuencia de entrenamiento del modelo de lenguaje.
...
- **Notas**:
  ...
  - `continuation` muestrea un tramo visible limitado y aplica la pérdida solo a sus frames objetivo finales. Usa las opciones de continuación siguientes para controlar su contexto.
  - `random` aislado requiere `--minimax_music_lm_max_frames` positivo; `continuation` no usa esa opción.

### `--minimax_music_lm_target_frames`

- **Qué**: En modo `continuation`, define cuántos frames finales de 25Hz reciben pérdida de siguiente token.
- **Predeterminado**: `128` (un segmento RVQ nativo, 5,12 segundos)
- **Notas**: Los frames visibles anteriores quedan como contexto causal y se excluyen de la entropía cruzada.

### `--minimax_music_lm_continuation_crop_mode`

- **Qué**: Elige dónde comienza un tramo visible de `continuation`.
- **Opciones**: `full` (predeterminado), `random`
- **Notas**:
  - `full` siempre comienza en el frame cero de la canción y muestrea un final entre los límites de duración.
  - `random` puede omitir el principio, pero conserva al menos un segmento nativo de 128 frames antes del objetivo y añade la posición al prompt.
  - Los tramos aleatorios omiten letras completas salvo que exista `lyrics_window`. Las pistas demasiado cortas usan un prefijo completo.

### `--minimax_music_lm_min_duration_seconds`

- **Qué**: Duración mínima visible por el modelo en modo `continuation`.
- **Predeterminado**: `5.12`
- **Notas**: Se redondea hacia arriba en intervalos nativos de 128 frames (5,12 segundos). Los tramos aleatorios pueden ser más largos para reservar un segmento de contexto.

### `--minimax_music_lm_max_duration_seconds`

- **Qué**: Limita la duración visible en `continuation` sin modificar la pista RVQ almacenada.
- **Predeterminado**: `0` (usar la longitud disponible)
- **Notas**:
...
- **Qué**: Activa atención sparse 3D experimental, consciente del entrenamiento, para los tokens de video objetivo de MiniMax-H3.
- **Opciones**: `disabled`, `moba3d`
- **Predeterminado**: `disabled`
- **Opciones relacionadas**:
  - `minimax_h3_sparse_block_shape`: dimensiones `(T,H,W)` separadas por coma o `x` cuyo producto es 128. Predeterminado: `1,8,16`.
  - `minimax_h3_sparse_video_kv_fraction`: fracción de bloques KV de video objetivo seleccionada por cada bloque query de video objetivo. Predeterminado: `0.5`.
  - `minimax_h3_sparse_share_heads`: comparte rutas entre heads de atención. Predeterminado: `false`.
  - `minimax_h3_sparse_start_layer`: mantiene las capas anteriores en atención densa. Predeterminado: `0`.
- **Notas**:
  - El texto, audio, contexto de referencia y queries que no son objetivo permanecen densos.
  - Requiere CUDA FlexAttention. Ulysses context parallelism funciona con `context_parallel_strategy=alltoall`; ring context parallelism y TREAD son incompatibles.
  - MiniMax no ha publicado la configuración exacta de sparse routing para H3. Esta aproximación es para experimentos controlados de fine-tuning y no garantiza mejor rendimiento.
...
- **Qué**: Objeto JSON o ruta a un archivo JSON que activa entrenamiento block-wise de diffusion Transformer.
- **Clave obligatoria**: `layers_per_block` define el maximo de capas consecutivas del denoiser por rango de ruido.
- **Claves opcionales**: `overlap` (predeterminado `0.05`), `blocks_to_train`, `block_paths` y `timestep_boundaries`.
...
- **Opciones**: `torch`, `torch-ffn`, `unsloth`, `unsloth-ffn`
- **Qué**: Selecciona implementación y scope para gradient checkpointing.
  - `torch` (por defecto): checkpoint del bloque completo compatible y recompute en backward.
...
- `metal-flash-attention` usa el backend PyTorch custom-op de Universal Metal Flash Attention en Apple Silicon. Instala primero el paquete UMFA `examples/pytorch-custom-op-ffi`; SimpleTuner enruta la atencion por el dispatcher MPS SDPA de PyTorch, que los builds UMFA actuales registran. Las llamadas SDPA MPS FP32/FP16/BF16 4D elegibles — cualquier cantidad de heads incluida single-head, cualquier sequence length, layouts transposed estilo FLUX, masks bool/additive de hasta 4D y llamadas causales (entrenamiento incluido — el backward causal pasa paridad exacta de gradientes) — se codifican directamente en el command stream MPS de PyTorch sin sincronizacion por llamada, y las entradas FP16/BF16 usan kernels nativos de baja precision sin promocion a FP32. Las llamadas con dropout o `enable_gqa` hacen fallback a PyTorch SDPA; builds antiguos `PrivateUse1` son bypassed por tensores `torch.device("mps")`. SimpleTuner ejecuta comprobaciones iniciales de paridad FP32/FP16/BF16 forward y autograd, ademas de paridad causal forward, y rechaza builds UMFA que no coincidan numericamente o que no expongan `get_dispatch_stats()`. `metal-flash-attention-int8` y `metal-flash-attention-int4` configuran el modo global de cuantizacion blockwise de UMFA con `set_quantization_mode(ext.QUANT_INT8, ext.QUANT_BLOCK_WISE)` o `set_quantization_mode(ext.QUANT_INT4, ext.QUANT_BLOCK_WISE)`, limpian ese modo al cambiar backends y requieren una comprobacion inicial adicional de salidas con autograd, gradientes multi-head finitos, soporte dispatcher-level para bool/additive masks y ningun fallback PyTorch. Usa `metal_sdpa_extension.get_dispatch_stats()` durante una ejecucion para confirmar que `fp32_instream` (o `quantized_autograd` para los alias quantized) aumenta y `pytorch_fallback` queda en `0`; bool masks all-true tambien aumentan `mask_all_true_skipped`. La extension expone ademas `rope_scaled_dot_product_attention`, un entry point RoPE+SDPA fusionado que sigue la convencion rotary interleaved-pair de FLUX.1/FLUX.2/Krea2/Z-Image; el entrenamiento fluye por un autograd fusionado (rotacion inversa sobre dQ/dK en backward); las llamadas con gradientes que usan mask o GQA hacen fallback a rotacion eager.
...
- **Qué**: Habilita la aplicación de clasificadores NSFW durante el preprocesamiento del caché VAE.
- **Predeterminado**: `false`.
- **Notas**: Solo se escanean las muestras sin caché que el caché VAE está a punto de procesar. Los cachés VAE existentes y `skip_file_discovery=vae` se consideran confiables y no se reescanean.
- **Más información**: Consulta [NSFW.es.md](NSFW.es.md) para notas de privacidad y responsabilidad, y [DATALOADER.es.md#nsfw-classifier-checks-during-vae-caching](DATALOADER.es.md#nsfw-classifier-checks-during-vae-caching) para detalles del caché VAE.

### `--nsfw_check_models`

- **Qué**: Lista CSV de modelos de clasificación de imágenes de Hugging Face Transformers. Puedes adjuntar un umbral por modelo con `:threshold=0.5`.
- **Predeterminado**: `Falconsai/nsfw_image_detection:threshold=0.5,AdamCodd/vit-base-nsfw-detector:threshold=0.5`.
- **Notas**: Solo se admiten modelos Transformers estándar. No se habilita `trust_remote_code` y no se cargan clasificadores basados en `timm`.

### `--nsfw_check_min_votes`

- **Qué**: Número de clasificadores configurados que deben devolver NSFW antes de rechazar un frame.
- **Predeterminado**: `2`.
- **Notas**: Debe ser al menos `1` y no mayor que el número de clasificadores configurados.

### `--nsfw_check_backend_types`

- **Qué**: Lista CSV de valores `type` de backend de datos que deben escanearse.
- **Predeterminado**: `all`.
- **Ejemplo**: `local,huggingface,csv,aws`.

### `--nsfw_check_sample_types`

- **Qué**: Lista CSV de valores `dataset_type` que deben escanearse.
- **Predeterminado**: `image,conditioning`.
- **Notas**: Los datasets de evaluación no se escanean.

### `--delete_nsfw_images`

- **Qué**: Elimina muestras rechazadas por el clasificador NSFW cuando el backend admite eliminación.
- **Predeterminado**: `false`.
- **Notas**: Cuando está deshabilitado, las muestras rechazadas se eliminan de los metadatos de la ejecución actual, pero permanecen en el dataset de origen.

### `--nsfw_check_video_frame_count`

- **Qué**: Número de frames muestreados desde entradas de video o conditioning con varios frames.
- **Predeterminado**: `3`.

### `--nsfw_check_video_frame_selection`

- **Qué**: Modo de selección de frames para comprobaciones NSFW en video.
- **Predeterminado**: `uniform`.
- **Opciones**: `uniform`, `first`, `middle`.

### `--nsfw_check_video_min_flagged_frames`

- **Qué**: Número de frames de video comprobados que deben ser rechazados antes de rechazar toda la muestra.
- **Predeterminado**: `1`.
...
### Opciones de validación de LTX-2
...
- **Qué**: Copia los pesos actuales en la EMA antes del paso de optimizador configurado y luego cambia directamente a `--ema_decay`.
- **Por qué**: Coincide con recetas de entrenamiento que retrasan el suavizado EMA sin dejar la EMA congelada en la inicialización. El valor predeterminado `0` conserva la rampa EMA existente de SimpleTuner.
...
- **Por qué**: Habilita monitoreo local o externo. `simpletuner` escribe JSONL, un manifiesto, metadatos de validación y un informe HTML autónomo en `output_dir`. Usa valores separados por comas para múltiples trackers.
- **Opciones**: simpletuner, wandb, tensorboard, swanlab, comet_ml, custom-tracker, none
...
Assistant LoRA y AnyFlow elevan temporalmente el límite de caché de compilación de Dynamo por código a un mínimo de 32 para las variantes del profesor, entrenamiento y validación. Se respeta un límite mayor del usuario y se restaura el original al salir, incluso tras errores.
