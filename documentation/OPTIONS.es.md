...
- `metal-flash-attention` usa el backend PyTorch custom-op de Universal Metal Flash Attention en Apple Silicon. Instala primero el paquete UMFA `examples/pytorch-custom-op-ffi`; las llamadas SDPA MPS FP32 4D sin mask elegibles con al menos cuatro heads y secuencia de longitud 64 o mayor, incluidos layouts transposed estilo FLUX, se envian directamente a `metal_sdpa_extension`, mientras que otras llamadas pasan por PyTorch SDPA. Builds UMFA actuales registran la dispatch key PyTorch `MPS`, asi que llamadas quantized con mask todavia pueden llegar a UMFA por el dispatcher MPS SDPA de PyTorch; builds antiguos `PrivateUse1` son bypassed por tensores `torch.device("mps")`. SimpleTuner ejecuta comprobaciones iniciales de paridad FP32 forward y autograd, y rechaza builds UMFA que no coincidan numericamente. `metal-flash-attention-int8` y `metal-flash-attention-int4` configuran el modo global de cuantizacion blockwise de UMFA con `set_quantization_mode(ext.QUANT_INT8, ext.QUANT_BLOCK_WISE)` o `set_quantization_mode(ext.QUANT_INT4, ext.QUANT_BLOCK_WISE)`, limpian ese modo al cambiar backends y requieren una comprobacion inicial adicional de salidas con autograd, gradientes multi-head finitos, soporte dispatcher-level para bool/additive masks y ningun fallback PyTorch. Usa `metal_sdpa_extension.get_dispatch_stats()` durante una ejecucion para confirmar que `quantized_autograd` aumenta y `pytorch_fallback` queda en `0`; bool masks all-true tambien aumentan `mask_all_true_skipped`.
