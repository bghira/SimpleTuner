...
कैप्शन खोजने के लिए फ़ाइल एक्सटेंशन की यह वैकल्पिक गैर-खाली सूची दे सकते हैं, जैसे `["jsonl"]`। डिफ़ॉल्ट `["txt", "json", "jsonl"]` है। बैकएंड की अपनी मेटाडेटा और बकेट-कैश JSON फ़ाइलें हमेशा बाहर रखी जाती हैं, सहेजी गई फ़ाइल सूची से पुनः शुरू करने पर भी।

### `data_generator`

केवल `dataset_type: "caption"` (एकवचन) के लिए मान्य है। यह वैकल्पिक ऑब्जेक्ट चित्र, टेक्स्ट एम्बेडिंग और VAE कैशिंग से पहले हर कैप्शन के लिए सूची के **हर** रिज़ॉल्यूशन पर एक PNG और उसका टेक्स्ट बनाता है। परिणाम सामान्य चित्र प्रशिक्षण में जाते हैं; डिस्टिलर आवश्यक नहीं है। इस ऑब्जेक्ट के बिना कैप्शन डेटासेट को अब भी कैप्शन स्वीकार करने वाला डिस्टिलर चाहिए।

- `resolutions` (आवश्यक): `WIDTHxHEIGHT` स्ट्रिंग की गैर-खाली सूची; चौड़ाई और ऊँचाई 32 के गुणज हों। हर रिज़ॉल्यूशन `<id>-generated-<resolution>` बनाता है, मूल छोटी भुजा के पिक्सेल आकार पर प्रशिक्षण करता है और क्रॉप बंद रखता है। मूल डेटासेट की `probability` बकेटों में बराबर बाँटी जाती है।
- `batch_size` (डिफ़ॉल्ट `1`): जनरेशन बैच, प्रशिक्षण बैच से अलग। एक्सेलेरेटर की मेमोरी समाप्त होने पर उस रिज़ॉल्यूशन के असफल बैच को आधा करके उन्हीं सैंपलों और उन्हीं सीडों से दोबारा प्रयास होता है। सफल सीमाएँ जनरेशन मैनिफ़ेस्ट में सहेजी जाती हैं। बैच एक पर मेमोरी त्रुटि या कोई दूसरी त्रुटि जनरेशन रोक देती है।
- `num_inference_steps` (डिफ़ॉल्ट `40`), `guidance_scale` (`1.0`) और `seed` (`0`): इन्फ़रेंस सेटिंग। Qwen Image में गाइडेंस `true_cfg_scale` को जाती है; एक से अधिक मान खाली नेगेटिव प्रॉम्प्ट इस्तेमाल करते हैं। वैलिडेशन गाइडेंस इसे नहीं बदलती।
- `output_dir` (वैकल्पिक): डिफ़ॉल्ट `cache_dir/generated-captions/<id>`। आउटपुट रेसिपी के फ़िंगरप्रिंट में सहेजे जाते हैं और पुनः शुरू करने पर इस्तेमाल होते हैं। इसमें कैप्शन, इन्फ़रेंस सेटिंग, हल किए गए Hub रिविज़न और स्थानीय वज़न फ़ाइलों के आकार/संशोधन समय शामिल हैं। जनरेशन के दौरान मॉडल फ़ाइलें न बदलें। कैप्शन, मॉडल या इन्फ़रेंस बदलने पर अलग कैश बनता है; बैच बदलने पर नहीं।

जनरेशन बेस मॉडल पर अडैप्टर बंद करके होती है और केवल प्रीप्रोसेसिंग के लिए लोड किया गया ट्रांसफ़ॉर्मर मुक्त किया जाता है। मूल कैप्शन नहीं बदलते। जनरेशन के लिए एकल-प्रोसेस तैयारी रन आवश्यक है; कई प्रोसेस वाले `data_generator` रन कैप्शन पढ़ने से पहले अस्वीकृत होते हैं। वितरित प्रशिक्षण के लिए जनरेटेड डायरेक्टरी को सामान्य `image` डेटासेट बनाएं: हर रिज़ॉल्यूशन के लिए एक डेटासेट, `caption_strategy: "textfile"`, `crop: false`, `resolution_type: "pixel"` और `resolution` में उस बकेट की छोटी भुजा का आकार रखें। साधारण कैप्शन डेटासेट में सैंपलर की चेकपॉइंट स्थिति सुरक्षित रखने के लिए `dataloader_prefetch=false` आवश्यक है। कैप्शन सूची, रिज़ॉल्यूशन या डेटासेट टोपोलॉजी बदलने पर नया प्रशिक्षण शुरू करें; जनरेशन कैश का पुनः उपयोग ऐसे चेकपॉइंट रिज़्यूम को संगत नहीं बनाता।

स्थानीय कैप्शन खोज में जनरेटेड आउटपुट डायरेक्टरी स्रोत `instance_data_dir` के बाहर होनी चाहिए; ओवरलैप करने वाली डायरेक्टरी स्कैन से पहले अस्वीकृत होती हैं।
...
- **Description:** इस dataset के लिए storage backend (local, memory, csv या cloud) तय करता है।

### Memory (tmpfs और RAM disk) backends

`type: "memory"` Linux tmpfs या native macOS RAM disk पर आधारित `LocalDataBackend` subclass `MemoryDataBackend` का उपयोग करता है। यह केवल `dataset_type: "text_embeds"` या `dataset_type: "image_embeds"` वाली cache-storage entries के लिए उपलब्ध है; primary image, video, audio, conditioning और evaluation datasets इसका उपयोग नहीं कर सकते। SimpleTuner एक अलग खाली filesystem mount करता है और configured `cache_dir` मौजूद होने पर उसे copy करता है। Run के दौरान read और write memory-resident copy का उपयोग करते हैं और filesystem release होने पर discard हो जाते हैं; बदलाव `cache_dir` में sync नहीं किए जाते।
...
- `memory_filesystem_path`: वैकल्पिक खाली mount directory। Default OS temporary directory में `simpletuner-memory/<dataset id>` है। यह और `cache_dir` overlap नहीं होने चाहिए और एक-दूसरे के भीतर नहीं होने चाहिए।
- `memory_filesystem_size`: Memory filesystem capacity, जैसे `512G`। Linux पर यह optional है, जहाँ default tmpfs capacity लागू होती है और `80%` जैसे percentages स्वीकार होते हैं। macOS पर यह required है और percentage के बजाय byte size होना चाहिए; SimpleTuner इसे `hdiutil` के लिए 512-byte sectors में बदलता है।
- `memory_filesystem_sudo`: केवल Linux। `true` होने पर `mount` और `umount` को `sudo -n` से चलाता है। Root के रूप में चलाते समय इसे `false` रखें। Enable करने से पहले passwordless non-interactive sudo configured होना चाहिए। macOS RAM disks इस option के बिना `hdiutil` और `diskutil` उपयोग करते हैं।

Memory backends के लिए Linux या macOS और मौजूदा cache तथा training के दौरान लिखे गए सभी embeddings को रखने लायक RAM या swap आवश्यक है। ये high-memory systems पर बड़े latent या text-embedding caches के repeated reads bottleneck होने पर उपयोगी हैं। मौजूदा cache को preload करने से startup time बढ़ता है; खाली cache से शुरू करने के लिए खाली या अनुपस्थित `cache_dir` उपयोग करें।
...
- **webshart** Webshart shard metadata से captions पढ़ता है और `type: "webshart"` के साथ `metadata_backend: "webshart"` requires करता है।
...
- **Values:** `discovery` | `parquet` | `huggingface` | `webshart`
- **Description:** dataset preparation के दौरान SimpleTuner image dimensions और अन्य metadata कैसे खोजेगा।
  ...
  - **webshart**: Webshart shard metadata उपयोग करता है, जिसमें logical-sample aspect buckets और captions शामिल हैं, image bytes scan किए बिना।
...
- **Values:** `true` | `false`
- **Description:** Dataset पर enable होने पर missing VAE latents startup में precompute होने के बजाय training के दौरान encode होकर उस dataset के VAE cache में लिखे जाते हैं।
- **Note:** Global `--vae_cache_ondemand` अभी भी सभी datasets पर लागू होता है। जब global option false हो और केवल चुने हुए datasets के VAE latents on demand encode करने हों, तब यह dataset option उपयोग करें।

### `vae_cache_disable`

- **Values:** `true` | `false`
- **Description:** किसी dataset पर सक्षम होने पर, वह dataset VAE latents को on demand encode करता है और नए generated latents को disk पर नहीं लिखता। यह बड़े datasets के लिए उपयोगी है जहाँ disk space चिंता का विषय है या लिखना व्यावहारिक नहीं है।
- **Note:** global `--vae_cache_disable` अभी भी सभी datasets पर लागू होता है। जब global option false हो, तब केवल चुने हुए datasets के लिए VAE cache writes disable करने के लिए इस dataset option का उपयोग करें। हटाया गया `disable_vae_cache` spelling reject किया जाता है; `vae_cache_disable` उपयोग करें।

### VAE cache strategy चुनना

`vae_cache_ondemand` startup time कम करने के बदले training के दौरान काम करता है; यह VRAM optimization नहीं है। अपेक्षित cache reuse और training के दौरान VAE को उपलब्ध रख पाने के आधार पर हर dataset के लिए strategy चुनें:

1. **बहुत बड़ा, reusable dataset:** जब पूरा precache pass training को बहुत देर तक रोकेगा, तब `vae_cache_ondemand=true` सेट करें। सभी images का इंतजार किए बिना training शुरू हो जाती है, और हर missing latent cache में लिखा जाता है ताकि बाद की visits में उसका reuse हो सके।
2. **बहुत high-resolution dataset:** जब training model loaded होने के साथ VAE encoding से OOM हो सकता है, तब `vae_cache_ondemand=false` रखें और global option भी false रखें। Default startup precache से training से पहले VAE unload किया जा सकता है। On-demand mode और `vae_cache_disable` दोनों को training के दौरान VAE encoding चाहिए, इसलिए वे इस memory constraint को हल नहीं करते। VAE tiling या slicing startup precache के दौरान peak memory कम कर सकता है।
3. **बहुत कम expected reuse वाला web-scale dataset:** जब dataset में millions या billions images हों और planned run के एक epoch के पास पहुँचने की संभावना न हो, तब `vae_cache_disable=true` सेट करें। Sampled images अब भी on demand encode होती हैं, लेकिन जब ज्यादातर latents दोबारा read नहीं होंगे तब cache writes और storage growth से बचा जाता है। इसकी training-time VAE memory requirement on-demand caching जैसी ही है।
...
VAE कैश तैयार करते समय, Webshart अगले शार्ड पर जाने से पहले एक शार्ड के सभी आस्पेक्ट-बकेट समूहों को प्रोसेस करता है। जब पूरे शार्ड की कैश में संपूर्ण डेटासेट नहीं समाता, तो इससे शार्ड को बार-बार डाउनलोड करना कम होता है। पहले से कैश किए गए और अन्य रैंक को सौंपे गए सैंपल समूह बनाने से पहले हटा दिए जाते हैं; प्रशिक्षण सैंपलर का क्रम नहीं बदलता।
