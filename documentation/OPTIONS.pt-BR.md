...
- **O quê**: Seleciona qual componente do MiniMax Music 3 recebe o treinamento.
...
- **O que**: Objeto JSON ou caminho de arquivo JSON que ativa treino block-wise de diffusion Transformer.
- **Chave obrigatoria**: `layers_per_block` define o maximo de camadas consecutivas do denoiser por faixa de ruido.
- **Chaves opcionais**: `overlap` (padrao `0.05`), `blocks_to_train`, `block_paths` e `timestep_boundaries`.
...
- **O que**: Intervalo dependente do modelo para checkpointing de blocos transformer. Um valor 1 e basicamente o mesmo que deixar `--gradient_checkpointing` habilitado.
- **Nota**: Flux, Flux.2, Krea 2, LTXVideo2, MageFlow, Qwen Image 2.1, Z-Image e Wan usam chunks contiguos de *n* blocos nos caminhos whole-block. Outras familias que expoem esta opcao podem continuar usando o comportamento antigo de "checkpoint a cada *n* blocos". Grupos contíguos maiores salvam menos estados de fronteira, mas ainda recalculam os blocos com checkpoint. Meça a relação entre velocidade e pico de memória; veja [Segmented Checkpointing](experimental/SEGMENTED_CHECKPOINTING.pt-BR.md#qwen-image-21).
...
- **Padrao**: `12`
- **O que**: Numero maximo de buckets distintos de tensores CPU pinned usados por activation offload.
- **Por que**: Pinned memory melhora transferencias CPU/GPU, mas resolucoes e comprimentos de texto variaveis podem criar shapes raros. Ao atingir esse limite, novos bucket shapes usam memoria CPU normal.
- **Nota**: Use `0` para desativar o pooling de pinned memory para activation offload.

### `--gradient_checkpointing_offload_prefetch`

- **Padrao**: `false`
- **O que**: Aprende a ordem de restore no backward para activations offloaded e prefetch o provavel proximo tensor para a GPU.
- **Por que**: Restore H2D just-in-time quase nao consegue overlap. Com ordem estavel, prefetch pode esconder parte da transferencia atras do backward compute.
- **Nota**: Experimental e ativo apenas com `--gradient_checkpointing_offload_attention`.
...
- **`--ltx2_validation_audio_guidance`**: Escala CFG de audio separada opcional para latentes de audio LTX-2 durante a validacao. Deixe sem definir para reutilizar `--validation_guidance`; use quando quiser dual CFG estilo Comfy com escalas separadas para video e audio.
...
- **O que**: Habilita o pós-treinamento MixFlow com interpolação desacelerada para modelos Flow Matching.
- **Por que**: Altera a amostragem de timesteps e a velocidade de interpolação sem mudar a arquitetura do modelo.
- **Nota**: Não pode ser combinado com outros schedules de timestep. Consulte o [guia do MixFlow](experimental/MIXFLOW.pt-BR.md).

### `--mixflow_gamma`

- **O que**: Controla o intervalo da interpolação desacelerada do MixFlow.
- **Intervalo**: `0.0` a `1.0`; padrão: `0.8`.
- **Nota**: `0.0` preserva a interpolação padrão, mas mantém a amostragem de timesteps do MixFlow.
...
- **Padrao**: `false`
- **Nota**: So se aplica a modelos de difusao baseados em Transformer (estilo DiT). Para modelos UNet (SDXL, SD1.5, Kolors), use U-REPA.
...
- **O que**: Normaliza a soma dos vizinhos pela soma de pesos por frame.
- **Por que**: Mantem `crepa_alignment_score` em [-1, 1] e deixa a escala da loss mais literal. Desvio experimental da equacao (6) do paper.
- **Padrao**: `false`
...
- **Por que**: Quando a media movel exponencial do score de alinhamento (`crepa_alignment_score`) atinge este valor, o CREPA e desabilitado para prevenir overfitting nas features profundas do encoder. Isto e particularmente util para treinamento text2video. O score pode passar de 1.0 se `crepa_normalize_neighbour_sum` nao estiver habilitado.
- **Padrao**: None (desabilitado)
...
## 🎯 U-REPA (Alinhamento de Representacoes para UNet)

U-REPA e uma tecnica de regularizacao para modelos de difusao baseados em UNet (SDXL, SD1.5, Kolors). Ela alinha as features do mid-block da UNet com features visuais pre-treinadas e adiciona uma loss de manifold para preservar a estrutura de similaridade relativa.

### `--urepa_enabled`

- **O que**: Habilita U-REPA durante o treino.
- **Por que**: Adiciona alinhamento de representacoes para o mid-block da UNet usando um encoder visual congelado.
- **Padrao**: `false`
- **Nota**: So se aplica a modelos UNet (SDXL, SD1.5, Kolors).

### `--urepa_lambda`

- **O que**: Peso da loss de alinhamento U-REPA em relacao a loss principal.
- **Por que**: Controla a forca da regularizacao.
- **Padrao**: `0.5`

### `--urepa_manifold_weight`

- **O que**: Peso da loss de manifold em relacao a loss de alinhamento.
- **Por que**: Prioriza a estrutura de similaridade relativa (padrao do paper: 3.0).
- **Padrao**: `3.0`

### `--urepa_model`

- **O que**: Identificador do torch hub para o encoder visual congelado.
- **Por que**: Default e DINOv2 ViT-G/14; modelos menores (ex.: `dinov2_vits14`) sao mais rapidos.
- **Padrao**: `dinov2_vitg14`

### `--urepa_encoder_image_size`

- **O que**: Resolucao de entrada para o preprocessamento do encoder.
- **Por que**: Use a resolucao nativa do encoder (518 para DINOv2 ViT-G/14; 224 para ViT-S/14).
- **Padrao**: `518`

### `--urepa_use_tae`

- **O que**: Usa Tiny AutoEncoder em vez do VAE completo para decodificar.
- **Por que**: Mais rapido e economiza VRAM, mas com menor qualidade de decodificacao.
- **Padrao**: `false`

### `--urepa_scheduler`

- **O que**: Agenda de decaimento do coeficiente U-REPA.
- **Por que**: Permite reduzir a forca da regularizacao ao longo do treino.
- **Opcoes**: `constant`, `linear`, `cosine`, `polynomial`
- **Padrao**: `constant`

### `--urepa_warmup_steps`

- **O que**: Numero de passos para aumentar linearmente o peso de 0 ate `urepa_lambda`.
- **Por que**: Warmup ajuda a estabilizar o inicio do treino.
- **Padrao**: `0`

### `--urepa_decay_steps`

- **O que**: Total de passos de decaimento (apos o warmup). 0 significa decair ao longo de todo o treino.
- **Por que**: Controla a duracao do decaimento.
- **Padrao**: `0` (usa `max_train_steps`)

### `--urepa_lambda_end`

- **O que**: Peso final do U-REPA apos o decaimento.
- **Por que**: 0 desativa efetivamente o U-REPA no fim do treino.
- **Padrao**: `0.0`

### `--urepa_power`

- **O que**: Expoente do decaimento polinomial. 1.0 = linear, 2.0 = quadratic, etc.
- **Por que**: Valores maiores decaem mais rapido no inicio e mais lento no fim.
- **Padrao**: `1.0`

### `--urepa_cutoff_step`

- **O que**: Passo de corte apos o qual o U-REPA e desativado.
- **Por que**: Util para desligar apos a convergencia do alinhamento.
- **Padrao**: `0` (sem corte)

### `--urepa_similarity_threshold`

- **O que**: Limiar de similaridade (EMA) para desativar U-REPA.
- **Por que**: Quando a media movel de `urepa_similarity` atinge esse valor, o U-REPA e desativado para evitar overfitting.
- **Padrao**: None (desativado)

### `--urepa_similarity_ema_decay`

- **O que**: Fator de decaimento da media movel exponencial da similaridade.
- **Por que**: Valores maiores suavizam (0.99 ~ 100 passos), menores reagem mais rapido.
- **Padrao**: `0.99`

### `--urepa_threshold_mode`

- **O que**: Comportamento ao atingir o limiar.
- **Opcoes**: `permanent` (desliga para sempre), `recoverable` (reativa se a similaridade cair)
- **Padrao**: `permanent`
...
# urepa_scheduler = "cosine"           # Tipo de decaimento: constant, linear, cosine, polynomial
# urepa_warmup_steps = 100             # Warmup antes do U-REPA
# urepa_decay_steps = 1000             # Passos de decaimento (0 = treino inteiro)
# urepa_lambda_end = 0.0               # Peso final apos o decaimento
# urepa_cutoff_step = 5000             # Corte rigido (0 = desativado)
# urepa_similarity_threshold = 0.9     # Corte baseado em similaridade
# urepa_threshold_mode = "permanent"   # permanent ou recoverable
```
...
XM avalia varios candidatos de treino e retropropaga apenas o candidato que melhor explica cada amostra supervisionada ou bloco de tokens. Cada familia de modelo precisa implementar suporte para o alvo selecionado. Veja [Explorative Modeling](experimental/EXPLORATION_MODELING.pt-BR.md) para o guia do usuário.
...
NextLat adiciona um pequeno preditor auxiliar que mapeia cada token oculto capturado para o proximo token oculto. Requer uma familia transformer que exponha hidden states pelo buffer do SimpleTuner. Veja [NextLat](experimental/NEXTLAT.pt-BR.md) para o guia do usuário.
...
options:
  ...
  --validation_prompt_library [VALIDATION_PROMPT_LIBRARY]
                        Usa a biblioteca de prompts integrada do SimpleTuner.
                        Use "audio" para prompts de musica/audio com caption
                        e letras.
  --user_prompt_library USER_PROMPT_LIBRARY
                        Caminho para uma biblioteca JSON personalizada. As
                        entradas podem usar prompt ou caption, alem de lyrics opcional.
  --eval_dataset_id EVAL_DATASET_ID
