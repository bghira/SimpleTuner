...
- `metal-flash-attention` usa o backend PyTorch custom-op do Universal Metal Flash Attention em Apple Silicon. Instale primeiro o pacote UMFA `examples/pytorch-custom-op-ffi`; chamadas SDPA MPS FP32 4D sem mask elegiveis com pelo menos quatro heads e sequence length 64 ou maior, incluindo layouts transposed estilo FLUX, sao despachadas diretamente por `metal_sdpa_extension`, enquanto outras chamadas passam pelo PyTorch SDPA. Builds UMFA atuais registram a dispatch key PyTorch `MPS`, entao chamadas quantizadas com mask ainda podem chegar ao UMFA pelo dispatcher MPS SDPA do PyTorch; builds antigos `PrivateUse1` sao bypassados por tensores `torch.device("mps")`. SimpleTuner executa verificacoes iniciais de paridade FP32 forward e autograd, e rejeita builds UMFA que nao correspondam numericamente. `metal-flash-attention-int8` e `metal-flash-attention-int4` configuram o modo global de quantizacao blockwise do UMFA com `set_quantization_mode(ext.QUANT_INT8, ext.QUANT_BLOCK_WISE)` ou `set_quantization_mode(ext.QUANT_INT4, ext.QUANT_BLOCK_WISE)`, limpam esse modo ao trocar backends e exigem um check inicial adicional para saidas com autograd, gradientes multi-head finitos, suporte dispatcher-level a bool/additive masks e nenhum fallback PyTorch. Use `metal_sdpa_extension.get_dispatch_stats()` durante uma execucao para confirmar que `quantized_autograd` aumenta e `pytorch_fallback` fica em `0`; bool masks all-true tambem aumentam `mask_all_true_skipped`.
