...
- `metal-flash-attention` usa o backend PyTorch custom-op do Universal Metal Flash Attention em Apple Silicon. Instale primeiro o pacote UMFA `examples/pytorch-custom-op-ffi`; chamadas SDPA MPS FP16/FP32 elegiveis sao despachadas por `metal_sdpa_extension`. SimpleTuner executa uma verificacao inicial de paridade contra PyTorch SDPA e rejeita builds UMFA que nao correspondam numericamente.
