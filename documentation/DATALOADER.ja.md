...
既存の `.bbox` アノテーションがない場合、[Florence-2](https://huggingface.co/microsoft/Florence-2-large) を使用して SimpleTuner に自動生成させることができます。`grounding` 設定内に `auto_detect` ブロックを追加します：
...
| `model` | `microsoft/Florence-2-large` | HuggingFace Florence-2 モデル ID。 |
