...
- **独立してサンプリングされる主要データセットのみに適用** (`image`, `video`, `audio`, `caption`)。`conditioning` はペアとなる補助データであり、独立してサンプリングされる主要データセットではありません。
- **説明:** このデータセットでグローバルな `--train_batch_size` を上書きします。解決後の値は、そのデータセットのサンプラのマイクロバッチサイズ、バケットサイズ、および選択されたときの実際の瞬間サンプル数を制御します。未設定の場合はグローバル値を使います。
...
- **`dataset_type=text_embeds` のみに適用**
- **説明:** 未キャッシュのテキスト埋め込みを事前計算するとき、1 回のテキストエンコーダ forward でエンコードするキャプション数。値を大きくするとスループットは向上しますが、VRAM 使用量が増えます。
- **既定値:** トレーナーの `--text_encoder_batch_size` 引数（既定値 1）にフォールバックします。

### `text_cache_ondemand`

- **`dataset_type=text_embeds` のみに適用**
- **値:** `true` | `false`
- **説明:** テキスト埋め込み全体の事前計算を省略します。既存エントリはこのキャッシュから読み込み、不足する埋め込みは学習または検証時にエンコードしてキャッシュへ書き込みます。
- **注記:** この `text_embeds` バックエンドに割り当てたすべての画像・動画データセットがこの動作を継承します。一部のソースデータセットで異なる動作を使うには、別の `text_embeds` バックエンドを作成し、ソースデータセットの `text_embeds` フィールドで割り当ててください。

### `text_cache_disable`

- **`dataset_type=text_embeds` のみに適用**
- **値:** `true` | `false`
- **説明:** 既存のテキスト埋め込みは読み込みますが、新しくエンコードした埋め込みは書き込みません。不足エントリは学習または検証時にエンコードされるため、`text_cache_ondemand` を暗黙に有効化します。
- **注記:** グローバルな `--text_cache_disable` はすべてのテキストキャッシュに適用されます。グローバル設定が false のとき、選択した `text_embeds` バックエンドだけ書き込みを無効化するにはこの設定を使います。
...
- **値:** `aws` | `local` | `memory` | `csv` | `huggingface` | `webshart`
- **説明:** このデータセットに使用するストレージバックエンド（ローカル、メモリ、csv、またはクラウド）を決定します。

### メモリ（tmpfs および RAM disk）バックエンド

`type: "memory"` は、Linux の tmpfs または macOS ネイティブの RAM disk を使用する `LocalDataBackend` のサブクラス `MemoryDataBackend` を使用します。利用できるのは `dataset_type: "text_embeds"` または `dataset_type: "image_embeds"` のキャッシュストレージエントリだけです。主要な画像、動画、音声、conditioning、評価データセットには使用できません。SimpleTuner は別の空のファイルシステムをマウントし、設定済みの `cache_dir` が存在する場合はそこからコピーします。実行中の読み書きはメモリ上のコピーを使用し、解放時に破棄されます。変更は `cache_dir` に同期されません。
...
- `memory_filesystem_path`: 任意の空のマウントディレクトリ。既定値は OS の一時ディレクトリ内の `simpletuner-memory/<dataset id>` です。このパスと `cache_dir` は重複したり、一方が他方を内包したりしてはいけません。
- `memory_filesystem_size`: `512G` などのメモリファイルシステム容量。Linux では任意で、システムの tmpfs 既定値が適用され、`80%` のような割合も使用できます。macOS では必須で、割合ではなくバイトサイズを指定します。SimpleTuner が `hdiutil` に必要な 512 バイトセクタ数へ変換します。
- `memory_filesystem_sudo`: Linux のみ。`true` の場合、`mount` と `umount` を `sudo -n` 経由で実行します。root 実行時は `false` のままにします。有効にする場合、パスワード不要の非対話 sudo を事前に設定する必要があります。macOS RAM disk はこのオプションなしで `hdiutil` と `diskutil` を使用します。

メモリバックエンドには Linux または macOS と、既存キャッシュおよび学習中に書き込まれるすべての埋め込みを保持できる十分な RAM または swap が必要です。大量メモリ環境で大規模な latent またはテキスト埋め込みキャッシュの反復読み込みがボトルネックになる場合に適しています。既存キャッシュのプリロードは起動時間を増やします。空のキャッシュから開始する場合は、空または存在しない `cache_dir` を使用してください。
...
- **Flow-DPO メモ:** `reference_strict` は [Flow-DPO](experimental/FLOW_DPO.ja.md) でも使用され、preferred 学習サンプルごとの rejected/lose 画像 latent を渡します。
...
- **webshart** は Webshart shard metadata からキャプションを取得します。`type: "webshart"` と `metadata_backend: "webshart"` が必要です。
...
- **値:** `discovery` | `parquet` | `huggingface` | `webshart`
- **説明:** SimpleTuner がデータセット準備中に画像サイズなどのメタデータをどのように取得するかを制御します。
  ...
  - **webshart**: Webshart shard metadata を使い、論理サンプル単位の aspect bucket と captions を画像バイトのスキャンなしで利用します。
...
- **値:** `true` | `false`
- **説明:** `--enable_nsfw_check` が有効な場合、NSFW 分類器で拒否されたサンプルを、このバックエンドが削除に対応していれば削除します。
- **警告:** 破壊的で元に戻せません。Hugging Face データセットは読み取り専用で、現在の実行のメタデータから外されるだけです。
- **既定値:** トレーナーの `--delete_nsfw_images` 引数（既定: `false`）にフォールバックします。

<a id="nsfw-classifier-checks-during-vae-caching"></a>

### VAE キャッシュ中の NSFW 分類器チェック

`--enable_nsfw_check=true` を設定すると、SimpleTuner は VAE キャッシュ前処理中に Hugging Face Transformers の画像分類器で学習サンプルをスキャンできます。スキャンは bucket discovery の後、VAE エンコードの前に行われるため、拒否されたサンプルは latents が書き込まれる前に現在のメタデータ bucket から削除されます。

プライバシー、責任、法的文脈については [NSFW.ja.md](NSFW.ja.md) を参照してください。

このスキャンは、VAE キャッシュがこれから処理する未キャッシュのサンプルだけに適用されます。既存の VAE キャッシュは信頼され、`skip_file_discovery=vae` は NSFW enforcement を回避します。これは、trainer がユーザー自身のポリシーでデータセットが準備済みであるとみなすためです。評価データセットはスキャンされません。

分類器設定:
- `--nsfw_check_models` は標準 Hugging Face Transformers 画像分類モデルの CSV リストです。`model/repo:threshold=0.5` でモデルごとの閾値を指定できます。
- 既定値は `Falconsai/nsfw_image_detection:threshold=0.5,AdamCodd/vit-base-nsfw-detector:threshold=0.5` です。
- `trust_remote_code` は有効化されず、`timm` やカスタムモデルコードが必要な分類器はサポートされません。
- `--nsfw_check_min_votes` は、1 フレームを拒否するために NSFW 判定を返す必要があるモデル数を制御します。

スコープ制御:
- `--nsfw_check_backend_types` は `all`, `local`, `huggingface`, `csv`, `aws` などのバックエンド `type` 値を受け取ります。
- `--nsfw_check_sample_types` はデータセットの `dataset_type` 値を受け取ります。既定値は `image,conditioning` です。動画データセットをスキャンするには `video` を追加します。
- `--delete_nsfw_images=true` は変更可能なバックエンドから拒否サンプルを削除します。無効時は現在の実行のメタデータからのみ外されます。

動画と複数フレームサンプル:
- `--nsfw_check_video_frame_count` はチェックするフレーム数を制御します。
- `--nsfw_check_video_frame_selection` は `uniform`, `first`, `middle` のいずれかです。
- `--nsfw_check_video_min_flagged_frames` はサンプル全体を拒否するために拒否される必要があるチェック済みフレーム数を制御します。

サンプルがスキャンされた場合、各 VAE キャッシュプロセスは VAE キャッシュディレクトリに `nsfw_classifier_report_rank*.json` を書き込みます。レポートにはモデル閾値、分類器の集計判定数、拒否サンプルパス、削除数が含まれます。

バックエンドごとの削除は `multidatabackend.json` で有効化できます。
...
- **nsfw**: VAE キャッシュ前処理中に NSFW 分類器で拒否されたファイル
...
- **値:** `true` | `false`
- **説明:** データセットで有効にすると、不足している VAE 潜在表現を起動時に事前計算せず、学習中にエンコードしてそのデータセットの VAE キャッシュへ書き込みます。
- **注記:** グローバルな `--vae_cache_ondemand` は引き続き全データセットに適用されます。グローバル設定が false のとき、選択したデータセットだけ VAE 潜在表現をオンデマンドでエンコードするには、このデータセット設定を使います。

### `vae_cache_disable`

- **値:** `true` | `false`
- **説明:** データセットで有効化すると、そのデータセットは VAE 潜在をオンデマンドでエンコードし、新しく生成した潜在をディスクに書き込みません。ディスク容量が懸念される大規模データセットや書き込みが難しい環境に有用です。
- **注記:** グローバルな `--vae_cache_disable` は引き続き全データセットに適用されます。グローバル設定が false のとき、選択したデータセットだけ VAE キャッシュ書き込みを無効化するにはこのデータセット設定を使います。削除された `disable_vae_cache` という名前は拒否されるため、`vae_cache_disable` を使ってください。

### VAE キャッシュ方式の選択

`vae_cache_ondemand` は起動時間を短縮する代わりに学習中に処理を行うものであり、VRAM の最適化ではありません。想定されるキャッシュの再利用率と、学習中に VAE を利用可能な状態で維持できるかどうかに応じて、データセットごとに方式を選択してください。

1. **非常に大規模で再利用が見込まれるデータセット:** 完全な事前キャッシュによって学習開始が大幅に遅れる場合は、`vae_cache_ondemand=true` を設定します。全画像の処理を待たずに学習を開始でき、不足している潜在はキャッシュに書き込まれるため、以降のアクセスで再利用できます。
2. **非常に高解像度のデータセット:** 学習モデルがロードされた状態で VAE エンコードすると OOM の可能性がある場合は、`vae_cache_ondemand=false` のままにし、グローバル設定も false にします。既定の起動時事前キャッシュなら、学習前に VAE をアンロードできます。オンデマンドモードと `vae_cache_disable` はどちらも学習中の VAE エンコードを必要とするため、このメモリ制約の解決にはなりません。VAE のタイリングまたはスライシングにより、起動時事前キャッシュのピークメモリを削減できる場合があります。
3. **再利用がほとんど見込めない Web スケールのデータセット:** 数百万から数十億枚の画像があり、予定している学習が 1 エポック近くまで到達しない場合は、`vae_cache_disable=true` を設定します。サンプリングされた画像は引き続きオンデマンドでエンコードされますが、ほとんどの潜在が二度と読み込まれない場合のキャッシュ書き込みとストレージ増加を避けられます。学習中の VAE メモリ要件はオンデマンドキャッシュと同じです。
...
Grounding パイプラインは、エンティティごとのバウンディングボックスとマスクアノテーションを使用したトレーニング時の空間グラウンディングを可能にします。これはマルチサブジェクトファインチューニング時のサブジェクトブリーディング（subject bleeding）を防止するのに役立ちます。

### Grounding の有効化

任意の画像または動画データセットに `grounding` ブロックを追加します：
...
また、`--max_grounding_entities` を 0 より大きい値（例: 8）に設定して Grounding パイプラインを有効にする必要があります。

### バウンディングボックスの自動検出

既存の `.bbox` アノテーションがない場合、[Florence-2](https://huggingface.co/florence-community/Florence-2-large) を使用して SimpleTuner に自動生成させることができます。`grounding` 設定内に `auto_detect` ブロックを追加します：
...
| キー | デフォルト | 説明 |
|-----|---------|-------------|
| `enabled` | `false` | 自動検出を有効にします。 |
| `model` | `florence-community/Florence-2-large` | HuggingFace Florence-2 モデル ID。 |
| `labels` | `[]` | ガイド付き検出用のオプションのエンティティラベルリスト（`<OPEN_VOCABULARY_DETECTION>` を使用）。空の場合、Florence-2 は各画像を自動的にキャプションし、検出したフレーズをグラウンディングします（`<CAPTION>` + `<CAPTION_TO_PHRASE_GROUNDING>` を使用）。 |
| `batch_size` | `4` | 推論バッチあたりの画像数。 |

自動検出はデータセットセットアップ時に一度実行されます。既に `.bbox` サイドカーファイルがある画像はスキップされるため、中断後も安全に再開できます。モデルはアクセラレータデバイスにロードされ、検出完了後すぐに解放されます。

> **注意:** 自動検出は `type: "local"` バックエンドのみサポートしています。

### `.bbox` サイドカーファイル

各画像と同じベース名で `.bbox` ファイルを配置します。

`.bbox` ファイルは3つの形式をサポートしています：JSON 配列、JSON Lines、YOLO txt 形式。

バウンディングボックスの座標は XYXY 形式で [0, 1] に正規化されます。YOLO 形式は中心ベースの XYWH を使用し、自動的に変換されます。`mask` フィールドはオプションです。

### Parquet / HuggingFace bbox カラム
