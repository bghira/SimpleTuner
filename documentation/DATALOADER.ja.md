...
##### `logc3_sdr` / `sdr`
HDR またはリニア画像サンプルから SDR/参照条件画像を生成します。既定の `logc3` transform は、LTX-2 HDR IC-LoRA ツールと同じ ARRI LogC3 圧縮定数を使用します:
```json
{
  "type": "logc3_sdr",
...
```
LogC3 エンコード条件ではなく表示用 SDR プレビューが必要な場合は、`transform: "srgb"` を使用します。`input_scale` は exposure の前に適用され、デコード済みソース値の正規化が必要な場合に使えます。このジェネレータは現在、SimpleTuner の画像バックエンドがデコードできる画像サンプルを対象とします。
