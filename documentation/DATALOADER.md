...
- **webshart** pulls captions from Webshart shard metadata and requires `type: "webshart"` with `metadata_backend: "webshart"`.
...
- **Description:** Controls how SimpleTuner discovers image dimensions and other metadata during dataset preparation.
  ...
  - **webshart**: Uses Webshart shard metadata, including logical-sample aspect buckets and captions, without scanning image bytes.
...
##### `logc3_sdr` / `sdr`
Generates SDR/reference conditioning images from HDR or linear image samples. The default `logc3` transform uses the same ARRI LogC3 compression constants used by LTX-2 HDR IC-LoRA tooling:
```json
{
  "type": "logc3_sdr",
...
```
Use `transform: "srgb"` when you want a display SDR preview instead of LogC3-encoded conditioning. `input_scale` is applied before exposure and is useful when your decoded source values need normalization. This generator currently operates on image samples that the SimpleTuner image backend can decode.
...
- `source` is required and points to a local dataset, URL-backed dataset, or Hugging Face dataset source discoverable by Webshart.
