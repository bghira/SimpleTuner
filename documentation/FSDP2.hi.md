...
FSDP2 मुख्य रूप से memory scaling tool है। जब model, resolution, video duration, या sequence length normal data parallelism में आराम से fit नहीं होता, या long attention sequences के लिए context parallelism चाहिए, तब यह सही विकल्प है। यह multi‑GPU पर अपने आप सबसे तेज़ path नहीं बनता।

LoRA/PEFT training के लिए, यदि model हर GPU पर fit होता है तो DDP को प्राथमिकता दें। 8x H100 पर LTX-2.3 IC-LoRA testing में, हर GPU `train_batch_size=1` वाली normal DDP run उसी छोटे per-rank batch shape के लिए 8x H100 FSDP2 run से लगभग 3-6x तेज़ थी। LoRA में FSDP2 तब उपयोग करें जब विकल्प resolution घटाना, clips छोटे करना, aggressive offload लगाना, या training बिल्कुल न चला पाना हो।

Torch Dynamo भी FSDP2 runs में ज्यादा guard churn देखता है, खासकर dynamic shapes, tokenwise schedules, training/validation shape changes, और regional compile के साथ। Dynamo cache limit hit न होने पर भी recompilation guard messages बार-बार दिख सकते हैं। नई FSDP2 configuration में compile को लाभदायक मानने से पहले `TORCH_LOGS=recompiles` को profiling tool की तरह उपयोग करें।
...
- FSDP2 full-model runs और memory-bound PEFT/LoRA runs के लिए सबसे उपयोगी है। यदि LoRA run DDP में fit होती है, तो DDP आमतौर पर बेहतर throughput देता है।
...
| Compiled FSDP2 runs लगातार Dynamo recompiles log करते हैं | Dynamic shapes या training/validation shape changes guards invalidate कर रहे हैं। | एक बार compile के बिना चलाएँ, या cache limits बढ़ाने से पहले `TORCH_LOGS=recompiles` से changing inputs खोजें। |
