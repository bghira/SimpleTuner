...
- `metal-flash-attention` Apple Silicon पर Universal Metal Flash Attention के PyTorch custom-op backend का उपयोग करता है। पहले UMFA का `examples/pytorch-custom-op-ffi` package install करें; eligible unmasked MPS FP32 4D SDPA calls जिनमें कम से कम चार heads और sequence length 64 या अधिक हो, और जिनमें transposed FLUX-style layouts भी शामिल हैं, `metal_sdpa_extension` पर direct-dispatch होते हैं, जबकि बाकी calls PyTorch SDPA से गुजरते हैं। Current UMFA builds PyTorch `MPS` dispatch key register करते हैं, इसलिए quantized masked calls भी PyTorch के MPS SDPA dispatcher से UMFA तक पहुंच सकते हैं; पुराने `PrivateUse1` builds `torch.device("mps")` tensors से bypass होते हैं। SimpleTuner startup पर FP32 forward और autograd parity checks चलाता है और numerically match न करने वाले UMFA builds को reject करता है। `metal-flash-attention-int8` और `metal-flash-attention-int4` `set_quantization_mode(ext.QUANT_INT8, ext.QUANT_BLOCK_WISE)` या `set_quantization_mode(ext.QUANT_INT4, ext.QUANT_BLOCK_WISE)` से UMFA का global blockwise quantization mode set करते हैं, backend switch करते समय उसे clear करते हैं, और autograd-attached output, finite multi-head gradients, dispatcher-level bool/additive mask support, तथा no PyTorch fallback के लिए अतिरिक्त startup check मांगते हैं। Run के दौरान `metal_sdpa_extension.get_dispatch_stats()` से confirm करें कि `quantized_autograd` बढ़ता है और `pytorch_fallback` `0` रहता है; all-true bool masks में `mask_all_true_skipped` भी बढ़ता है।
