...
- **What**: frozen Cosmos3 reasoner component के लिए Hugging Face repo id या local path।
- **Default**: `auto`
- **Use**: Cosmos3 text cache इस component से per-layer reasoner K/V entries लिखता है।
...
- **Note**: Cosmos3 on-demand text cache force करता है क्योंकि reasoner cache keys में sample geometry शामिल होती है।

### `--cosmos3_generator_component`

- **What**: Cosmos3 generation-only transformer component के लिए Hugging Face repo id या local path।
- **Default**: `auto`
- **Use**: `auto` matching Edge, Nano, Super, Super-I2V, या Super-T2I component चुनता है।
...
- **Note**: यह component transformer के अंदर के reasoner weights हटाता है और reasoner K/V entries require करता है।
...
- **Note**: Flux, Flux.2, Krea 2, LTXVideo2, MageFlow, Z-Image, और Wan segmented paths पर supported। stride interval से कम नहीं हो सकता। [Segmented Checkpointing](experimental/SEGMENTED_CHECKPOINTING.md) देखें।
...
- **Note**: इसे अकेले enable किया जा सकता है। Model जिस भी checkpoint backend को support करे, उसके साथ भी combine किया जा सकता है। Attention offload अभी Chroma, Flux, Flux.2, Krea 2, LTXVideo2, MageFlow, Wan, और Z-Image support करता है; unsupported model families साफ error देती हैं।
...
- `int4-torchao`, `int8-torchao`, `fp8-torchao`, और `fp8wo-torchao` Diffusers के माध्यम से TorchAoConfig का उपयोग करते हैं (CUDA)। `torchao` और recent diffusers/transformers आवश्यक है।
...
- `fp8-native` and `fp8-torchao` के लिए FP8 scaled matmul support वाले Ada Lovelace (RTX 40/L40S), Hopper (H100/H200), या newer accelerators चाहिए
- `fp8-transformerengine` eligible Linear layers को TransformerEngine FP8 modules से बदलता है और model forward को TE FP8 autocast से wrap करता है। `pip install 'simpletuner[transformerengine]'` से install करें; यह preset Ada Lovelace, Hopper, या newer CUDA accelerators के लिए है।
...
- `fp8-sdnq` - SDNQ native FP8 matmul के साथ FP8 weights, H100/H200 class accelerators के लिए
...
- `--sdnq_modules_to_not_convert`, `--sdnq_modules_to_not_use_matmul` - JSON array, file path, या comma-separated module patterns।
- `--sdnq_modules_dtype_dict`, `--sdnq_modules_quant_config` - Per-module dtype और quantization overrides के JSON objects या file paths।
...
- `metal-flash-attention` Apple Silicon पर Universal Metal Flash Attention के PyTorch custom-op backend को उपयोग करता है। पहले UMFA का `examples/pytorch-custom-op-ffi` package install करें; SimpleTuner attention को PyTorch के MPS SDPA dispatcher से route करता है, जिसे मौजूदा UMFA builds register करते हैं। योग्य MPS FP32/FP16/BF16 4D SDPA calls — single-head समेत कोई भी head count, कोई भी sequence length, transposed FLUX-style layouts, 4D तक के bool/additive masks, और causal calls (training समेत — causal backward exact gradient parity पास करता है) — बिना per-call synchronization के PyTorch के MPS command stream में सीधे encode होती हैं, और FP16/BF16 inputs बिना FP32 promotion के native low-precision kernels पर चलते हैं। dropout या `enable_gqa` वाली calls PyTorch SDPA पर fallback करती हैं; पुराने `PrivateUse1` builds `torch.device("mps")` tensors द्वारा bypass हो जाते हैं। SimpleTuner startup पर FP32/FP16/BF16 forward और autograd parity checks तथा causal forward parity चलाता है, और numerically न मिलने वाले या `get_dispatch_stats()` expose न करने वाले UMFA builds को reject करता है। `metal-flash-attention-int8` और `metal-flash-attention-int4` `set_quantization_mode(ext.QUANT_INT8, ext.QUANT_BLOCK_WISE)` या `set_quantization_mode(ext.QUANT_INT4, ext.QUANT_BLOCK_WISE)` से UMFA का global blockwise quantization mode सेट करते हैं, backend बदलने पर उसे clear करते हैं, और attached autograd outputs, finite multi-head gradients, dispatcher-level bool/additive mask support तथा कोई PyTorch fallback न होने की अतिरिक्त startup check मांगते हैं। run के दौरान `metal_sdpa_extension.get_dispatch_stats()` से पुष्टि करें कि `fp32_instream` (quantized aliases के लिए `quantized_autograd`) बढ़ता है और `pytorch_fallback` `0` रहता है; all-true bool masks `mask_all_true_skipped` भी बढ़ाते हैं। extension `rope_scaled_dot_product_attention` भी expose करता है — FLUX.1/FLUX.2/Krea2/Z-Image की interleaved-pair rotary convention से मेल खाता fused RoPE+SDPA entry point; training एक fused autograd से चलती है (backward में dQ/dK पर inverse rotation); masked या GQA gradient calls eager rotation पर fallback करती हैं।
...
- **What**: VAE cache preprocessing के दौरान NSFW classifier enforcement सक्षम करता है।
- **Default**: `false`.
- **Notes**: केवल उन uncached samples को scan किया जाता है जिन्हें VAE cache अभी process करने वाला है। मौजूदा VAE caches और `skip_file_discovery=vae` trusted माने जाते हैं और दुबारा scan नहीं होते।
- **More info**: Privacy और responsibility notes के लिए [NSFW.hi.md](NSFW.hi.md), और VAE cache details के लिए [DATALOADER.hi.md#nsfw-classifier-checks-during-vae-caching](DATALOADER.hi.md#nsfw-classifier-checks-during-vae-caching) देखें।

### `--nsfw_check_models`

- **What**: Hugging Face Transformers image-classification models की CSV list। हर model के साथ `:threshold=0.5` जोड़ा जा सकता है।
- **Default**: `Falconsai/nsfw_image_detection:threshold=0.5,AdamCodd/vit-base-nsfw-detector:threshold=0.5`.
- **Notes**: केवल standard Transformers models supported हैं। `trust_remote_code` enabled नहीं होता, और `timm` पर आधारित classifiers load नहीं किए जाते।

### `--nsfw_check_min_votes`

- **What**: किसी frame को reject करने से पहले NSFW verdict देने वाले configured classifiers की संख्या।
- **Default**: `2`.
- **Notes**: यह कम से कम `1` और configured classifier models की संख्या से अधिक नहीं होना चाहिए।

### `--nsfw_check_backend_types`

- **What**: जिन data backend `type` values को scan करना है उनकी CSV list।
- **Default**: `all`.
- **Example**: `local,huggingface,csv,aws`.

### `--nsfw_check_sample_types`

- **What**: जिन dataset `dataset_type` values को scan करना है उनकी CSV list।
- **Default**: `image,conditioning`.
- **Notes**: Evaluation datasets scan नहीं होते।

### `--delete_nsfw_images`

- **What**: backend deletion support करता हो तो NSFW classifier से rejected source samples को delete करता है।
- **Default**: `false`.
- **Notes**: Disabled होने पर rejected samples current run के metadata से हटते हैं, पर source dataset में बने रहते हैं।

### `--nsfw_check_video_frame_count`

- **What**: video या multi-frame conditioning inputs से sample किए जाने वाले frames की संख्या।
- **Default**: `3`.

### `--nsfw_check_video_frame_selection`

- **What**: video NSFW checks के लिए frame selection mode।
- **Default**: `uniform`.
- **Options**: `uniform`, `first`, `middle`.

### `--nsfw_check_video_min_flagged_frames`

- **What**: पूरे sample को reject करने से पहले reject होने वाले checked video frames की संख्या।
- **Default**: `1`.
...
- **What**: DeepFloyd validation में stage chaining control करता है।
- **Choices**: `auto`, `trained-stage`, `full-pipeline`
- **Default**: `auto`
- **Why**: `auto` prompt validation के लिए DeepFloyd stage I -> stage II चलाता है, जबकि explicit dataset image validation में केवल trained stage चलता है। Single-stage validation बनाए रखने के लिए `trained-stage`, और fixed peer stages force-load करने के लिए `full-pipeline` उपयोग करें।

### `--deepfloyd_validation_stage1_model`

- **What**: Trained stage II को full pipeline से validate करते समय उपयोग होने वाला fixed DeepFloyd stage I model.
- **Default**: `DeepFloyd/IF-I-XL-v1.0`

### `--deepfloyd_validation_stage2_model`

- **What**: Trained stage I को full pipeline से validate करते समय उपयोग होने वाला fixed DeepFloyd stage II model.
- **Default**: `DeepFloyd/IF-II-M-v1.0`

### `--deepfloyd_validation_stage3_mode`

- **What**: DeepFloyd stage II के बाद optional terminal upscaler.
- **Choices**: `none`, `sd-x4-upscaler`
- **Default**: `none`
- **Why**: DeepFloyd का unreleased stage III मूल रूप से 4x upscaler था। `sd-x4-upscaler` उस role के लिए `stabilityai/stable-diffusion-x4-upscaler` उपयोग करता है।

### `--deepfloyd_validation_stage3_model`

- **What**: `--deepfloyd_validation_stage3_mode=sd-x4-upscaler` होने पर उपयोग होने वाला model repository.
- **Default**: `stabilityai/stable-diffusion-x4-upscaler`

### `--deepfloyd_validation_stage1_num_inference_steps`

- **What**: Stage I validation step count का optional override.
- **Default**: `--validation_num_inference_steps` उपयोग करता है, stage I के लिए 30 तक capped.

### `--deepfloyd_validation_stage2_num_inference_steps`

- **What**: Stage II validation step count का optional override.
- **Default**: `--validation_num_inference_steps`

### `--deepfloyd_validation_stage1_guidance`

- **What**: Stage I validation guidance का optional override.
- **Default**: `--validation_guidance`

### `--deepfloyd_validation_stage2_guidance`

- **What**: Stage II validation guidance का optional override.
- **Default**: `--validation_guidance`

### `--deepfloyd_validation_stage3_guidance`

- **What**: SD x4 upscaler guidance का optional override.
- **Default**: `--validation_guidance`
...
- **Choices**: `trained-stage`, `full-pipeline`
- **Default**: `trained-stage`
- **What**: PixArt validation में केवल trained stage चलाना है या v0.7 split pipeline चलानी है, यह चुनता है।
- **Why**: `full-pipeline` stage 1 को `1 - refiner_training_strength` तक latent output के साथ चलाता है, फिर उसी schedule boundary से stage 2 resume करता है।

### `--pixart_validation_stage1_model`

- **What**: Full-pipeline validation में trained stage 2 model को refine कराते समय उपयोग होने वाला fixed PixArt stage 1 model.
- **Default**: `terminusresearch/pixart-900m-1024-ft-v0.7-stage1`

### `--pixart_validation_stage2_model`

- **What**: Full-pipeline validation में trained stage 1 model पहले चलाते समय उपयोग होने वाला fixed PixArt stage 2 model.
- **Default**: `terminusresearch/pixart-900m-1024-ft-v0.7-stage2`
...
- **Format**: entries की array या `runs` array वाला object। हर entry में शामिल हो सकता है:
  ...
  - `target_stage`: इस adapter के लिए optional multi-stage validation target. Supported aliases model पर निर्भर करते हैं; common values में `one`, `two`, `stage1`, `stage2`, और Wan `high`/`low` शामिल हैं.
...
- **Intrinsic target-token conditioning**: चुने गए target video tokens clean latents से copy किए जाते हैं, उनके timesteps `0` किए जाते हैं, और उन्हें video loss से exclude किया जाता है।
  - `ltx2_intrinsic_conditioning`: condition objects का JSON array, जैसे `[{"type":"first_frame","probability":1.0}]`। Supported `type` values: `first_frame`, `prefix`, `suffix`, `spatial_crop`, और `mask`।
  - Shorthand keys: `ltx2_first_frame_conditioning_probability`, `ltx2_prefix_conditioning_probability`, `ltx2_prefix_conditioning_frames`, `ltx2_suffix_conditioning_probability`, `ltx2_suffix_conditioning_frames`, और `ltx2_mask_conditioning_probability`।
  - `mask` के लिए value `1` का अर्थ clean conditioning/no loss है; value `0` normal noisy training है।
- **IC-LoRA reference scaling**: `ltx2_reference_spatial_scale_factor` और `ltx2_reference_temporal_scale_factor` reference-token coordinates adjust करते हैं। unset होने पर spatial scale reference/target latent sizes से infer होता है।
- **IC-LoRA validation reference**: `validation_ltx2_video_conditioning` validation के लिए local reference videos की JSON list है, जैसे `[{"path":"data/reference.mp4","strength":1.0}]`।
- **Scope**: ये settings केवल model-side LTX-2 conditioning behavior control करती हैं। Dataset pairing, mask files, reference datasets, WebUI controls, और dataset templates अलग से configure होते हैं।
...
- **Why**: पेपर DINOv2‑g (ViT‑Giant) उपयोग करता है। छोटे DINO variants कम memory लेते हैं, जबकि Qwen-VL teacher अधिक मजबूत semantic visual features दे सकते हैं।
- **Default**: `dinov2_vitg14`
- **Choices**: `dinov2_vitg14`, `dinov2_vitb14`, `dinov2_vits14`, `qwen3-vl-4b`, `qwen2.5-vl-7b`, या Hugging Face Qwen-VL model id।
...
- **Why**: external vision encoders VRAM बचाने के लिए frames को sliced batches में प्रोसेस कर सकते हैं, गति की कीमत पर।
- **Default**: `-1`
...
crepa_model = "dinov2_vitg14"  # या Qwen-VL visual features के लिए "qwen3-vl-4b"
