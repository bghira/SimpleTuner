...
| Qwen Image 2.1 / Qwen Image | 2.1: 可选; v1.0/v2.0: int8/nf4 | bf16 |
...
| 模型 | Prediction | Flow Shift | TwinFlow | Self-Flow | LayerSync | NextLat | XM | Internal Guidance | Sliders |
| --- | --- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
...
*TwinFlow 在 `twinflow_enabled=true` 时为原生支持；diffusion models 仍需要 `diff2flow_enabled=true` 和 `twinflow_allow_diff2flow=true`。Self-Flow 指 CREPA self-flow support。Internal Guidance 在 diffusion transformer 上使用 auxiliary head。LayerSync 标记公开 hidden states 用于 alignment 的 backbones。NextLat 使用捕获的 hidden states 作为辅助 next-latent 目标。XM 标记 Explorative Modeling support：diffusion models 使用 noise-candidate selection，autoregressive token models 使用 route selection。*
