...
- **训练行为:** 独立采样的主要数据集（`image`、`video`、`audio`、`caption`）使用解析后的值作为训练微批大小。`conditioning` 是配对的辅助数据；其解析值仍可用于确定元数据 bucket 的大小，但不会生成独立的训练微批，也不会单独计入优化器更新的样本总数。
- **说明:** 为此数据集覆盖全局 `--train_batch_size`。对于主要数据集，解析后的值控制采样器微批大小、bucket 大小以及每次被选中时的即时样本数；对于 `conditioning`，它只影响元数据和 bucket 准备。不设置时使用全局值。
...
- **webshart** 从 Webshart shard metadata 中提取字幕，并要求 `type: "webshart"` 与 `metadata_backend: "webshart"`。
...
- **取值:** `discovery` | `parquet` | `huggingface` | `webshart`
- **说明:** 控制 SimpleTuner 在数据集准备阶段如何获取图像尺寸和其他元数据。
  ...
  - **webshart**: 使用 Webshart shard metadata，包括逻辑样本级 aspect bucket 与 captions，无需扫描图像字节。
...
- 将该数据集的流匹配（flow-matching）时间步采样偏移到更高或更低的噪声水平。作用于默认的 logit-normal（sigmoid）调度。
- **负值**会使采样偏向低噪声时间步，让训练聚焦于精细细节（如特写或纹理密集的数据）；**正值**会偏向高噪声时间步，聚焦于整体结构（如全身或构图密集的数据）。
- 该值会加到 sigmoid 前的正态采样上；常用幅度通常在 `-1.0` 到 `1.0` 之间。默认为 `0.0`（无偏置，与原始行为完全一致）。
- 由于每个 batch 都取自单一数据集，请按语义粒度将图像分到不同数据集，并为每个数据集分别设置 `timestep_sampling_offset`。
...
- **取值:** `true` | `false`
- **说明:** 启用 `--enable_nsfw_check` 时，被 NSFW 分类器拒绝的样本会在后端支持删除时从该后端删除。
- **警告:** 此操作不可恢复。Hugging Face 数据集是只读的，只会从当前运行的元数据中移除。
- **默认值:** 回退到训练器的 `--delete_nsfw_images` 参数（默认：`false`）。

<a id="nsfw-classifier-checks-during-vae-caching"></a>

### VAE 缓存期间的 NSFW 分类器检查

设置 `--enable_nsfw_check=true` 后，SimpleTuner 会在 VAE 缓存预处理期间使用 Hugging Face Transformers 图像分类器扫描训练样本。扫描发生在 bucket discovery 之后、VAE 编码之前，因此被拒绝的样本会在写入 latents 之前从当前元数据 bucket 中移除。

隐私、责任和法律背景说明见 [NSFW.zh.md](NSFW.zh.md)。

该扫描只应用于 VAE 缓存即将处理的未缓存样本。已有 VAE 缓存会被信任，`skip_file_discovery=vae` 会绕过 NSFW 强制检查，因为 trainer 会假定数据集已按用户自己的策略准备好。评估数据集不会被扫描。

分类器配置：
- `--nsfw_check_models` 接受标准 Hugging Face Transformers 图像分类模型的 CSV 列表，可用 `model/repo:threshold=0.5` 为每个模型设置阈值。
- 默认值为 `Falconsai/nsfw_image_detection:threshold=0.5,AdamCodd/vit-base-nsfw-detector:threshold=0.5`。
- 不启用 `trust_remote_code`，也不支持需要 `timm` 或自定义模型代码的分类器。
- `--nsfw_check_min_votes` 控制一帧被拒绝前需要多少个模型返回 NSFW 结果。

范围控制：
- `--nsfw_check_backend_types` 接受 `all`、`local`、`huggingface`、`csv`、`aws` 等后端 `type` 值。
- `--nsfw_check_sample_types` 接受数据集 `dataset_type` 值。默认是 `image,conditioning`；添加 `video` 可扫描视频数据集。
- `--delete_nsfw_images=true` 会从可变后端删除被拒绝样本。禁用时，只从当前运行的元数据中移除。

视频和多帧样本：
- `--nsfw_check_video_frame_count` 控制检查的帧数。
- `--nsfw_check_video_frame_selection` 可为 `uniform`、`first` 或 `middle`。
- `--nsfw_check_video_min_flagged_frames` 控制拒绝整个样本前需要被拒绝的已检查帧数。

当有样本被扫描时，每个 VAE 缓存进程都会在 VAE 缓存目录写入 `nsfw_classifier_report_rank*.json` 报告。报告包含模型阈值、分类器结果计数、被拒绝样本路径和删除计数。

可在 `multidatabackend.json` 中为单个后端启用删除：
...
- **nsfw**：VAE 缓存预处理期间被 NSFW 分类器拒绝的文件
- **other**：因其他原因被过滤的文件
...
- **取值:** `true` | `false`
- **说明:** 在数据集上启用后，缺失的 VAE 潜变量不会在启动时预计算，而会在训练期间编码并写入该数据集的 VAE 缓存。
- **注记:** 全局 `--vae_cache_ondemand` 仍会应用于所有数据集。当全局选项为 false 且仅需为选定数据集按需编码 VAE 潜变量时，请使用此数据集选项。

### `vae_cache_disable`

- **取值:** `true` | `false`
- **说明:** 在某个数据集上启用后，该数据集会按需编码 VAE 潜变量，并且不会把新生成的潜变量写入磁盘。这适用于磁盘空间紧张或写入不便的大型数据集。
- **注记:** 全局 `--vae_cache_disable` 仍会应用于所有数据集。当全局选项为 false 时，可使用此数据集选项只为选定数据集禁用 VAE 缓存写入。已移除的 `disable_vae_cache` 写法会被拒绝；请使用 `vae_cache_disable`。

### 选择 VAE 缓存策略

`vae_cache_ondemand` 是用训练期间的额外工作换取更短的启动时间，并不是 VRAM 优化。应根据预期的缓存复用率以及训练期间能否保留 VAE，逐个数据集选择策略：

1. **规模极大且会重复使用的数据集：**当完整的预缓存过程会让训练等待过久时，设置 `vae_cache_ondemand=true`。训练无需等待所有图像处理完毕即可开始，每个缺失的潜变量都会写入缓存，以供后续访问复用。
2. **分辨率非常高的数据集：**如果在训练模型已加载时进行 VAE 编码可能导致 OOM，请保留 `vae_cache_ondemand=false`，并确保全局选项也是 false。默认的启动时预缓存允许在训练开始前卸载 VAE。按需模式和 `vae_cache_disable` 都需要在训练期间进行 VAE 编码，因此都无法解决这一内存限制。VAE tiling 或 slicing 可能降低启动时预缓存的峰值内存。
3. **预期复用率很低的 Web 规模数据集：**如果数据集包含数百万或数十亿张图像，并且计划的训练很可能连一个 epoch 都无法接近，请设置 `vae_cache_disable=true`。采样到的图像仍会按需编码，但当绝大多数潜变量不会再次读取时，可以避免缓存写入和存储空间持续增长。它与按需缓存具有相同的训练期间 VAE 内存要求。
...
Grounding 流水线支持为每个实体添加边界框和遮罩标注，用于训练时的空间定位。这对于在多主体微调中防止主体渗透（subject bleeding）非常有用。

### 启用 grounding

在任意图像或视频数据集中添加 `grounding` 块：
...
还必须将 `--max_grounding_entities` 设置为大于 0 的值（例如 8）以启用 grounding 流水线。

### 自动检测边界框

如果您没有现成的 `.bbox` 标注文件，可以让 SimpleTuner 使用 [Florence-2](https://huggingface.co/florence-community/Florence-2-large) 自动生成。在 `grounding` 配置中添加 `auto_detect` 块：
...
| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `enabled` | `false` | 启用自动检测。 |
| `model` | `florence-community/Florence-2-large` | HuggingFace Florence-2 模型 ID。 |
| `labels` | `[]` | 可选的实体标签列表，用于引导检测（使用 `<OPEN_VOCABULARY_DETECTION>`）。为空时，Florence-2 会自动为每张图像生成描述并定位其中的短语（使用 `<CAPTION>` + `<CAPTION_TO_PHRASE_GROUNDING>`）。 |
| `batch_size` | `4` | 每次推理批次的图像数量。 |

自动检测在数据集设置期间运行一次。已有 `.bbox` 辅助文件的图像会被跳过，因此中断后可以安全恢复。模型加载到加速器设备上，检测完成后立即释放。

> **注意：** 自动检测仅支持 `type: "local"` 后端。

### `.bbox` 辅助文件

在每张图像旁放置同名的 `.bbox` 文件。

`.bbox` 文件支持三种格式：JSON 数组、JSON Lines、YOLO txt 格式。

边界框坐标以 XYXY 格式归一化至 [0, 1]。YOLO 格式使用以中心为基准的 XYWH，会自动转换。`mask` 字段为可选项。

### Parquet / HuggingFace bbox 列
