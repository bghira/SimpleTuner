...
SimpleTuner には自動 GPU ヘルスモニタリングが含まれており、ハードウェア障害を早期に検出します。これは分散学習で特に重要で、単一の GPU 障害がクラスタ全体の計算時間とコストを無駄にする可能性があります。

### GPU サーキットブレーカ

**GPU サーキットブレーカ** は常に有効で、以下を監視します：

- **ECC エラー** - 訂正不可能なメモリエラーを検出（A100/H100 GPU に重要）
- **温度** - サーマルシャットダウン閾値に近づくとアラート
- **スロットリング** - 熱または電力問題によるハードウェアスローダウンを検出
- **CUDA エラー** - トレーニング中のランタイムエラーをキャッチ

GPU 障害が検出されると：

1. `gpu.fault` webhook が送信される（設定されている場合）
2. サーキットブレーカが開き、障害のあるハードウェアでの学習継続を防止
3. トレーニングがクリーンに終了し、オーケストレータがインスタンスを終了できる

### Webhook 設定

`config.json` で webhook を設定して GPU 障害アラートを受信します：
...
Discord アラート用の `webhooks.json` 例：
...
- 各ノードが独自の GPU ヘルスモニターを実行
- いずれかのノードでの GPU 障害がそのノードから webhook をトリガー
- 分散通信の失敗により、すべてのノードで学習ジョブが失敗
- オーケストレータはクラスタ内のすべてのノードからの障害を監視する必要がある

詳細な webhook ペイロード形式とプログラムによるアクセスについては、[レジリエンスインフラストラクチャ](experimental/cloud/RESILIENCE.md#gpu-サーキットブレーカ) を参照してください。
