...
| Qwen Image 2.1 / Qwen Image | 2.1: 任意; v1.0/v2.0: int8/nf4 | bf16 |
...
| モデル | Prediction | Flow Shift | TwinFlow | Self-Flow | LayerSync | NextLat | XM | Internal Guidance | Sliders |
| --- | --- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
...
*TwinFlow は `twinflow_enabled=true` のとき native support です。diffusion models では `diff2flow_enabled=true` と `twinflow_allow_diff2flow=true` も必要です。Self-Flow は CREPA self-flow support です。Internal Guidance は diffusion transformer に auxiliary head を追加します。LayerSync は alignment 用 hidden states を公開する backbone を示します。NextLat はキャプチャした hidden states を auxiliary next-latent objective に使用します。XM は Explorative Modeling support を示し、diffusion models では noise-candidate selection、autoregressive token models では route selection を使います。*
