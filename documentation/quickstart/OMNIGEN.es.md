## Guía rápida de OmniGen

En este ejemplo, entrenaremos un Lycoris LoKr para OmniGen, enfocado en mejoras generales de rendimiento T2I (no entrenamiento de edición/instrucción por ahora).

### Requisitos de hardware

OmniGen es un modelo de tamaño bastante moderado, alrededor de 3.8B parámetros; usa el VAE de SDXL pero no usa un codificador de texto. En su lugar, OmniGen usa IDs de tokens nativos como entradas y se comporta como un modelo multimodal.

El uso de memoria durante el entrenamiento aún no se conoce, pero se espera que quepa fácilmente en una tarjeta de 24G con tamaño de lote 2 o 3. El modelo puede cuantizarse, ahorrando más VRAM.

OmniGen tiene una arquitectura extraña respecto a otros modelos entrenables por SimpleTuner;

- Actualmente, solo se admite entrenamiento t2i (texto‑a‑imagen), donde la salida del modelo se alinea con los prompts de entrenamiento y las imágenes de entrada.
- Aún no se admite entrenamiento imagen‑a‑imagen, pero podría en el futuro.
  - Este modo permite proporcionar una segunda imagen como entrada, y el modelo la usa como condicionamiento/referencia para la salida.
- El valor de pérdida al entrenar OmniGen es muy alto, y no se sabe por qué.
...
- `model_family` - Configúralo en `omnigen`.
- `model_flavour` - Configúralo en `v1`
...
- `validation_resolution` - Este checkpoint de OmniGen es un modelo 1024px, debes configurarlo en `1024x1024` o una de las otras resoluciones compatibles de OmniGen.
  - Se pueden especificar otras resoluciones separándolas con comas: `1024x1024,1280x768,2048x2048`
- `validation_guidance` - Usa el valor que suelas seleccionar en inferencia para OmniGen; un valor más bajo alrededor de 2.5-3.0 da resultados más realistas
...
- `optimizer` - Puedes usar cualquier optimizador con el que te sientas cómodo, pero usaremos `adamw_bf16` para este ejemplo.
...
Dentro de `config/config.json` está el "prompt de validación principal" (`--validation_prompt`), que suele ser el instance_prompt principal en el que estás entrenando para tu único sujeto o estilo. Además, se puede crear un archivo JSON que contiene prompts adicionales para ejecutar durante las validaciones.
...
> ℹ️ OmniGen parece limitarse a alrededor de 122 tokens de comprensión. No se sabe si puede entender más que esto.
...
Actualmente, OmniGen está hardcodeado para usar su propia formulación de flow-matching, y el schedule shift no se aplica.
...
> ℹ️ Se admite ejecutar datasets de 512px y 1024px de forma concurrente, y podría resultar en mejor convergencia.

> ℹ️ OmniGen no genera embeddings de codificador de texto, pero aún se requiere definir uno (por ahora).

Mi configuración para OmniGen fue muy básica y se veía así, ya que usé entrenamiento con pérdida de evaluación estable:
...
La configuración de VRAM más baja de OmniGen aún no se conoce, pero se espera que sea similar a lo siguiente:

- OS: Ubuntu Linux 24
- GPU: Un solo dispositivo NVIDIA CUDA (10G, 12G)
- Memoria del sistema: alrededor de 50G de memoria del sistema
- Precisión del modelo base: `int8-quanto` (o `fp8-torchao`, `int8-torchao` siguen perfiles de uso de memoria similares)
...
**NOTA**: El pre-caché de embeddings del VAE y salidas del codificador de texto puede usar más memoria y aun así producir OOM. Si sucede, se pueden habilitar tiling y slicing del VAE. Los codificadores de texto pueden descargarse a CPU durante el caché del VAE con `offload_during_startup=true`. Para evitar uso de disco del caché VAE en datasets grandes, usa `--vae_cache_disable`.

La velocidad fue de aproximadamente 3.4 iteraciones por segundo en una AMD 7900XTX usando Pytorch 2.7 y ROCm 6.3.
...
- Configurar `is_regularisation_data` tiene impacto/efecto desconocido con OmniGen (no probado)

### Artefactos de imagen

OmniGen tiene una respuesta desconocida a artefactos de imagen, aunque usa el VAE de SDXL y tiene limitaciones idénticas de detalle fino.
...
### Valores de pérdida altos

OmniGen tiene un valor de pérdida muy alto, y no se sabe por qué. Se recomienda ignorar el valor de pérdida y enfocarse en la calidad visual de las imágenes generadas.
