## OmniGen क्विकस्टार्ट

इस उदाहरण में, हम OmniGen के लिए Lycoris LoKr प्रशिक्षण करेंगे, जिसका फोकस सामान्य T2I प्रदर्शन सुधार पर है (अभी edit/instruct प्रशिक्षण नहीं)।

### हार्डवेयर आवश्यकताएँ

OmniGen लगभग 3.8B पैरामीटर का मध्यम आकार का मॉडल है; यह SDXL VAE का उपयोग करता है लेकिन टेक्स्ट एन्कोडर उपयोग नहीं करता। इसके बजाय, OmniGen native token IDs को इनपुट के रूप में लेता है और multi‑modal मॉडल की तरह व्यवहार करता है।

ट्रेनिंग के दौरान मेमोरी उपयोग अभी ज्ञात नहीं है, लेकिन batch size 2 या 3 के साथ 24G कार्ड पर आसानी से फिट होने की उम्मीद है। मॉडल को quantise किया जा सकता है, जिससे VRAM और बचती है।

OmniGen की आर्किटेक्चर अन्य SimpleTuner‑trainable मॉडलों से अलग है;

- वर्तमान में केवल t2i (text‑to‑image) प्रशिक्षण समर्थित है, जहाँ मॉडल आउटपुट प्रशिक्षण prompts और इनपुट images से align होता है।
- image‑to‑image प्रशिक्षण अभी समर्थित नहीं है, लेकिन भविष्य में हो सकता है।
  - इस मोड में दूसरी इमेज इनपुट दी जाती है, और मॉडल इसे conditioning/reference डेटा के रूप में उपयोग करता है।
- OmniGen ट्रेनिंग के दौरान loss मान बहुत ऊँचा होता है, और इसका कारण ज्ञात नहीं है।
...
- `model_family` - इसे `omnigen` पर सेट करें।
- `model_flavour` - इसे `v1` पर सेट करें।
...
- `validation_resolution` - OmniGen का यह checkpoint 1024px मॉडल है; इसे `1024x1024` या अन्य समर्थित resolutions पर सेट करें।
  - अन्य resolutions को कॉमा से अलग कर सकते हैं: `1024x1024,1280x768,2048x2048`
- `validation_guidance` - OmniGen inference में जिस मान के साथ आप सहज हों, वही रखें; 2.5‑3.0 के आसपास का कम मान अधिक यथार्थवादी परिणाम देता है।
...
- `optimizer` - आप कोई भी optimiser उपयोग कर सकते हैं जिसे आप जानते हों, लेकिन इस उदाहरण में हम `adamw_bf16` इस्तेमाल करेंगे।
...
और एक सरल `config/lycoris_config.json` फ़ाइल — ध्यान दें कि अतिरिक्त training stability के लिए `FeedForward` हटाया जा सकता है।
...
`config/config.json` के अंदर "primary validation prompt" (`--validation_prompt`) होता है, जो आमतौर पर आपके single subject या style के लिए मुख्य instance_prompt होता है। इसके अतिरिक्त, एक JSON फ़ाइल बनाई जा सकती है जिसमें वैलिडेशन के दौरान चलाने के लिए अतिरिक्त प्रॉम्प्ट्स हों।
...
> ℹ️ OmniGen लगभग 122 tokens के आसपास समझ तक सीमित लगता है। यह स्पष्ट नहीं कि यह इससे अधिक समझ पाता है या नहीं।
...
वर्तमान में OmniGen अपने विशेष flow‑matching formulation का उपयोग करने के लिए hard‑coded है, और schedule shift उस पर लागू नहीं होता।
...
अपने मॉडल को प्रशिक्षित करने के लिए पर्याप्त बड़ा डेटासेट होना महत्वपूर्ण है। डेटासेट आकार पर सीमाएँ हैं, और आपको सुनिश्चित करना होगा कि आपका डेटासेट पर्याप्त बड़ा हो। ध्यान दें कि न्यूनतम डेटासेट आकार `train_batch_size * gradient_accumulation_steps` और `vae_batch_size` से अधिक होना चाहिए। यदि डेटासेट बहुत छोटा है, तो वह उपयोग योग्य नहीं होगा।
...
> ℹ️ 512px और 1024px datasets को साथ चलाना समर्थित है, और इससे बेहतर convergence हो सकता है।

> ℹ️ OmniGen text encoder embeds नहीं बनाता, लेकिन एक को परिभाषित करना अभी भी आवश्यक है (फिलहाल)।

OmniGen के लिए मेरा कॉन्फ़िग बहुत basic था, और stable eval loss प्रशिक्षण सेट का उपयोग करते हुए ऐसा दिखा:
...
सबसे कम VRAM वाला OmniGen कॉन्फ़िग अभी ज्ञात नहीं है, लेकिन उम्मीद है कि यह लगभग इस जैसा होगा:

- OS: Ubuntu Linux 24
- GPU: एक NVIDIA CUDA डिवाइस (10G, 12G)
- System memory: लगभग 50G सिस्टम मेमोरी
- Base model precision: `int8-quanto` (या `fp8-torchao`, `int8-torchao` — सभी समान मेमोरी प्रोफ़ाइल रखते हैं)
...
**नोट**: VAE embeds और text encoder outputs की pre‑caching अधिक मेमोरी ले सकती है और फिर भी OOM हो सकता है। ऐसा हो तो VAE tiling और slicing वैकल्पिक रूप से सक्षम की जा सकती है। VAE caching के दौरान text encoders को CPU पर ऑफ़लोड किया जा सकता है (`offload_during_startup=true`). बड़े डेटासेट्स में VAE cache के लिए डिस्क उपयोग से बचने हेतु `--vae_cache_disable` उपयोग करें।

AMD 7900XTX पर Pytorch 2.7 और ROCm 6.3 के साथ गति लगभग 3.4 iterations per second थी।
...
- `is_regularisation_data` का OmniGen के साथ प्रभाव अज्ञात है (टेस्ट नहीं हुआ)

### Image artifacts

OmniGen का image artifacts पर response अज्ञात है, हालांकि यह SDXL VAE उपयोग करता है और fine‑details की समान सीमाएँ हैं।
...
OmniGen में loss मान बहुत ऊँचा होता है, और इसका कारण ज्ञात नहीं है। loss मान को नज़रअंदाज़ करके generated images की दृश्य गुणवत्ता पर ध्यान देने की सलाह दी जाती है।
