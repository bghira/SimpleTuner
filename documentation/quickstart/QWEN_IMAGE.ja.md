...
### 補助アダプターと REPA のプリセット

標準例は [補助アダプター v2](https://huggingface.co/SimpleTuner/Qwen-Image-2.1-training-assistant-v2)、合成データによる正則化、REPA、解像度に応じた自動フローシフトを組み合わせます。従来の 250 ステップ速度測定用設定を置き換えます。画像比較は[実験集](https://huggingface.co/SimpleTuner/Qwen-Image-2.1-LoRA-experiments)を参照してください。以前の速度測定値はこの設定には適用できません。
...
`webshart` をインストールしてください。各合成データの縦横比サブセットは最大 1,024 枚で、個別の潜在キャッシュを使います。VAE タイリングは無効、検証は既定のテクスチャ修正版 VAE を使います。データ・解像度・バッチを変更するときは新規学習を開始してください。

24/32 GB プリセットはメモリ余裕のため 1024px を含めません。`qwen_image.peft-lora` は多解像度です。以前の測定ではこの組み合わせのメモリ量は確認できず、コンパイルやキャプション長も影響します。AnyFlow と補助アダプター作成は別の実験です。
...
全段階で 1024px、BF16、AdamW、rank 32、batch 4 を使用します。段階 1 は H200 で勾配チェックポイントを無効にし、段階 2 と 3 は連続 2 ブロック単位で適用します。上記の補助 REPA プリセットとは別の蒸留学習です。実行前に `webshart` をインストールしてください。
