...
यह गाइड FLUX.2 पर LoRAs ट्रेन करने को कवर करती है, जो Black Forest Labs की नवीनतम इमेज जनरेशन मॉडल फैमिली है।

> **नोट**: डिफ़ॉल्ट मॉडल फ्लेवर `klein-9b` है, लेकिन यह गाइड `dev` (24B Mistral-3 टेक्स्ट एन्कोडर वाला पूर्ण 12B transformer) पर केंद्रित है क्योंकि इसकी संसाधन आवश्यकताएँ सबसे अधिक हैं। Klein मॉडल चलाना आसान है - नीचे [मॉडल वेरिएंट](#मॉडल-वेरिएंट) देखें।

## मॉडल वेरिएंट

FLUX.2 तीन वेरिएंट में आता है:

| वेरिएंट | Transformer | Text Encoder | कुल ब्लॉक | डिफ़ॉल्ट |
|---------|-------------|--------------|-----------|---------|
| `dev` | 12B पैरामीटर | Mistral-3 (24B) | 56 (8+48) | |
| `klein-9b` | 9B पैरामीटर | Qwen3 (शामिल) | 32 (8+24) | ✓ |
| `klein-4b` | 4B पैरामीटर | Qwen3 (शामिल) | 25 (5+20) | |

**मुख्य अंतर:**
- **dev**: स्टैंडअलोन Mistral-Small-3.1-24B टेक्स्ट एन्कोडर का उपयोग करता है, guidance embeddings हैं
- **klein मॉडल**: मॉडल रिपॉजिटरी में शामिल Qwen3 टेक्स्ट एन्कोडर का उपयोग करते हैं, **guidance embeddings नहीं** (guidance ट्रेनिंग विकल्प अनदेखे होते हैं)

वेरिएंट चुनने के लिए, अपनी config में `model_flavour` सेट करें:
```json
{
  "model_flavour": "dev"
}
...
- **Text Encoder**: Mistral-Small-3.1-24B (dev) या Qwen3 (klein)
...
- **Embedding Dimension**: dev के लिए 15,360 (3×5,120), klein-9b के लिए 12,288 (3×4,096), klein-4b के लिए 7,680 (3×2,560)

## हार्डवेयर आवश्यकताएँ

हार्डवेयर आवश्यकताएँ मॉडल वेरिएंट के अनुसार काफी भिन्न होती हैं।

### Klein मॉडल (अधिकांश उपयोगकर्ताओं के लिए अनुशंसित)

Klein मॉडल अधिक सुलभ हैं:

| वेरिएंट | bf16 VRAM | int8 VRAM | System RAM |
|---------|-----------|-----------|------------|
| `klein-4b` | ~12GB | ~8GB | 32GB+ |
| `klein-9b` | ~22GB | ~14GB | 64GB+ |

**klein-9b के लिए अनुशंसित**: एकल 24GB GPU (RTX 3090/4090, A5000)
**klein-4b के लिए अनुशंसित**: एकल 16GB GPU (RTX 4080, A4000)

### FLUX.2-dev (उन्नत)

Mistral-3 टेक्स्ट एन्कोडर के कारण FLUX.2-dev को काफी संसाधन चाहिए:
...
Mistral-3 टेक्स्ट एन्कोडर और transformer के संयुक्त आकार के कारण FLUX.2-dev के लिए multi-GPU distributed training (FSDP2 या DeepSpeed) लगभग अनिवार्य है।
...
FLUX.2 मॉडल के लिए Hugging Face पर access approval चाहिए:
...
**klein मॉडल के लिए:**
1. [black-forest-labs/FLUX.2-klein-base-9B](https://huggingface.co/black-forest-labs/FLUX.2-klein-base-9B) या [black-forest-labs/FLUX.2-klein-base-4B](https://huggingface.co/black-forest-labs/FLUX.2-klein-base-4B) पर जाएँ
2. License agreement स्वीकार करें

सुनिश्चित करें कि आप Hugging Face CLI में लॉगिन हैं: `huggingface-cli login`
...
> **नोट**: Klein मॉडल (`klein-4b`, `klein-9b`) में guidance embeddings नहीं होते। निम्नलिखित guidance विकल्प केवल `dev` पर लागू होते हैं।

FLUX.2-dev, FLUX.1 की तरह guidance embedding उपयोग करता है:
...
|--------|--------|------------|-----------------|-----------------|
| Text Encoder | CLIP-L/14 + T5-XXL | Mistral-3 (24B) | Qwen3 (शामिल) | Qwen3 (शामिल) |
...
| Guidance Embeds | हाँ | हाँ | नहीं | नहीं |
