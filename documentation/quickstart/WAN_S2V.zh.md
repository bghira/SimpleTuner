...
S2V 训练需要配对的视频和音频数据。默认情况下，SimpleTuner 会从视频数据集中自动拆分音频，因此除非你需要自定义处理，
否则无需单独的音频数据集。将 `audio.auto_split: false` 关闭，并手动配置 `s2v_datasets` 即可。
...
如果你的视频已经包含音轨，SimpleTuner 可以自动提取和处理音频，无需单独的音频数据集。这是最简单且默认的方法：
...
启用音频 auto-split（默认）后，SimpleTuner 将：
...
**音频配置选项：**
- `audio.auto_split`（布尔值）：启用从视频自动提取音频（默认：true）
...
如果你更喜欢使用单独的音频文件、需要自定义音频处理，或关闭了 auto-split，S2V 模型也可以使用与视频文件按文件名匹配的预提取音频文件。例如：
