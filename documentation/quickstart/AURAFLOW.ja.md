...
この例では、Auraflow の Lycoris LoKr をトレーニングします。

このモデルのフルファインチューニングは 6B パラメータのため大量の VRAM が必要で、動作させるには [DeepSpeed](../DEEPSPEED.md) を使う必要があります。

### ハードウェア要件

Auraflow v0.3 は 6B パラメータの MMDiT としてリリースされ、エンコード済みテキスト表現に Pile T5、潜在画像表現に 4ch SDXL VAE を使用します。
...
- `model_family` - `auraflow` に設定します。
- `model_flavour` - `pony` に設定するか、既定モデルを使う場合は未設定のままにします。
- `output_dir` - チェックポイントと検証画像を保存するディレクトリに設定します。フルパスの使用を推奨します。
- `train_batch_size` - 24G カードなら 1〜4 が動作します。
- `validation_resolution` - `1024x1024` または Auraflow がサポートする他の解像度に設定します。
  - 他の解像度はカンマ区切りで指定できます: `1024x1024,1280x768,1536x1536`
  - Auraflow の位置埋め込みは少し癖があり、マルチスケール画像（複数の基準解像度）での学習結果は不確実です。
- `validation_guidance` - 推論時に慣れている値を使用します。3.5〜4.0 付近の低い値ほど現実的な結果になります。
- `validation_num_inference_steps` - おおよそ 30〜50 を使用します。
- `use_ema` - `true` に設定すると、メインの学習済みチェックポイントに加えてより滑らかな結果が得られます。

- `optimizer` - 使い慣れたオプティマイザを使用できますが、この例では `optimi-lion` を使います。
  - Pony Flow の作者は、最も問題が少なく安定した学習結果になるとして `adamw_bf16` を推奨しています。
  - このデモではより速く学習が進むよう Lion を使いますが、長期実行では `adamw_bf16` が安全です。
- `learning_rate` - Lion + Lycoris LoKr の場合、`4e-5` が良い出発点です。
  - `adamw_bf16` を使うなら、学習率は約 10 倍（`2.5e-4`）が目安です。
  - 小さな Lycoris/LoRA ランクは **高い学習率** を必要とし、大きなランクは **低い学習率** が必要です。
...
そしてシンプルな `config/lycoris_config.json`:
...
> ℹ️ Auraflow は 128 トークンにデフォルトで切り詰めます。
...
Auraflow の最低 VRAM 構成は約 20〜22G です:
...
**注**: VAE 埋め込みとテキストエンコーダ出力の事前キャッシュは、より多くのメモリを使用して OOM になる場合があります。VAE タイリングとスライシングは既定で有効です。OOM が出る場合は `offload_during_startup=true` を有効にする必要があるかもしれません。そうでなければ厳しいかもしれません。
...
Auraflow は `int4` 精度でも比較的うまく動作しますが、`bf16` が使えない場合の品質/安定性のスイートスポットは `int8` です。
...
- 他のアルゴはさらなる検証が必要です。
- `is_regularisation_data` の設定は Auraflow での影響が不明です（未テストですが、問題ないはずです）。

### 画像アーティファクト

Auraflow は画像アーティファクトへの反応が不明ですが、Flux VAE を使用しており、細部の制約は類似しています。

問題が出る場合は、GitHub に issue を立ててください。

### アスペクトバケッティング

モデルのパッチ埋め込み実装の制約により、エラーになる解像度がいくつか存在します。

実験と詳細なバグ報告が有用です。

### フルランクチューニング

DeepSpeed は Auraflow で大量のシステムメモリを使用し、フルチューニングは概念学習や崩壊回避の面で期待通りに動かない可能性があります。
