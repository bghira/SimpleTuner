## Kwai Kolors クイックスタート

この例では、SimpleTuner ツールキットを使用して Kwai Kolors モデルをトレーニングし、`lora` モデルタイプを使用します。

Kolors は SDXL とほぼ同じサイズのため、`full` トレーニングも試せますが、その変更点はこのクイックスタートでは説明しません。
...
- `pretrained_model_name_or_path` - `Kwai-Kolors/Kolors-diffusers` に設定します。
- `MODEL_TYPE` - `lora` に設定します。
- `USE_DORA` - DoRA をトレーニングしたい場合は `true` に設定します。
- `MODEL_FAMILY` - `kolors` に設定します。
- `OUTPUT_DIR` - チェックポイントと検証画像を保存するディレクトリに設定します。フルパスの使用を推奨します。
- `VALIDATION_RESOLUTION` - この例では `1024x1024` に設定します。
  - Kolors はマルチアスペクトバケットでファインチューニングされているため、カンマ区切りで他の解像度を指定できます: `1024x1024,1280x768`
- `VALIDATION_GUIDANCE` - 推論時に慣れている値を使用します。`4.2`〜`6.4` の範囲に設定します。
- `USE_GRADIENT_CHECKPOINTING` - 大量の VRAM があり速度のために一部を犠牲にしたい場合を除き、`true` が推奨です。
- `LEARNING_RATE` - `1e-4` は低ランクネットワークで一般的ですが、「焼き付き」や早期の過学習に気づいた場合は `1e-5` がより保守的です。

Mac M-series を使用している場合の追加設定:

- `mixed_precision` を `no` に設定します。
- `attention_mechanism` は `diffusers` に設定します。`xformers` などは動作しない可能性があります。

#### 量子化モデルトレーニング

Apple および NVIDIA システムでテスト済みで、Hugging Face Optimum-Quanto により特に ChatGLM 6B（テキストエンコーダ）の精度と VRAM 要件を削減できます。
...
*   **[Diff2Flow](../experimental/DIFF2FLOW.md):** Flow Matching 目的で Kolors をトレーニングし、生成の直線性と品質を改善する可能性があります。
...
`OUTPUT_DIR` ディレクトリに multidatabackend.json を作成します:
