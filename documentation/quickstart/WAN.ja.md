...
- `wan_validation_load_other_stage=true`を切り替えると、トレーニングしているステージとは反対のステージを検証レンダリング用に読み込みます。
  - validation は完全な paired-stage pipeline を使い、Wan 2.2 の境界で denoiser を切り替えます。そのため high-stage / low-stage LoRA を推論時と同じ形で確認できます。AnimeGen など互換 staged flavour も同じ validation path を使います。
- 標準のWan 2.1 text-to-video実行には、flavourを未設定のままにする(または`t2v-480p-1.3b-2.1`を使用)してください。

#### 画像データセットによるI2V検証

i2v検証には、完全なコンディショニングデータセットのペアリングなしで、シンプルな画像データセットを使用できます：
...
これにより、指定されたデータセットの画像が検証動画生成の最初のフレームコンディショニング入力として使用されます。トレーニング時に使用される複雑なコンディショニング設定は不要です。
