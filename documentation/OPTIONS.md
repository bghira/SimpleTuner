...
- `metal-flash-attention` wraps the Universal Metal Flash Attention PyTorch custom-op backend on Apple Silicon. Install the UMFA `examples/pytorch-custom-op-ffi` package first; eligible unmasked MPS FP32 4D SDPA calls with at least four heads and sequence length 64 or larger, including transposed FLUX-style layouts, direct-dispatch through `metal_sdpa_extension`, while other calls go through PyTorch SDPA. Current UMFA builds register the PyTorch `MPS` dispatch key, so quantized masked calls can still reach UMFA through PyTorch's MPS SDPA dispatcher; older `PrivateUse1` builds are bypassed by `torch.device("mps")` tensors. SimpleTuner runs startup FP32 forward and autograd parity checks and rejects UMFA builds that do not match numerically. `metal-flash-attention-int8` and `metal-flash-attention-int4` set UMFA's global blockwise quantization mode with `set_quantization_mode(ext.QUANT_INT8, ext.QUANT_BLOCK_WISE)` or `set_quantization_mode(ext.QUANT_INT4, ext.QUANT_BLOCK_WISE)`, clear it when switching backends, and require an additional startup check for attached autograd outputs, finite multi-head gradients, dispatcher-level bool/additive mask support, and no PyTorch fallback. Use `metal_sdpa_extension.get_dispatch_stats()` during a run to confirm `quantized_autograd` increases and `pytorch_fallback` stays at `0`; all-true bool masks also increase `mask_all_true_skipped`.
