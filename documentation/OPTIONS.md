...
- `metal-flash-attention` wraps the Universal Metal Flash Attention PyTorch custom-op backend on Apple Silicon. Install the UMFA `examples/pytorch-custom-op-ffi` package first; eligible MPS FP32 4D SDPA calls with at least four heads and sequence length 64 or larger, including transposed FLUX-style layouts, dispatch through `metal_sdpa_extension`, while FP16/BF16, masked, causal, tiny, and 2D calls fall back to PyTorch SDPA. SimpleTuner runs startup FP32 forward and autograd parity checks and rejects UMFA builds that do not match numerically.
