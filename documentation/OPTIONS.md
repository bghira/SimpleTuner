...
- `metal-flash-attention` wraps the Universal Metal Flash Attention PyTorch custom-op backend on Apple Silicon. Install the UMFA `examples/pytorch-custom-op-ffi` package first; eligible MPS FP16/FP32 SDPA calls dispatch through `metal_sdpa_extension`. SimpleTuner runs a startup parity check against PyTorch SDPA and rejects UMFA builds that do not match numerically.
