...
- **Aplica-se apenas a datasets primários amostrados de forma independente** (`image`, `video`, `audio`, `caption`). `conditioning` é um dado auxiliar pareado, não um dataset primário amostrado de forma independente.
- **Descrição:** Sobrescreve o `--train_batch_size` global para este dataset. O valor resolvido controla o tamanho do microbatch do sampler, o dimensionamento dos buckets e a quantidade instantânea de amostras sempre que o dataset é selecionado. Deixe sem definir para usar o valor global.
...
- **Aplica-se apenas a `dataset_type=text_embeds`**
- **Valores:** `true` | `false`
- **Descrição:** Ignora a pré-computação completa dos embeddings de texto. Entradas existentes são lidas deste cache; embeddings ausentes são codificados durante o treinamento ou a validação e gravados no cache.
- **Nota:** Todo dataset de imagem ou vídeo associado a este backend `text_embeds` herda esse comportamento. Para usar outro comportamento em datasets de origem selecionados, crie outro backend `text_embeds` e associe-o pelo campo `text_embeds` do dataset de origem.

### `text_cache_disable`

- **Aplica-se apenas a `dataset_type=text_embeds`**
- **Valores:** `true` | `false`
- **Descrição:** Lê embeddings de texto existentes, mas não grava embeddings recém-codificados. Entradas ausentes são codificadas durante o treinamento ou a validação; portanto, esta opção implica `text_cache_ondemand`.
- **Nota:** A opção global `--text_cache_disable` aplica-se a todos os caches de texto. Use esta opção de backend quando a opção global estiver falsa e apenas backends `text_embeds` selecionados não devam gravar.
...
O bloco `audio` pode ser colocado em datasets de **vídeo** para extrair áudio automaticamente dos arquivos de vídeo. Para modelos que requerem treinamento S2V (Sound-to-Video) (ex: Wan S2V), este bloco é injetado automaticamente. Para modelos que suportam mas não exigem áudio (ex: LTX-2), você deve adicionar uma seção `audio` explicitamente para optar — sem ela, apenas vídeo é treinado e a loss de áudio é mascarada automaticamente.
...
Isso cria automaticamente um dataset `my-videos_audio` e o vincula via `s2v_datasets`.
...
| `audio.duration_interval` | float | 3.0 | Intervalo de duração para agrupamento em buckets (segundos) |
| `audio.max_duration_seconds` | float | null | Duração máxima de áudio (arquivos mais longos são ignorados) |
| `audio.truncation_mode` | string | "beginning" | Como truncar áudio longo: "beginning", "end", "random" |
...
- **Descrição:** Determina o backend de armazenamento (local, memória, csv ou nuvem) usado para este dataset.

### Backends em memória (tmpfs e RAM disk)

`type: "memory"` usa `MemoryDataBackend`, uma subclasse de `LocalDataBackend` baseada em tmpfs no Linux ou em um RAM disk nativo no macOS. Ele está disponível apenas para entradas de armazenamento de cache com `dataset_type: "text_embeds"` ou `dataset_type: "image_embeds"`; datasets primários de imagem, vídeo, áudio, conditioning e avaliação não podem usá-lo. O SimpleTuner monta um sistema de arquivos vazio separado e copia o `cache_dir` configurado quando esse diretório existe. Leituras e gravações usam a cópia residente em memória e são descartadas ao liberá-la; elas não são sincronizadas de volta para `cache_dir`.
...
- `memory_filesystem_path`: Diretório de montagem vazio opcional. O padrão é `simpletuner-memory/<id do dataset>` dentro do diretório temporário do sistema. Esse diretório e `cache_dir` não podem se sobrepor nem conter um ao outro.
- `memory_filesystem_size`: Capacidade do sistema de arquivos em memória, como `512G`. É opcional no Linux, onde se aplica o padrão do tmpfs e percentuais como `80%` são aceitos. No macOS é obrigatório e deve ser um tamanho em bytes, não um percentual; o SimpleTuner o converte para o número de setores de 512 bytes exigido pelo `hdiutil`.
- `memory_filesystem_sudo`: Apenas Linux. Quando `true`, executa `mount` e `umount` por `sudo -n`. Deixe `false` ao executar como root. O sudo não interativo e sem senha deve estar configurado previamente quando habilitado. RAM disks do macOS usam `hdiutil` e `diskutil` sem esta opção.

Backends de memória exigem Linux ou macOS e RAM ou swap suficiente para o cache existente e todos os embeddings gravados durante o treinamento. São úteis para caches grandes de latentes ou embeddings de texto em sistemas com muita memória quando leituras repetidas são o gargalo. Pré-carregar um cache existente aumenta o tempo de inicialização; use um `cache_dir` vazio ou inexistente para iniciar com um cache vazio.
...
- **Nota Flow-DPO:** `reference_strict` também é usado pelo [Flow-DPO](experimental/FLOW_DPO.pt-BR.md) para fornecer os latentes rejected/lose de cada amostra preferred.
...
##### `sdr` / `logc3_sdr`
Gera imagens SDR/de referência de condicionamento para datasets de condicionamento pareados. O transform `rec709` padrão normaliza e recorta entradas Rec.709 SDR já existentes, igual ao caminho de condicionamento de referência HDR IC-LoRA do LTX-2:
```json
{
  "type": "sdr",
...
```
Use `transform: "srgb"` quando os valores decodificados forem lineares e você quiser um proxy SDR de tela. Use `transform: "logc3"` (ou o alias `logc3_sdr`) apenas quando quiser amostras codificadas em LogC3. `input_scale` é aplicado antes da exposure e é útil quando os valores decodificados precisam de normalização. Este gerador atualmente opera em amostras de imagem que o backend de imagens do SimpleTuner consegue decodificar.
...
- **Valores:** `true` | `false`
- **Descrição:** Quando habilitado em um dataset, latentes VAE ausentes são codificados durante o treinamento e gravados no cache VAE desse dataset, em vez de serem pré-computados na inicialização.
- **Nota:** A opção global `--vae_cache_ondemand` ainda se aplica a todos os datasets. Use esta opção de dataset quando a opção global estiver falsa e apenas datasets selecionados devam codificar latentes VAE sob demanda.
...
## Grounding (Anotacoes Espaciais)

O pipeline de grounding permite anotacoes de bounding box e mascara por entidade para grounding espacial durante o treinamento. Isso e util para prevenir o sangramento de sujeitos (subject bleeding) durante o fine-tuning com multiplos sujeitos.

### Habilitando o grounding

Adicione um bloco `grounding` a qualquer dataset de imagem ou video:
...
Voce tambem deve definir `--max_grounding_entities` para um valor maior que 0 (por exemplo, 8) para habilitar o pipeline de grounding.

### Auto-deteccao de bounding boxes

Se voce nao possui anotacoes `.bbox` pre-existentes, pode deixar o SimpleTuner gera-las automaticamente usando [Florence-2](https://huggingface.co/florence-community/Florence-2-large). Adicione um bloco `auto_detect` dentro da configuracao `grounding`:
...
|-----|---------|-------------|
| `enabled` | `false` | Habilitar auto-deteccao. |
| `model` | `florence-community/Florence-2-large` | ID do modelo HuggingFace Florence-2. |
| `labels` | `[]` | Lista opcional de labels de entidades para deteccao guiada (usa `<OPEN_VOCABULARY_DETECTION>`). Quando vazia, Florence-2 gera automaticamente uma descricao de cada imagem e localiza as frases encontradas (usa `<CAPTION>` + `<CAPTION_TO_PHRASE_GROUNDING>`). |
| `batch_size` | `4` | Numero de imagens por lote de inferencia. |

A auto-deteccao e executada uma vez durante a configuracao do dataset. Imagens que ja possuem um arquivo sidecar `.bbox` sao ignoradas, permitindo retomar com seguranca apos uma interrupcao. O modelo e carregado no dispositivo do acelerador e liberado imediatamente apos a conclusao da deteccao.

> **Nota:** A auto-deteccao suporta apenas backends `type: "local"`.

### Arquivos sidecar `.bbox`

Coloque um arquivo `.bbox` ao lado de cada imagem com o mesmo nome base.

O arquivo `.bbox` suporta tres formatos: array JSON, JSON lines, formato txt YOLO.

As coordenadas do bounding box sao normalizadas para [0, 1] no formato XYXY. O formato YOLO usa XYWH baseado no centro e e convertido automaticamente. O campo `mask` e opcional.

### Coluna bbox Parquet / HuggingFace
