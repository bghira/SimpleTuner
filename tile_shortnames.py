...
image_files = [os.path.join(output_dir, file) for file in os.listdir(output_dir) if file.endswith('.png')]
...
for shortname, files in shortname_to_files.items():
    ...
    prompt_img = Image.new('RGB', (200, max_height), 'white')  # 200 is the width of the prompt frame
    prompt_draw = ImageDraw.Draw(prompt_img)
    prompt_font = ImageFont.truetype("arial.ttf", 16)  # Change the font size here
    font = ImageFont.load_default()
    prompt_draw.text((10, max_height // 2), get_wrapped_text(prompt, prompt_font, 25), font=prompt_font, fill='black')
...
for file in image_files:
    ...
    print(f'Found basename: {basename}')
...
for key, GUIDANCE_dict in grouped_files.items():
    ...
    for GUIDANCE, GUIDANCE_RESCALE_dict in GUIDANCE_dict.items():
        ...
        print(f'Working on GUIDANCE {GUIDANCE}')
    ...
    print(f'Stacking images like shit on a log.')
    stacked_image = stack_images_vertically(all_guidance_images)
    print(f'Saving stack')
