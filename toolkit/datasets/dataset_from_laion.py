...
def fetch_image(info):
    ...
    try:
        ...
        if r.status_code != 200:
            logging.warn(f'Failed to fetch {filename} from {url}. Status code: {r.status_code}')
            return
        ...
        if 'nikon' not in camera_model and 'canon' not in camera_model and 'fujifilm' not in camera_model:
...
def fetch_data(data):
    ...
    for row in data:
        try:
            if float(int(row['WIDTH'])) < 1024 or float(int(row['HEIGHT'])) < 1024 or float(row['pwatermark']) > 0.4:
                continue
        except Exception as e:
            continue
        ...
        if new_filename not in to_fetch and count < 100000:
            to_fetch = {
                content_to_filename(row['TEXT']): {'url': row['URL'], 'filename': content_to_filename(row['TEXT']), 'TEXT': row['TEXT']}
                for row in data 
                if row.get('WIDTH') and float(row.get('WIDTH', 0)) >= 1024 
                and row.get('HEIGHT') and float(row.get('HEIGHT', 0)) >= 1024 
                and row.get('pwatermark') and float(row.get('pwatermark', 1)) <= 0.4
            }
            count += 1
