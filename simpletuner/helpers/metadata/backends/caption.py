...
class CaptionMetadataBackend(MetadataBackend):
    ...
    # ------------------------------------------------------------------
    def is_caption_source(self, filepath: str) -> bool:
        """Exclude metadata generated by this backend from caption ingestion."""
        own_files = {os.path.abspath(self.cache_file), os.path.abspath(self.metadata_file)}
        return (
            os.path.abspath(filepath) not in own_files
            and Path(filepath).suffix.lstrip(".").lower() in self.caption_extensions
        )
    ...
    def ingest_from_file_cache(self, caption_files: Dict[str, bool]) -> int:
        ...
        for file_path in sorted(caption_files.keys()):
            if not self.is_caption_source(file_path):
                continue
            try:
