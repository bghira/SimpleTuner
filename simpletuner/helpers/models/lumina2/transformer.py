...
# Modifications Copyright 2025 SimpleTuner contributors. AGPL-3.0-or-later.
#
# Originally from diffusers.models.transformers.transformer_lumina2
# Copied and modified for SimpleTuner advanced features:
# - TREAD (Token Reduction via Attention Dropout)
# - Musubi Block Swapping
# - LayerSync
# - TwinFlow
# - CFG Layer Skip
#
...
class Lumina2RotaryPosEmbed(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: torch.Tensor):
        ...
        cap_freqs_cis = torch.zeros(batch_size, encoder_seq_len, freqs_cis.shape[-1], device=device, dtype=freqs_cis.dtype)
        img_freqs_cis = torch.zeros(batch_size, image_seq_len, freqs_cis.shape[-1], device=device, dtype=freqs_cis.dtype)
    ...
    def forward(
        ...
        for idx, layer in enumerate(self.layers):
            if use_segmented_checkpointing:
                ...
                def run_segmented_block(_idx, segment_layer, segment_hidden_states):
                    return segment_layer(segment_hidden_states, attention_mask if use_mask else None, rotary_emb, temb)
