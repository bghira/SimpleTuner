...
from .transformer import BooguImageTransformer2DModel, PromptEmbedding
...
def retrieve_timesteps(
    ...
    **kwargs,
    ...
    """
    set_timesteps_parameters = set(inspect.signature(scheduler.set_timesteps).parameters.keys())
    kwargs = {key: value for key, value in kwargs.items() if key in set_timesteps_parameters}
    if timesteps is not None:
        accepts_timesteps = "timesteps" in set_timesteps_parameters
    ...
    def encode_instruction(
        ...
        if instruction is None:
            if instruction_embeds is None:
                raise ValueError("`instruction` must be provided when `instruction_embeds` is not supplied.")
