...
class Krea2(ImageModelFoundation):
    ...
    def _prepare_xm_noise_candidates(self, prepared_batch: dict) -> dict:
        ...
        if not torch.is_tensor(latents) or not torch.is_tensor(timesteps) or not torch.is_tensor(sigmas):
            raise ValueError("Krea 2 XM noise-candidate training requires latents, timesteps, and sigmas tensors.")
        if "noisy_latents" not in prepared_batch:
            raise ValueError("Krea 2 XM noise-candidate training requires prepared noisy_latents.")
        if prepared_batch.get("target") is not None:
            raise ValueError("Krea 2 XM noise-candidate training cannot be used with an explicit prepared target.")
        ...
        if not torch.is_tensor(sigma_source):
            raise ValueError("Krea 2 XM noise-candidate training requires tensor sigmas for interpolation.")
    ...
    def _xm_diffusion_loss_tensor(
        ...
        if target is None:
            raise ValueError("Target is None. Cannot compute Krea 2 XM loss.")
        ...
        if loss_type in ["huber", "smooth_l1"]:
            ...
            raise ValueError(f"Krea 2 XM noise-candidate training does not support loss_type={loss_type!r}.")
        ...
        if loss_mask_type == "mask" and apply_conditioning_mask:
            ...
            raise ValueError("Krea 2 XM noise-candidate training does not support stochastic segmentation masked loss.")
        return loss
