# Vendored from huggingface/diffusers#14046 at 3993de59e37344d92aa24ec25bdc39413157b744.
...
from simpletuner.helpers.models.krea2.lora_pipeline import Krea2LoraLoaderMixin
from simpletuner.helpers.models.krea2.pipeline_output import Krea2PipelineOutput
...
class Krea2Pipeline(DiffusionPipeline, Krea2LoraLoaderMixin):
    ...
    def get_text_hidden_states(
        ...
        if images is not None:
            if processor is None:
                raise ValueError("Krea 2 image-context text encoding requires a Qwen3VL processor.")
            images = [images] if isinstance(images, Image.Image) else images
            if len(images) != len(prompt):
                raise ValueError(f"Expected {len(prompt)} reference images for Krea 2 prompt encoding, got {len(images)}.")
            ...
            text_tokens = processor(text=text, images=images, padding=True, return_tensors="pt").to(device)
            outputs = self.text_encoder(
                **text_tokens,
                output_hidden_states=True,
            )
            hidden_states = torch.stack([outputs.hidden_states[i] for i in self.text_encoder_select_layers], dim=2)
            attention_mask = text_tokens.attention_mask.bool()
    ...
    def _encode_vae_image(self, image: torch.Tensor, generator: torch.Generator | list[torch.Generator] | None = None):
    ...
    def prepare_reference_latents(
        ...
        reference_latents = self._encode_vae_image(reference_image, generator=generator)
        if batch_size > reference_latents.shape[0] and batch_size % reference_latents.shape[0] == 0:
            reference_latents = torch.cat([reference_latents] * (batch_size // reference_latents.shape[0]), dim=0)
        elif batch_size > reference_latents.shape[0] and batch_size % reference_latents.shape[0] != 0:
            raise ValueError(
                f"Cannot duplicate `reference_image` of batch size {reference_latents.shape[0]} to {batch_size} prompts."
            )
        reference_latents = reference_latents.squeeze(2)
        latent_height, latent_width = reference_latents.shape[2:]
        packed_reference = self._pack_latents(
            reference_latents,
            batch_size,
            num_channels_latents,
            latent_height,
        ...
        return packed_reference, (latent_height // self.patch_size, latent_width // self.patch_size)
    ...
    @staticmethod
    def prepare_position_ids_for_grids(text_seq_len: int, grids: list[tuple[int, int]], device: torch.device):
        text_ids = torch.zeros(text_seq_len, 3, device=device)
        image_ids = []
    ...
    def __call__(
        ...
        Args:
            ...
            reference_image (`PIL.Image.Image`, list of images, or tensor, *optional*):
                Clean reference image to append as packed VAE latent tokens during denoising.
            prompt_embeds (`torch.Tensor`, *optional*):
                Pre-generated text embeddings of shape `(batch_size, text_seq_len, num_text_layers, text_hidden_dim)`.
        ...
        target_image_seq_len = latents.shape[1]
        reference_latents, reference_grid = self.prepare_reference_latents(
            reference_image,
            batch_size * num_images_per_prompt,
            num_channels_latents,
            height,
        ...
        if reference_grid is not None:
            grids.append(reference_grid)
        position_ids = self.prepare_position_ids_for_grids(prompt_embeds.shape[1], grids, device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                transformer_latents = (
                    torch.cat([latents, reference_latents], dim=1) if reference_latents is not None else latents
                )
                ...
                noise_pred = noise_pred[:, :target_image_seq_len]
