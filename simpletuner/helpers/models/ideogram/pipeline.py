...
from simpletuner.helpers.models.ideogram.autoencoder import AutoEncoder, AutoEncoderParams, convert_diffusers_state_dict
...
def _load_subfolder_state_dict(repo_id: str, subfolder: str, basename: str) -> dict[str, torch.Tensor]:
    ...
    try:
        ...
        single_path = hf_hub_download(repo_id=repo_id, filename=f"{prefix}{basename}.safetensors")
...
def _load_fp8_text_encoder(
    ...
    *,
    ...
    """
    config = AutoConfig.from_pretrained(repo_id, subfolder=text_encoder_subfolder, trust_remote_code=True)
    model = AutoModel.from_config(config, trust_remote_code=True)
    state_dict = _load_subfolder_state_dict(repo_id, text_encoder_subfolder, "model")
    if not device_supports_float8(device):
        # MPS can't hold float8 buffers: dequantize to the compute dtype and load
        # into the standard Linear layers. assign=True lets the loaded bf16 tensors
        # replace the from_config params directly while computed rotary buffers
        # (absent from the checkpoint) survive.
...
def _load_qwen3_vl(
    ...
    *,
    ...
    if is_fp8:
        ...
        model = AutoModel.from_pretrained(repo_id, torch_dtype=dtype, trust_remote_code=True, **model_kwargs)
        model.to(device)
        model.eval()
...
def _build_transformer(
    ...
    if fp8 and not device_supports_float8(device):
        # MPS (and other backends without float8 storage) can't hold the FP8
        # buffers, so fold the scales back into bf16 weights and load normally.
        state_dict = dequantize_fp8_state_dict(state_dict, dtype)
        fp8 = False
    if fp8:
...
def _load_sharded_state_dict(repo_id: str, index_filename: str) -> dict[str, torch.Tensor]:
    ...
    index_path = (
        local_index_path if local_index_path.is_file() else Path(hf_hub_download(repo_id=repo_id, filename=index_filename))
    )
...
def _load_indexed_or_single_state_dict(repo_id: str, index_filename: str) -> dict[str, torch.Tensor]:
...
class Ideogram4PipelineConfig:
    ...
    conditional_index_filename: str = "transformer/diffusion_pytorch_model.safetensors.index.json"
    unconditional_index_filename: str = "unconditional_transformer/diffusion_pytorch_model.safetensors.index.json"
    ...
    def load_lora_weights(
        ...
        **kwargs,
        ...
        if low_cpu_mem_usage and is_peft_version("<", "0.13.0"):
            raise ValueError("`low_cpu_mem_usage=True` is not compatible with this PEFT version. Please update PEFT.")
    ...
    def load_lora_into_transformer(
        ...
        if incompatible_keys is not None:
            ...
            if unexpected_keys:
                ...
                if lora_unexpected:
                    warnings.warn(
                        "Loading Ideogram LoRA weights produced unexpected LoRA keys: " + ", ".join(lora_unexpected),
                        stacklevel=2,
                    )
...
class Ideogram4Pipeline(Ideogram4LoraLoaderMixin):
    ...
    def from_pretrained(
        ...
        *,
        ...
        conditional_state_dict = _load_indexed_or_single_state_dict(config.weights_repo, config.conditional_index_filename)
        unconditional_state_dict = _load_indexed_or_single_state_dict(
            config.weights_repo, config.unconditional_index_filename
        )
        autoencoder_weights = hf_hub_download(repo_id=config.weights_repo, filename=config.autoencoder_filename)
    ...
    def _build_inputs_from_embeds(
        ...
        if attention_mask is None:
            attention_mask = torch.ones(batch_size, max_text_tokens, dtype=torch.bool, device=prompt_embeds.device)
        else:
    ...
    def _normalize_negative_prompts(
        ...
        if len(negative_prompts) != batch_size:
            raise ValueError(f"negative_prompts must have length {batch_size}, got {len(negative_prompts)}")
        ...
        *,
        ...
        if guidance_schedule is not None:
            ...
            if gw_per_step.shape != (num_steps,):
                raise ValueError(f"guidance_schedule must have shape ({num_steps},), " f"got {tuple(gw_per_step.shape)}")
        else:
