...
class Ideogram4Transformer(nn.Module, PeftAdapterMixin):
    ...
    def enable_flowmap_time_conditioning(self, gate_value: float = 0.25, deltatime_type: str = "r") -> None:
        ...
        if self.delta_t_embedding is None:
            self.delta_t_embedding = clone_flowmap_embedder(self.t_embedding)
    ...
    def forward(
        ...
        *,
        ...
        if r_timestep is not None:
            if self.delta_t_embedding is None or self.flowmap_deltatime_type is None:
                raise ValueError(
                    "Ideogram FlowMap conditioning requires `enable_flowmap_time_conditioning()` before training."
                )
            ...
            delta_t_cond = self.delta_t_embedding(delta_timestep)
            t_cond = blend_flowmap_embeddings(t_cond, delta_t_cond, self.flowmap_delta_emb_gate)
        if t.dim() == 1:
