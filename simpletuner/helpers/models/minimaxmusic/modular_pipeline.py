...
_COMPONENT_PREFIXES = ("text_encoder.", "text_encoder_2.", "controlnet.", "unet.", "transformer.")
_COMFY_PREFIXES = ("model.diffusion_model.", "diffusion_model.")
_MINIMAX_MUSIC_NATIVE_LORA_PREFIXES = (
    "diffusion_transformer.",
    "latent_conditioners.",
)
MINIMAX_MUSIC_SWIGLU_GATE_FIRST_METADATA_KEY = "swiglu_gate_first"
MINIMAX_MUSIC_FLOWMAP_GATE_METADATA_KEY = "flowmap_gate_value"
MINIMAX_MUSIC_FLOWMAP_DELTATIME_METADATA_KEY = "flowmap_deltatime_type"
_MINIMAX_MUSIC_CUSTOM_ADAPTER_METADATA_KEYS = {
...
def _is_minimax_music_native_lora_state_dict(state_dict: Dict[str, Any]) -> bool:
    for raw_key in state_dict:
        key = _strip_comfy_lora_prefix(raw_key)
        if ".lora" not in key:
            continue
        if key.startswith(_MINIMAX_MUSIC_NATIVE_LORA_PREFIXES):
            return True
    return False


def _minimax_music_swiglu_gate_first_from_metadata(
    ...
    *,
    ...
    candidate_keys = (
        f"{target_prefix}.{MINIMAX_MUSIC_SWIGLU_GATE_FIRST_METADATA_KEY}",
        f"transformer.{MINIMAX_MUSIC_SWIGLU_GATE_FIRST_METADATA_KEY}",
        MINIMAX_MUSIC_SWIGLU_GATE_FIRST_METADATA_KEY,
    )
    for key in candidate_keys:
        ...
        raise ValueError(f"MiniMax Music 3 LoRA metadata `{key}` must be a boolean, received {value!r}.")
    return None


def _minimax_music_adapter_metadata_value(metadata: Optional[dict], key: str, *, target_prefix: str):
    if not metadata:
        return None
    for candidate in (f"{target_prefix}.{key}", f"transformer.{key}", key):
        if candidate in metadata:
            return metadata[candidate]
    return None


def _minimax_music_peft_adapter_metadata(metadata: Optional[dict], *, target_prefix: str) -> Optional[dict]:
    if not metadata:
        return None
    filtered = {}
    for key, value in metadata.items():
        if key.rsplit(".", 1)[-1] in _MINIMAX_MUSIC_CUSTOM_ADAPTER_METADATA_KEYS:
            continue
        if key.startswith("transformer."):
            key = f"{target_prefix}.{key.removeprefix('transformer.')}"
        else:
...
def _convert_minimax_music_swiglu_lora_layout(
    ...
    *,
    ...
    for key, value in state_dict.items():
        if ".ff_in" not in key or not key.endswith(_LORA_B_SUFFIXES) or not torch.is_tensor(value):
            continue
        if value.ndim == 0 or value.shape[0] % 2 != 0:
            raise ValueError(f"MiniMax Music 3 SwiGLU LoRA tensor `{key}` cannot be split into value/gate rows.")
        first, second = value.chunk(2, dim=0)
        converted[key] = torch.cat((second, first), dim=0).contiguous()
    return converted


def _map_minimax_music_comfy_key_to_diffusers(key: str) -> list[str]:
    key = _strip_comfy_lora_prefix(key)
    if key.startswith("diffusion_transformer."):
        key = key.removeprefix("diffusion_transformer.")

    direct_map = {
        "preprocess_conv": "preprocess_conv",
        "postprocess_conv": "postprocess_conv",
        "timestep_features": "time_proj",
        "to_timestep_embed.0": "time_embed.linear_1",
        "to_timestep_embed.2": "time_embed.linear_2",
        "transformer.project_in": "proj_in",
    ...
    if key.startswith("transformer.layers."):
        key = key.replace("transformer.layers.", "transformer_blocks.", 1)
        key = key.replace(".pre_norm.", ".norm1.")
        key = key.replace(".ff_norm.", ".norm2.")
        key = key.replace(".self_attn.to_out", ".attn.to_out.0")
        key = key.replace(".ff.ff.0.proj", ".ff_in")
        key = key.replace(".ff.ff.2", ".ff_out")
        if ".self_attn.to_qkv" in key:
            return [
                key.replace(".self_attn.to_qkv", ".attn.to_q"),
                key.replace(".self_attn.to_qkv", ".attn.to_k"),
                key.replace(".self_attn.to_qkv", ".attn.to_v"),
            ]
        return [key.replace(".self_attn.", ".attn.")]
...
def _map_comfy_lora_module(module_key: str, target_prefix: str) -> list[str]:
    ...
    mapped_weight_keys = _map_minimax_music_comfy_key_to_diffusers(f"{module_key}.weight")
...
def _convert_minimax_music_comfy_lora_to_diffusers(
    state_dict: Dict[str, Any],
    *,
    target_prefix: str,
    ...
    for raw_key, value in state_dict.items():
        ...
        if qkv_split is not None:
            ...
            if value.shape[0] % 3 != 0:
                raise ValueError(f"MiniMax Music 3 fused QKV LoRA tensor {raw_key} cannot be split into q/k/v tensors.")
            values = value.split(value.shape[0] // 3, dim=0)
        elif (
            target_suffix == ".lora.up.weight"
            and module_key.endswith(".ff.ff.0.proj")
            and target_swiglu_gate_first
            and torch.is_tensor(value)
        ):
            if value.shape[0] % 2 != 0:
                raise ValueError(f"MiniMax Music 3 SwiGLU LoRA tensor {raw_key} cannot be split into value/gate tensors.")
            value_rows, gate_rows = value.split(value.shape[0] // 2, dim=0)
            values = (torch.cat([gate_rows, value_rows], dim=0).contiguous(),)
        else:
...
def _map_minimax_music_diffusers_lora_module_to_native(module_key: str) -> str:
    ...
    direct_map = {
        ...
        "time_proj": "diffusion_transformer.timestep_features",
    ...
    if module_key.startswith("transformer_blocks."):
        module_key = module_key.replace("transformer_blocks.", "diffusion_transformer.transformer.layers.", 1)
    module_key = module_key.replace(".attn.to_out.0", ".self_attn.to_out")
    if module_key.endswith(".attn"):
        module_key = f"{module_key.removesuffix('.attn')}.self_attn"
    else:
        module_key = module_key.replace(".attn.", ".self_attn.")
    module_key = module_key.replace(".ff_in", ".ff.ff.0.proj")
    module_key = module_key.replace(".ff_out", ".ff.ff.2")
    module_key = module_key.replace(".norm1.", ".pre_norm.")
    module_key = module_key.replace(".norm2.", ".ff_norm.")
    return module_key


def _convert_minimax_music_diffusers_lora_to_comfyui(
    state_dict: Dict[str, Any],
    *,
    ...
    for key, value in state_dict.items():
        ...
        if matched_suffix is None:
            if not key.endswith((".alpha", ".lora_alpha")):
                native_key = _map_minimax_music_diffusers_lora_module_to_native(key)
    ...
    def module_alpha(module_key: str, down: torch.Tensor) -> float:
        stripped_module_key = module_key.removeprefix("transformer.")
    ...
    for module_key, tensors in regular_modules:
        if "down" not in tensors or "up" not in tensors:
            raise ValueError(f"Incomplete MiniMax Music 3 LoRA module `{module_key}` while exporting ComfyUI weights.")
        down, up = tensors["down"], tensors["up"]
        native_module = _map_minimax_music_diffusers_lora_module_to_native(module_key)
        if native_module.endswith(".ff.ff.0.proj") and source_swiglu_gate_first:
            if up.shape[0] % 2 != 0:
                raise ValueError(f"MiniMax Music 3 SwiGLU LoRA tensor for `{module_key}` has an odd output size.")
            gate_rows, value_rows = up.chunk(2, dim=0)
            up = torch.cat((value_rows, gate_rows), dim=0).contiguous()
    ...
    for group_key, projections in qkv_groups.items():
        ...
        for module_key, tensors in present:
            if "down" not in tensors or "up" not in tensors:
                raise ValueError(f"Incomplete MiniMax Music 3 LoRA module `{module_key}` while exporting fused QKV weights.")
        ...
        if len(output_sizes) != 1 or len(input_sizes) != 1:
            raise ValueError(f"MiniMax Music 3 QKV LoRA modules under `{group_key}` have incompatible dimensions.")
        ...
        if fused_down.shape[1] != input_size:
            raise ValueError(f"MiniMax Music 3 fused QKV LoRA under `{group_key}` has an invalid input dimension.")

        native_group = _map_minimax_music_diffusers_lora_module_to_native(group_key)
        prefix = f"diffusion_model.{native_group}.to_qkv"
...
class MiniMaxMusic3LoraLoaderMixin(LoraBaseMixin):
    ...
    def load_lora_weights(
        ...
        **kwargs,
        ...
        if not USE_PEFT_BACKEND:
            raise ValueError("PEFT backend is required for MiniMax Music 3 LoRA loading.")
        ...
        if not isinstance(state_dict, dict):
            raise ValueError("MiniMax Music 3 LoRA checkpoint did not return a state dict.")

        transformer = getattr(self, self.transformer_name, None)
        if transformer is None:
            raise ValueError(
                f"MiniMax Music 3 pipeline has no `{self.transformer_name}` component to load LoRA weights into."
            )
        ...
        if lora_format == PEFTLoRAFormat.COMFYUI:
            state_dict, network_alphas = _convert_minimax_music_comfy_lora_to_diffusers(
                state_dict,
            ...
            if source_gate_first is not None:
                state_dict = _convert_minimax_music_swiglu_lora_layout(
                    state_dict,
        ...
        if not transformer_state_dict:
            raise ValueError("No transformer LoRA weights found for MiniMax Music 3.")
        ...
        has_delta_time = any(".delta_time_embed." in key for key in transformer_state_dict)
        gate_value = _minimax_music_adapter_metadata_value(
            adapter_metadata,
            MINIMAX_MUSIC_FLOWMAP_GATE_METADATA_KEY,
            target_prefix=self.transformer_name,
        )
        deltatime_type = _minimax_music_adapter_metadata_value(
            adapter_metadata,
            MINIMAX_MUSIC_FLOWMAP_DELTATIME_METADATA_KEY,
            target_prefix=self.transformer_name,
        )
        if has_delta_time or gate_value is not None or deltatime_type is not None:
        ...
        peft_metadata = _minimax_music_peft_adapter_metadata(
            adapter_metadata,
            target_prefix=self.transformer_name,
        )
        if has_delta_time and (
            peft_metadata is None
            or _minimax_music_adapter_metadata_value(
                peft_metadata,
                "modules_to_save",
                target_prefix=self.transformer_name,
            )
            ...
            for key, alpha in (network_alphas or {}).items():
                local_state_dict[key.removeprefix(f"{self.transformer_name}.")] = torch.tensor(alpha, dtype=torch.float32)
    ...
    def save_lora_weights(
        ...
        **kwargs,
        ...
        if kwargs:
            ...
            raise ValueError(
                f"MiniMax Music 3 LoRA saving only supports transformer weights, got unsupported keys: {unsupported}."
            )
        if not transformer_lora_layers:
            raise ValueError("You must pass `transformer_lora_layers` to save MiniMax Music 3 LoRA weights.")
...
class MiniMaxMusic3ModularPipeline(MiniMaxMusic3LoraLoaderMixin, ModularPipeline):
    """
