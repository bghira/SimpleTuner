...
class MiniMaxMusic3TextEncoderStep(ModularPipelineBlocks):
    ...
    def inputs(self) -> list[InputParam]:
        return [
            ...
            InputParam(
                ...
                description="Precomputed per-frame hidden states for cached validation prompt conditioning.",
            ),
        ]
    ...
    def check_inputs(block_state):
        if not isinstance(block_state.prompt, str) or not block_state.prompt.strip():
            raise ValueError(f"`prompt` (the music description) must be a non-empty string, got {block_state.prompt!r}")
    ...
    def __call__(self, components: MiniMaxMusic3ModularPipeline, state: PipelineState) -> PipelineState:
        ...
        if input_ids.shape[1] > _MAX_PROMPT_TOKENS:
            raise ValueError(f"The assembled prompt has {input_ids.shape[1]} tokens; the maximum is {_MAX_PROMPT_TOKENS}")
...
class MiniMaxMusic3SemanticGenerationStep(ModularPipelineBlocks):
    ...
    @staticmethod
    def check_inputs(block_state):
        if block_state.text_ids is None:
            raise ValueError("`text_ids` must be provided unless precomputed `frame_hiddens` are supplied.")
    ...
    def __call__(self, components: MiniMaxMusic3ModularPipeline, state: PipelineState) -> PipelineState:
        ...
        for frame_index in range(max_frames + 1):
            ...
            frame_codes, depth_hidden = _generate_depth_codes(components, last_hidden, semantic_code.repeat(2), generator)
            if frame_index > 0:
