...
class AutoencoderKLQwenImage21(ModelMixin, AutoencoderMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self._cached_conv_counts = {
            "decoder": (
                sum(isinstance(m, QwenImage21CausalConv3d) for m in self.decoder.modules())
                if self.decoder is not None
                else 0
            ),
            "encoder": (
                sum(isinstance(m, QwenImage21CausalConv3d) for m in self.encoder.modules())
    ...
    def _decode(self, z: torch.Tensor, return_dict: bool = True):
        ...
        if num_frame != 1:
            raise ValueError("Qwen Image 2.1 VAE decoding requires exactly one frame.")
    ...
    def tiled_decode(self, z: torch.Tensor, return_dict: bool = True) -> DecoderOutput | torch.Tensor:
        ...
        for i in range(0, height, tile_latent_stride_height):
            ...
            for j in range(0, width, tile_latent_stride_width):
                ...
                row.append(self.decoder(tile, first_chunk=True))
            rows.append(row)
