...
# Vendored from huggingface/diffusers at e6ddb4676c32e3a59420a82e0da768b27289b0ab.
...
def apply_rotary_emb_qwen(x: torch.Tensor, freqs_cis: torch.Tensor) -> torch.Tensor:
    # Real arithmetic lets Inductor fuse RoPE with the preceding Q/K normalization.
    real, imag = x.float().unflatten(-1, (-1, 2)).unbind(-1)
    cos = freqs_cis.real.unsqueeze(1)
    sin = freqs_cis.imag.unsqueeze(1)
    # Match the fused multiply-add ordering of the original complex CUDA multiply.
    rotated_real = torch.addcmul(-imag * sin, real, cos)
    rotated_imag = torch.addcmul(imag * cos, real, sin)
    return torch.stack((rotated_real, rotated_imag), dim=-1).flatten(-2).to(x.dtype)
...
class QwenImage21TimestepProjEmbeddings(nn.Module):
    def __init__(self, embedding_dim: int):
        ...
        self.delta_timestep_embedder = None
    ...
    def enable_flowmap_time_conditioning(self, gate_value: float = 0.25, deltatime_type: str = "r") -> None:
        self.flowmap_deltatime_type = validate_flowmap_deltatime_type(deltatime_type, model_name="Qwen Image 2.1")
    ...
    def forward(
        self, timestep: torch.Tensor, hidden_states: torch.Tensor, r_timestep: torch.Tensor | None = None
    ) -> torch.Tensor:
        timesteps_proj = self.time_proj(timestep)
        conditioning = self.timestep_embedder(timesteps_proj.to(dtype=hidden_states.dtype))
        if r_timestep is not None:
            if self.delta_timestep_embedder is None:
                raise ValueError("Qwen Image 2.1 requires enable_flowmap_time_conditioning() before using r_timestep.")
            delta_timestep = prepare_flowmap_delta_timestep(
                timestep, r_timestep, self.flowmap_deltatime_type, model_name="Qwen Image 2.1"
            )
            delta_conditioning = self.delta_timestep_embedder(self.time_proj(delta_timestep).to(hidden_states.dtype))
...
class QwenImage21AttnProcessor:
    ...
    image. The segment boundaries are computed once per forward by the model and passed in as `segments`.
    """
    ...
    def __call__(
        ...
        if segments is None:
            ...
            for start, end, is_text in segments:
                seg_mask = None
                causal = is_text and start == 0 and key_valid is None
                if is_text and not causal:
                    backend = self._attention_backend
                    if backend is None:
                        backend = _AttentionBackendRegistry.get_active_backend()[0]
                    if isinstance(backend, AttentionBackendName):
                        backend = backend.value
                    if "flash" in backend or "sage" in backend:
                        raise ValueError(
                            "Qwen Image 2.1 FlashAttention requires an omitted mask for unpadded text-to-image prompts. "
                            "Use native or flex attention for padded prompts or interleaved conditioning; "
                            "a varlen padding mask cannot represent their block-causal mask."
                        )
                    seg_len = end - start
                    seg_mask = torch.cat(
                        [
...
class QwenImage21Attention(torch.nn.Module, AttentionModuleMixin):
    ...
    # The default also works without compilation. FlexAttention requires compilation to avoid dense fp32 scores.
...
class QwenImage21Rope(nn.Module):
    ...
    def text_to_image(self, text_length: int, height: int, width: int, device: torch.device) -> torch.Tensor:
        self.freqs = [freq.to(device) for freq in self.freqs]
        text_positions = torch.arange(text_length, device=device)
        frame = torch.cat([text_positions, text_positions.new_full((height * width,), text_length)])
        rows = torch.arange(-(height - height // 2), height // 2, device=device).repeat_interleave(width)
        columns = torch.arange(-(width - width // 2), width // 2, device=device).repeat(height)
        positions = (frame, torch.cat([text_positions, rows]), torch.cat([text_positions, columns]))
        return torch.cat([freq[index] for freq, index in zip(self.freqs, positions)], dim=-1)
...
class QwenImage21Transformer2DModel(
    ...
    def __init__(
        ...
        if deltatime_type is not None:
            self.enable_flowmap_time_conditioning(
                gate_value=0.25 if gate_value is None else gate_value, deltatime_type=deltatime_type
            )
    ...
    def set_gradient_checkpointing_segment_stride(self, stride: int):
        self.gradient_checkpointing_segment_stride = stride

    def enable_gradient_checkpointing(self, gradient_checkpointing_func=None):
        if gradient_checkpointing_func is None:
            gradient_checkpointing_func = partial(get_checkpoint_function(), use_reentrant=False)
        super().enable_gradient_checkpointing(gradient_checkpointing_func)
    ...
    def forward(
        ...
        Args:
            ...
            r_timestep (`torch.Tensor`, *optional*):
                FlowMap interval endpoint in the same `[0, 1]` scale.
            img_shapes (`list[list[tuple[int, int, int]]]`):
        ...
        is_text_to_image = len(img_shapes[0]) == 1
        if is_text_to_image:
            text_length = encoder_hidden_states.shape[1]
            joint_hidden_states = torch.cat([encoder_hidden_states, hidden_states], dim=1)
            target_token_mask = torch.arange(text_length + target_tokens, device=hidden_states.device) >= text_length
            image_ids = target_token_mask.long() - 1
            _, height, width = img_shapes[0][0]
            rotary_emb = self.pos_embed.text_to_image(text_length, height, width, hidden_states.device)
        else:
        ...
        if r_timestep is not None:
            r_timestep = prepare_flowmap_delta_timestep(timestep, r_timestep, "r", model_name="Qwen Image 2.1")
        if self.config.causal_condition:
            ...
            if r_timestep is not None:
                r_timestep = torch.cat([r_timestep, r_timestep.new_zeros(1)], dim=0)
            modulation_mask = target_token_mask
        else:
            modulation_mask = None
        temb = self.time_text_embed(timestep, hidden_states, r_timestep=r_timestep)
        ...
        if encoder_hidden_states_mask is not None:
            if is_text_to_image:
                joint_key_valid = F.pad(encoder_hidden_states_mask.bool(), (0, target_tokens), value=True)
            else:
        ...
        prefix_len = joint_hidden_states.shape[1] - target_tokens
        ...
        if kv_cache_mode == "cached":
            ...
            block_segments = (
                ...
                else ([(0, prefix_len, True)] if is_text_to_image else _qwenimage21_prefix_segments(image_ids, prefix_len))
            )
        ...
        musubi_active = musubi_manager is not None and musubi_manager.activate(
            self.transformer_blocks, hidden_states.device, torch.is_grad_enabled()
        )
        segmented = (
            ...
            and not musubi_active
            and hidden_states_buffer is None
            and kv_cache is None
        )
        if segmented:

            def run_block(_index, block, states):
                return block(
                    states,
                    modulation,
                    rotary_emb,
            ...
            (joint_hidden_states,) = checkpoint_sequential_state(
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            ...
            checkpoint_block = torch.is_grad_enabled() and should_checkpoint_block(
                index_block,
                self.gradient_checkpointing,
                self.gradient_checkpointing_interval,
                self.gradient_checkpointing_segment_stride,
            )
            if musubi_active and musubi_manager.is_managed_block(index_block):
                musubi_manager.stream_in(block, hidden_states.device, checkpointed=checkpoint_block)
            if checkpoint_block:
            ...
            if hidden_states_buffer is not None:
                hidden_states_buffer[f"layer_{index_block}"] = joint_hidden_states[:, -target_tokens:]
            if musubi_active and musubi_manager.is_managed_block(index_block):
