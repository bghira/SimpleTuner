...
from .autoencoder_21 import AutoencoderKLQwenImage21
from .transformer_21 import QwenImage21KVCache, QwenImage21Transformer2DModel
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ...     transformer=QwenImage21Transformer2DModel.from_pretrained(model_id, subfolder="transformer", torch_dtype=torch.bfloat16),
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            if self.processor is None or self.text_encoder is None:
                raise ValueError("Prompt embeddings are required when the processor or text encoder is not loaded.")
        ...
        if images is not None:
            ...
            for image in images:
                ...
                all_image_latents.append(
                    self._pack_latents(encoded, batch_size, num_channels_latents, image_latent_height, image_latent_width)
                )
    ...
    def __call__(
        ...
        if image is not None:
            ...
            for img in image:
                ...
                vae_images.append(self.image_processor.preprocess(img, width=input_width, height=input_height).unsqueeze(2))
