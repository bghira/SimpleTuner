...
configs_fwd_bsa_varlen = [
    triton.Config({"BLOCK_N": BN}, num_stages=s, num_warps=w) for BN in [32, 64, 128] for s in [2, 3, 4, 5] for w in [4, 8]
]
...
configs_fwd_bsa_varlen_align = [triton.Config({}, num_stages=s, num_warps=w) for s in [2, 3, 4, 5] for w in [4, 8]]
...
bwd_dkdv_bsa_varlen_reevaluate_keys = (
    ["N_CTX", "BLOCK_M", "BLOCK_N_DQ_LG", "SPARSITY"] if os.environ.get("TRITON_REEVALUATE_KEY", "0") == "1" else []
)
...
configs_bwd_dq_bsa_varlen_align = [triton.Config({}, num_stages=s, num_warps=w) for s in [2, 3, 4, 5] for w in [4, 8]]
