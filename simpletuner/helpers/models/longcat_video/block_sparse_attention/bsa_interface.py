...
_AUTOTUNE_ENABLED = os.environ.get("TRITON_AUTOTUNE_ENABLE", "0") == "1"
...
if HAS_TMA_DESC:
    logger.info("TMA benchmarks will run with experimental grid-constant TMA descriptor.")
else:
    logger.info("TMA benchmarks will run without grid-constant TMA descriptor.")
...
def cal_score_triton(q, k):
    ...
    kernel_config = {} if _AUTOTUNE_ENABLED else configs_gating_preset["default"]

    def _grid_gating(args):
        return (triton.cdiv(s_q, args["BLOCK_M"]), B * H, 1)

    _attn_fwd_gating[_grid_gating](
        q,
        k,
        score,
        q.stride(0),
...
def attn_fwd_bsa_varlen_triton(q, k, v, sm_scale, block_indices, block_indices_lens, chunk_size_q, chunk_size_k, sparsity):
    ...
    if chunk_size_k > 128:
        ...
        kernel_config = {} if _AUTOTUNE_ENABLED else configs_fwd_bsa_varlen_preset[config_key]
    else:
        fwd_func = _attn_fwd_bsa_varlen_align
        kernel_config = {} if _AUTOTUNE_ENABLED else configs_fwd_bsa_varlen_align_preset[config_key]
...
def attn_bwd_bsa_varlen_triton(
    do, q, k, v, o, dq, dk, dv, sm_scale, M, block_indices, block_indices_lens, chunk_size_q, chunk_size_k, sparsity
):
    ...
    kernel_config = {} if _AUTOTUNE_ENABLED else configs_bwd_dkdv_bsa_varlen_preset[config_key]

    def _grid_dkdv(args):
        return (triton.cdiv(arg_k.shape[2], args["BLOCK_N"]), 1, arg_k.shape[0] * arg_k.shape[1])

    _attn_bwd_dkdv_bsa_varlen_wrapper[_grid_dkdv](
        q,
        arg_k,
        v,
    ...
    if chunk_size_k > 128:
        ...
        kernel_config = {} if _AUTOTUNE_ENABLED else configs_bwd_dq_bsa_varlen_preset[config_key]
    else:
        bwd_dq_func = _attn_bwd_dq_bsa_varlen_align_wrapper
        kernel_config = {} if _AUTOTUNE_ENABLED else configs_bwd_dq_bsa_varlen_align_preset[config_key]

    def _grid_dq(args):
        return (triton.cdiv(q.shape[2], args["BLOCK_M"]), 1, q.shape[0] * q.shape[1])

    bwd_dq_func[_grid_dq](
        q,
        arg_k,
        v,
        do,
...
class _attention_bsa(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, chunk_size_q, chunk_size_k, sparsity, cdf_threshold, sm_scale, use_tma=False):
        ...
        o, lse = attn_fwd_bsa_varlen_triton(
            q, k, v, sm_scale, block_indices, block_indices_lens, chunk_size_q, chunk_size_k, sparsity
        )
