...
class MiniMaxH3AudioSnake1d(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        alpha = self.alpha.to(device=hidden_states.device, dtype=hidden_states.dtype)
        return hidden_states + (alpha + 1e-9).reciprocal() * torch.sin(alpha * hidden_states).pow(2)
...
class MiniMaxH3AudioSnakeBeta(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        alpha = torch.exp(self.alpha.to(device=hidden_states.device, dtype=hidden_states.dtype).unsqueeze(0).unsqueeze(-1))
        beta = torch.exp(self.beta.to(device=hidden_states.device, dtype=hidden_states.dtype).unsqueeze(0).unsqueeze(-1))
...
class MiniMaxH3AudioAttnProcessor:
    ...
    def __call__(self, attn: "MiniMaxH3AudioCausalAttention", hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        qkv = F.linear(
            ...
            bias=torch.cat((attn.q_bias, attn.zero_k_bias, attn.v_bias)).to(
...
class MiniMaxH3AudioAMPBlock(nn.Module):
    ...
    def __init__(self, channels: int, kernel_size: int, dilation: tuple[int, ...]):
        ...
        self.convs1 = nn.ModuleList(
            [_wn_conv1d(channels, channels, kernel_size, dilation=d, padding=(kernel_size * d - d) // 2) for d in dilation]
        )
