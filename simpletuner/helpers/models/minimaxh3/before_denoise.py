...
def _layout_inputs() -> list[InputParam]:
    ...
    return [
        ...
        InputParam(name="num_latent_frames", type_hint=int, required=True, description="Number of video latent frames."),
...
def _single_text_token_tags(text_token_tags: torch.Tensor, name: str) -> torch.Tensor:
    if text_token_tags.ndim == 2 and text_token_tags.shape[0] == 1:
        text_token_tags = text_token_tags[0]
    if text_token_tags.ndim != 1:
        raise ValueError(f"MiniMax-H3 `{name}` must be 1-D, or batched with one row, got {tuple(text_token_tags.shape)}.")
    return text_token_tags


def _set_layout_state(block_state, layout: MiniMaxH3PackedSequence, device: torch.device, prefix: str = "") -> None:
    setattr(block_state, f"{prefix}layout", layout)
    setattr(block_state, f"{prefix}position_ids", layout.position_ids.to(device))
    setattr(block_state, f"{prefix}token_tags", layout.token_tags.to(device))
    setattr(block_state, f"{prefix}video_indices", layout.video_indices.to(device))
    setattr(block_state, f"{prefix}audio_indices", layout.audio_indices.to(device))
    setattr(block_state, f"{prefix}text_indices", layout.text_indices.to(device))
    if not prefix:
...
class MiniMaxH3PrepareLayoutStep(ModularPipelineBlocks):
        ...
        return [
            *_layout_inputs(),
            InputParam(
                ...
                type_hint=torch.Tensor,
                description="Optional negative-branch text tags for real CFG.",
            ),
            InputParam(
    ...
    def __call__(self, components: MiniMaxH3ModularPipeline, state: PipelineState) -> PipelineState:
        ...
        text_token_tags = _single_text_token_tags(block_state.text_token_tags, "text_token_tags")
        ...
        negative_text_token_tags = getattr(block_state, "negative_text_token_tags", None)
        if negative_text_token_tags is not None:
            negative_text_token_tags = _single_text_token_tags(negative_text_token_tags, "negative_text_token_tags")
...
class MiniMaxH3Ref2VAPrepareLayoutStep(ModularPipelineBlocks):
    ...
    def __call__(self, components: MiniMaxH3Ref2VAModularPipeline, state: PipelineState) -> PipelineState:
        ...
        if negative_text_token_tags is not None:
            negative_text_token_tags = _single_text_token_tags(negative_text_token_tags, "negative_text_token_tags")
            negative_layout = build_ref2va_packed_sequence(
                negative_text_token_tags,
                block_state.prepared_references,
                block_state.num_latent_frames,
                block_state.latent_height,
...
class MiniMaxH3PrepareLatentsStep(ModularPipelineBlocks):
    ...
    def inputs(self) -> list[InputParam]:
        return [
            ...
            InputParam.template(
                "generator",
                description=("The generator of the request. The video noise is drawn from it first, then the audio noise."),
            ),
            InputParam(
                name="latents",
                type_hint=torch.Tensor,
...
class MiniMaxH3SetTimestepsStep(ModularPipelineBlocks):
    ...
    def inputs(self) -> list[InputParam]:
        return [
            ...
            InputParam(
                name="negative_layout",
                type_hint=MiniMaxH3PackedSequence,
                description="Optional negative-branch structural description for real CFG.",
            ),
        ]
    ...
    def intermediate_outputs(self) -> list[OutputParam]:
        return [
            ...
            OutputParam(
                ...
                type_hint=list,
                description="Optional row-to-timestep plan for the negative branch.",
            ),
        ]
    ...
    def __call__(self, components: MiniMaxH3ModularPipeline, state: PipelineState) -> PipelineState:
        ...
        negative_layout = getattr(block_state, "negative_layout", None)
        if negative_layout is not None:
            block_state.negative_row_timestep_plan = [
                tuple(
                    tensor.to(device)
                    for tensor in build_row_timesteps(
                        negative_layout,
