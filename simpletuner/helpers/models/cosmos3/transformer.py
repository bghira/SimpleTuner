...
@dataclass
class Cosmos3ReasonerMemoryState:
    layer_kv: list[dict[str, torch.Tensor]]
...
class Cosmos3PackedMoTAttention(nn.Module, AttentionModuleMixin):
    ...
    def __init__(
        ...
        self.load_reasoning_layers = load_reasoning_layers

        if load_reasoning_layers:
    ...
    def forward(
        ...
        if not self.load_reasoning_layers:
            raise RuntimeError("Cosmos3 generator-only attention cannot run the understanding pathway.")
    ...
    def forward_gen_only(
        ...
        cos_gen, sin_gen = rotary_emb
        cos_gen = cos_gen.unsqueeze(1)
        sin_gen = sin_gen.unsqueeze(1)
        q_gen = q_gen * cos_gen + _rotate_half(q_gen) * sin_gen
        k_gen = k_gen * cos_gen + _rotate_half(k_gen) * sin_gen

        k_und = reasoner_kv.get("k_for_gen", reasoner_kv["k"]).to(device=k_gen.device, dtype=k_gen.dtype)
        v_und = reasoner_kv["v"].to(device=v_gen.device, dtype=v_gen.dtype)
...
class Cosmos3VLTextMoTDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.self_attn = Cosmos3PackedMoTAttention(
            ...
            norm_type=norm_type,
            load_reasoning_layers=load_reasoning_layers,
        )
    ...
    def forward(
        ...
        if not self.load_reasoning_layers:
            raise RuntimeError("Cosmos3 generator-only layer cannot run the understanding pathway.")
    ...
    def forward_und_only(
        ...
        if not self.load_reasoning_layers:
            raise RuntimeError("Cosmos3 generator-only layer cannot produce reasoner K/V tensors.")
        und_norm = self.input_layernorm(und_seq)
        und_attn_out, reasoner_kv = self.self_attn.forward_und_only(und_norm, rotary_emb)
        residual_und = und_seq + und_attn_out
        mlp_out_und = self.mlp(self.post_attention_layernorm(residual_und))
        return residual_und + mlp_out_und, reasoner_kv
    ...
    def forward_gen_only(
        ...
        gen_norm = self.input_layernorm_moe_gen(gen_seq)
        gen_attn_out = self.self_attn.forward_gen_only(gen_norm, rotary_emb, reasoner_kv)
        residual_gen = gen_seq + gen_attn_out
        mlp_out_gen = self.mlp_moe_gen(self.post_attention_layernorm_moe_gen(residual_gen))
...
class Cosmos3OmniTransformer(ModelMixin, ConfigMixin, PeftAdapterMixin, AttentionMixin):
    ...
    def __init__(
        ...
        self.rotary_emb = Cosmos3VLTextRotaryEmbedding(head_dim=head_dim, rope_theta=rope_theta, rope_axes_dim=rope_axes_dim)
    ...
    def forward(
        ...
        target_dtype = packed_tokens_vision.dtype
        split_reasoner = reasoner_memory_state is not None
        if not split_reasoner and not self.load_reasoning_layers:
            raise ValueError("Cosmos3 generator-only transformer requires `reasoner_memory_state`.")
        if split_reasoner:
            gen_sequence_length = sequence_length - und_len
            if gen_sequence_length <= 0:
                raise ValueError("Cosmos3 reasoner replay requires generation tokens after the understanding prefix.")
            hidden_states = packed_tokens_vision.new_zeros(size=(gen_sequence_length, self.config.hidden_size))
            vision_sequence_indexes_local = vision_sequence_indexes - und_len
            vision_mse_loss_indexes_local = vision_mse_loss_indexes - und_len
            if torch.any(vision_sequence_indexes_local < 0) or torch.any(vision_mse_loss_indexes_local < 0):
                raise ValueError("Cosmos3 reasoner replay received vision indexes inside the understanding prefix.")
        else:
            ...
            vision_sequence_indexes_local = vision_sequence_indexes
            vision_mse_loss_indexes_local = vision_mse_loss_indexes
        ...
        hidden_states[vision_sequence_indexes_local] = packed_tokens_vision

        # Pack + project sound latents (when present); all sound frames are noisy.
        if has_sound:
            sound_sequence_indexes_local = sound_sequence_indexes - und_len if split_reasoner else sound_sequence_indexes
            sound_mse_loss_indexes_local = sound_mse_loss_indexes - und_len if split_reasoner else sound_mse_loss_indexes
            if split_reasoner and (
                torch.any(sound_sequence_indexes_local < 0) or torch.any(sound_mse_loss_indexes_local < 0)
            ):
                raise ValueError("Cosmos3 reasoner replay received sound indexes inside the understanding prefix.")
            ...
            hidden_states[sound_sequence_indexes_local] = packed_tokens_sound
        else:
            sound_mse_loss_indexes_local = None

        # Pack + project action latents (when present). Domain ids select the action head weights.
        if has_action:
            action_sequence_indexes_local = action_sequence_indexes - und_len if split_reasoner else action_sequence_indexes
            action_mse_loss_indexes_local = action_mse_loss_indexes - und_len if split_reasoner else action_mse_loss_indexes
            if split_reasoner and (
                torch.any(action_sequence_indexes_local < 0) or torch.any(action_mse_loss_indexes_local < 0)
            ):
                raise ValueError("Cosmos3 reasoner replay received action indexes inside the understanding prefix.")
            ...
            hidden_states[action_sequence_indexes_local] = packed_tokens_action
        else:
            action_mse_loss_indexes_local = None
        ...
        if split_reasoner:
            layer_kv = (
                ...
                if isinstance(reasoner_memory_state, Cosmos3ReasonerMemoryState)
                else reasoner_memory_state["layer_kv"]
            )
            if len(layer_kv) != len(self.layers):
                raise ValueError(f"Cosmos3 reasoner cache has {len(layer_kv)} layers, expected {len(self.layers)}.")
        ...
        preds_vision_packed = self.proj_out(last_hidden_state[vision_mse_loss_indexes_local])
        ...
        if has_sound:
            preds_sound_packed = self.audio_proj_out(last_hidden_state[sound_mse_loss_indexes_local])
        ...
        if has_action:
            ...
            preds_action_packed = self.action_proj_out(
                last_hidden_state[action_mse_loss_indexes_local], per_noisy_domain_ids
            )
            preds_action = self._unpack_action_latents(preds_action_packed, action_token_shapes, action_noisy_frame_indexes)
