...
class Cosmos3AudioSpectrogramConvNeXtEncoder(nn.Module):
    ...
    def __init__(
        ...
        layers.append(weight_norm(nn.Conv1d(self.channel_multiples[-1] * channels, latent_dim, kernel_size=1, bias=False)))
    ...
    def forward(self, audio: torch.Tensor) -> torch.Tensor:
        ...
        if num_channels != self.input_channels:
            raise ValueError(f"Cosmos3 AVAE encoder expected {self.input_channels} audio channels, got {num_channels}.")
...
class Cosmos3AVAEAudioTokenizer(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if not dec_use_snake or dec_final_tanh or dec_anti_aliasing or dec_use_nearest_upsample or dec_use_tanh_at_final:
