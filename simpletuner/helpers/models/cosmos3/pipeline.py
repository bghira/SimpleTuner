...
def get_3d_mrope_ids_vae_tokens(
    ...
    effective_base_tcf = (
        base_temporal_compression_factor if base_temporal_compression_factor is not None else temporal_compression_factor
    )
...
class Cosmos3OmniPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.vae_scale_factor_temporal = int(self.vae.config.scale_factor_temporal) if getattr(self, "vae", None) else 4
    ...
    def _get_execution_device(self) -> torch.device:
        # Walk the actual compute modules first so optional non-module components do not decide the execution device.
    ...
    def _prepare_action_video_conditioning(
        ...
        frames = frames_t.reshape(frames.shape[0], num_frames, frames.shape[1], target_h, target_w).permute(0, 2, 1, 3, 4)
    ...
    def prepare_latents(
        ...
        if enable_sound and x0_tokens_sound is not None:
            ...
            if sound_latents is None:
                ...
                sound_latents = sound_condition_mask.T * x0_tokens_sound + (1.0 - sound_condition_mask.T) * pure_noise_sound
            else:
    ...
    @staticmethod
    def _format_duration_and_range(num_frames: int, fps: float) -> tuple[str, str]:
        duration_seconds = num_frames / fps if fps > 0 else 0.0
        duration = max(int(round(duration_seconds)), 0) if math.isfinite(duration_seconds) else 0
        minutes, seconds = divmod(duration, 60)
        return f"{duration}s", f"0:00-{minutes}:{seconds:02d}"
    ...
    def _build_action_json_prompt(
        ...
        *,
        ...
        aspect_ratio = Cosmos3OmniPipeline._aspect_ratio_label(width=width, height=height)
    ...
    @classmethod
    def _build_image_json_prompt(cls, description: str, *, height: int, width: int) -> str:
        parsed = cls._maybe_json_object(description)
        desc = " ".join(str(description or "").split())
        base = {
            "subjects": [
                {
                    "description": desc,
                    "appearance_details": desc,
                    "relationship": "primary subject of the requested scene",
                    "location": "center of the frame",
                    "relative_size": "medium within frame",
                    "orientation": "facing the camera or main action",
                    "pose": "as described by the prompt",
                    "clothing": "",
                    "expression": "",
                    "gender": "Unknown",
                    "age": "Unknown",
                    ...
                    "facial_features": "",
                    "number_of_subjects": 1 if desc else 0,
                    "number_of_arms": 0,
                    "number_of_legs": 0,
                    "number_of_hands": 0,
                    "number_of_fingers": 0,
            ...
            "subject_details": {},
            "background_setting": desc,
            "lighting": {
                "conditions": "natural, balanced lighting",
                "direction": "front-lit",
                "shadows": "soft shadows",
                "illumination_effect": "clear visibility across the scene",
            },
            "aesthetics": {
                "composition": "balanced composition centered on the requested subject",
                "color_scheme": "colors follow the prompt",
                "mood_atmosphere": "grounded and coherent",
                "patterns": "",
            },
            "cinematography": {
                "framing": "medium shot",
                "camera_angle": "eye-level",
                "depth_of_field": "uniform focus",
                "focus": "the requested subject and scene",
                "lens_focal_length": "natural perspective",
            },
            "style_medium": "image",
            "artistic_style": "as described by the prompt",
            "context": desc,
            "text_and_signage_elements": [],
            "quadrant_scan": {
                "top_left": "scene detail follows the prompt",
                "top_right": "scene detail follows the prompt",
                "bottom_left": "scene detail follows the prompt",
                "bottom_right": "scene detail follows the prompt",
                "absolute_center": desc,
            },
            "comprehensive_t2i_caption": desc,
        }
        if parsed is not None:
            base.update(parsed)
        base["resolution"] = {"H": int(height), "W": int(width)}
        base["aspect_ratio"] = cls._aspect_ratio_label(width=width, height=height)
        return cls._compact_json(base)

    @classmethod
    def _build_video_json_prompt(
        cls,
        description: str,
        *,
        num_frames: int,
        ...
        parsed = cls._maybe_json_object(description)
        desc = " ".join(str(description or "").split())
        duration, time_range = cls._format_duration_and_range(num_frames=num_frames, fps=fps)
        base = {
            "subjects": [
                {
                    "description": desc,
            ...
            "cinematography": {
                ...
                "focus": "the requested subject and action",
                "lens_focal_length": "natural perspective",
            },
            "style_medium": "video",
            "artistic_style": "as described by the prompt",
            "context": desc,
            "actions": [{"time": time_range, "description": desc}],
            "text_and_signage_elements": [],
            "segments": [
                {
                    "segment_index": 0,
        ...
        base["fps"] = int(fps) if float(fps).is_integer() else float(fps)
        return cls._compact_json(base)

    @classmethod
    def _build_generation_json_prompt(
        cls,
        description: str,
        *,
        num_frames: int,
        ...
        if num_frames == 1:
            return cls._build_image_json_prompt(description, height=height, width=width)
        return cls._build_video_json_prompt(description, num_frames=num_frames, fps=fps, height=height, width=width)
    ...
    def tokenize_prompt(
        ...
        When ``negative_prompt`` is ``None``, an empty string is used. Positive prompts are converted to Cosmos3
        structured JSON captions before tokenization. Negative prompts keep the flat inverse metadata templates.
        ...
        if action_mode is not None:
            ...
            cond_text = self._build_generation_json_prompt(
                prompt, num_frames=num_frames, fps=fps, height=height, width=width
            )
    ...
    def __call__(
        ...
        image: torch.Tensor | None = None,
        video: list[Any] | torch.Tensor | np.ndarray | None = None,
        condition_frame_indexes_vision: Iterable[int] = (0, 1),
        condition_video_keep: Literal["first", "last"] = "first",
        num_frames: int | None = None,
        height: int | None = None,
        ...
        callback_on_step_end: (
            Callable[[int, int, dict[str, Any]], None] | PipelineCallback | MultiPipelineCallbacks | None
        ) = None,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                sound_timesteps = torch.full((sound_len,), timestep, device=device) if sound_tokens is not None else None
                action_timesteps = (
