...
def register_advanced_fields(registry: "FieldRegistry") -> None:
    ...
    registry._add_field(
        ConfigField(
            ...
            help_text="Order for the chosen solver (UniPC up to 9; DPM/Euler ignore >3/1).",
    ...
    registry._add_field(
        ConfigField(
            ...
            default_value=1,
            validation_rules=[ValidationRule(ValidationRuleType.MIN, value=1, message="Must be at least 1")],
            help_text="Number of ranks used for context parallel sharding across the sequence dimension.",
            tooltip="Set to >1 to shard attention across GPUs. Without FSDP, each CP group keeps replicated model weights.",
            importance=ImportanceLevel.ADVANCED,
            order=19,
            platform_specific=["cuda"],
        )
