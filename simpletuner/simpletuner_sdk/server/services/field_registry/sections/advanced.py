...
def register_advanced_fields(registry: "FieldRegistry") -> None:
    ...
    registry._add_field(
        ConfigField(
            ...
            section="data_config",
            default_value=False,
            help_text="If set, will delete images that are not of a minimum size to save on disk space for large training runs",
            tooltip="Default behaviour: Unset, remove images from bucket only.",
            importance=ImportanceLevel.ADVANCED,
            order=43,
        )
    )
    ...
    registry._add_field(
        ConfigField(
            ...
            section="data_config",
            default_value=False,
            help_text="If set, any images that error out during load will be removed from the underlying storage medium",
            tooltip="This is useful to prevent repeatedly attempting to cache bad files on a cloud bucket.",
            importance=ImportanceLevel.ADVANCED,
            order=44,
        )
    )
    ...
    registry._add_field(
        ConfigField(
            ...
            tooltip="This is the path to a local directory that will contain your VAE outputs. Unlike the text embed cache, your VAE latents will be stored in the AWS data backend. Each backend can have its own value, but if that is not provided, this will be the default value.",
            importance=ImportanceLevel.ADVANCED,
            order=51,
        )
    )
    ...
    registry._add_field(
        ConfigField(
            ...
            choices=[
                ...
                {"value": "v", "label": "V-shaped"},
                {"value": "cosine", "label": "Cosine"},
            ],
            help_text="Strategy for biasing timestep sampling",
            tooltip="'none': no bias. 'range': bias to specific range. 'v': bias to early/late timesteps. 'cosine': cosine weighting.",
            importance=ImportanceLevel.ADVANCED,
            order=3,
        )
    )
