...
def register_training_fields(registry: "FieldRegistry") -> None:
    ...
    registry._add_field(
        ConfigField(
            ...
            default_value="torch",
            choices=[
                {"value": "torch", "label": "PyTorch layer (recompute)"},
                {"value": "torch-ffn", "label": "PyTorch FFN-only (recompute)"},
                {"value": "unsloth", "label": "Unsloth layer (CPU offload)"},
                {"value": "unsloth-ffn", "label": "Unsloth FFN-only (CPU offload)"},
            ],
            dependencies=[FieldDependency(field="gradient_checkpointing", operator="equals", value=True, action="show")],
            validation_rules=[
                ValidationRule(ValidationRuleType.CHOICES, value=["torch", "torch-ffn", "unsloth", "unsloth-ffn"]),
            ],
            help_text="Backend and scope for gradient checkpointing. PyTorch recomputes activations; Unsloth offloads to CPU.",
            tooltip="FFN-only variants checkpoint the feed-forward side on supported models for a cheaper memory trade.",
            importance=ImportanceLevel.ADVANCED,
            order=2,
