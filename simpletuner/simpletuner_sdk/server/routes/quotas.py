"""Quota management endpoints for training.

NOTE: This module was moved from routes/cloud/quotas.py to become a top-level
global route, as quotas apply to all jobs, not just cloud jobs.
"""
...
async def get_quota(
    ...
    user_store = UserStore()
    quota = await user_store.get_quota(quota_id)
    ...
    return QuotaResponse(
        ...
        level_id=quota.level_id,
        is_global=quota.is_global,
    )
...
async def estimate_cost_impact(
    ...
    """Estimate the impact of a job on user quotas.

    Returns whether the job would exceed any quotas and what the
    new usage would be after the job completes.
    """
    user_store = UserStore()
    job_store = get_job_store()
    ...
    for s in statuses:
        if s.quota_type in (QuotaType.COST_DAILY, QuotaType.COST_MONTHLY):
            new_value = s.current_value + estimated_cost
            new_percent = (new_value / s.limit_value) * 100 if s.limit_value > 0 else 0
            exceeds = new_value >= s.limit_value

            if exceeds and s.action == QuotaAction.BLOCK:
                would_exceed = True
            ...
            impacts.append(
                {
                    ...
                    "after": new_value,
                    "limit": s.limit_value,
                    "percent_before": s.percent_used,
                    "percent_after": round(new_percent, 1),
