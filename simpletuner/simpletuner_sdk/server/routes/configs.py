...
from simpletuner.simpletuner_sdk.server.services.config_store import (
    ConfigStore,
    ConfigMetadata,
    ConfigValidation
)
...
async def get_active_config() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Active configuration '{active_name}' not found"
        )
...
@router.post("/")
async def create_config(request: ConfigRequest) -> Dict[str, Any]:
    ...
    try:
        saved_metadata = store.save_config(
            request.name,
            request.config,
            metadata,
            overwrite=False
...
@router.put("/{name}")
async def update_config(name: str, request: ConfigRequest) -> Dict[str, Any]:
...
async def rename_config(name: str, request: ConfigRenameRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "message": f"Configuration renamed from '{name}' to '{request.new_name}'",
            "metadata": metadata.model_dump()
        }
    except FileNotFoundError:
        raise HTTPException(
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Configuration '{request.new_name}' already exists"
        )


@router.post("/{name}/copy")
async def copy_config(name: str, request: ConfigCopyRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "message": f"Configuration '{name}' copied to '{request.target_name}'",
            "metadata": metadata.model_dump()
        }
    except FileNotFoundError:
        raise HTTPException(
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Configuration '{request.target_name}' already exists"
        )
...
async def export_config(name: str, include_metadata: bool = True) -> Dict[str, Any]:
...
async def create_from_template(request: ConfigFromTemplateRequest) -> Dict[str, Any]:
    ...
    try:
        metadata = store.create_from_template(
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Configuration '{request.config_name}' already exists"
        )


@router.post("/{name}/validate")
async def validate_config(name: str) -> Dict[str, Any]:
...
async def validate_config_data(config: Dict[str, Any]) -> Dict[str, Any]:
