...
NOTE: This module was moved from routes/cloud/webhooks.py to become a top-level
global route, as webhooks are a global concept that cloud providers reuse.
"""
...
def _verify_webhook_signature(
    ...
    try:
        ...
        now = int(time.time())
        if abs(now - ts) > 300:
            logger.warning("Webhook timestamp too old or in future: %s", webhook_timestamp)
            return False
...
async def replicate_webhook(request: Request, _user: User = Depends(get_current_user)) -> Dict[str, Any]:
    ...
    # Note: Rate limiting is handled by RateLimitMiddleware (100 req/min for /api/webhooks/)
