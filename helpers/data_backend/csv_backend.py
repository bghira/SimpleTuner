...
class CSVDataBackend(BaseDataBackend):
    ...
    def list_files(self, str_pattern: str, instance_data_root: str = None) -> tuple:
        ...
        filtered_ids = set(filter(lambda id: fnmatch.fnmatch(id, str_pattern), list(self.df.index)))
...
if __name__ == "__main__":
    data = CSVDataBackend(None, "test", "/media/second8TBNVME/cache/SimpleTuner/jewelry-v13.csv", image_cache_loc="/media/second8TBNVME/cache/SimpleTuner/csv-data-cache", compress_cache=False)
    results = data.list_files("*.[jJpP][pPnN][gG]", instance_data_root="/media/second8TBNVME/cache/SimpleTuner/jewelry-v13")[0][2]
    # print(results)
