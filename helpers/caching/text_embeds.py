...
class TextEmbeddingCache(WebhookMixin):
    ...
    def compute_embeddings_for_prompts(
        ...
        if self.model is not None:
            ...
            output = self.compute_embeddings_for_wan_prompts(
                raw_prompts,
                return_concat=return_concat,
                load_from_cache=load_from_cache,
                is_negative_prompt=is_negative_prompt,
    ...
    def compute_embeddings_for_wan_prompts(
        self,
        prompts: list = None,
        return_concat: bool = True,
        load_from_cache: bool = True,
        ...
        logger.debug(
            f"compute_embeddings_for_wan_prompts arguments: prompts={prompts}, return_concat={return_concat}, load_from_cache={load_from_cache}"
        )
        prompt_embeds_all = []
        prompt_embeds_all = []
        should_encode = not load_from_cache
        ...
        if (
            ...
            logger.debug("Setting should_encode = True")
        # self.debug_log(
        #     f"compute_embeddings_for_wan_prompts received list of prompts: {list(prompts)[:5]}"
        # )
        if should_encode:
            local_caption_split = self.split_captions_between_processes(
                prompts or self.prompts
