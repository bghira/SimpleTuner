...
class DistillerFactory:
    ...
    def _create_dcm_distiller(
        ...
        try:
            ...
            raise ImportError("DCM distiller not found. Please ensure helpers.distillation.dcm is available.")
        ...
        if model_type == "lora":
            logger.info("Loading DCM flow-matching distillation via low-rank adapter training.")
            return DCMDistiller(
                teacher_model=teacher_model,
            ...
            logger.info("Loading DCM distillation with separate teacher/student models.")
            return DCMDistiller(
                teacher_model=teacher_model,
                student_model=student_model,
                noise_scheduler=noise_scheduler,
    ...
    def _create_lcm_distiller(
        ...
        try:
            ...
            raise ImportError("LCM distiller not found. Please ensure helpers.distillation.lcm is available.")
        ...
        if model_type == "lora":
            ...
            logger.info("Loading LCM distillation with separate teacher/student models.")
            return LCMDistiller(
                teacher_model=teacher_model,
                student_model=student_model,
                noise_scheduler=noise_scheduler,
