...
class SlimFlowDistiller(DistillationBase):
    ...
    def __init__(self, teacher_model, student_model, config=None):
        ...
        self.current_beta = 1.0 if self.config['train_mode'] == 'annealing_reflow' else 0.0
    ...
    def _compute_beta(self, step):
        ...
        if schedule == 'linear':
            ...
            return (1 + torch.cos(torch.tensor(min(1, step / (2 * beta_steps)) * torch.pi)).item()) / 2
        elif schedule == 'exp':
    ...
    def prepare_batch(self, batch, model, state):
        ...
        if self.config['train_mode'] == 'annealing_reflow':
            ...
            with torch.no_grad():
                ...
                prepared_batch["x1_beta"] = torch.sqrt(1 - self.current_beta**2) * prepared_batch["latents"] + self.current_beta * x1_prime
                
                # For teacher model, we need to compute the target
                if self.current_beta < 1.0:  # Only compute teacher target if we're using it
                    with self.teacher_model.get_trained_component().eval():
                        ...
                        prepared_batch["teacher_target"] = teacher_output["model_prediction"]
            ...
            with torch.no_grad():
                ...
                bsz = prepared_batch["latents"].shape[0]
                device = prepared_batch["latents"].device
                t = torch.rand(bsz, device=device).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
    ...
    def compute_distill_loss(self, prepared_batch, model_output, original_loss):
        ...
        if self.config['train_mode'] == 'annealing_reflow':
            ...
            if self.current_beta < 1.0 and "teacher_target" in prepared_batch:
                ...
                interpolated_loss = self.current_beta * original_loss + (1 - self.current_beta) * reflow_loss
            ...
            if "intermediate_t" in prepared_batch:
                ...
                with torch.no_grad():
                    ...
                    teacher_intermediate = self.teacher_model.model_predict(intermediate_input)
