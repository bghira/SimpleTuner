...
class Sana(ImageModelFoundation):
    ...
    def _encode_prompts(self, prompts: list, is_negative_prompt: bool = False):
        ...
        prompt_embeds, prompt_attention_mask, _, _ = self.pipelines[
            ...
            complex_human_instruction=(
                ...
                else self.config.sana_complex_human_instruction
            ),
        )
    ...
    def check_user_config(self):
        ...
        if self.config.base_model_precision == "fp8-quanto":
            raise ValueError(
                "Sana does not support fp8-quanto. Please use fp8-torchao or int8 precision level instead."
            )
        if self.config.tokenizer_max_length is not None:
            logger.warning(
                "Tokenizer max length is ignored for Sana. It is fixed to 300 tokens."
            )
