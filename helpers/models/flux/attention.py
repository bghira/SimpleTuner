...
class FluxFlashAttnProcessor3(object):
    """
    Flash Attention 3 processor for Flux models.
    Replaces PyTorch SDPA with Flash Attention 3 kernels.
    """
    
    def __init__(self):
        if flash_attn_func is None:
            raise ImportError(
                "FluxFlashAttnProcessor3 requires flash-attn library. "
    ...
    def __call__(
        ...
        # Flash Attention 3
        # Input: (batch, heads, seq_len, head_dim) -> (batch, seq_len, heads, head_dim)
        query = query.permute(0, 2, 1, 3)
        key = key.permute(0, 2, 1, 3)
        value = value.permute(0, 2, 1, 3)
        
        hidden_states = flash_attn_func(query, key, value)[0]
        
        # Output: (batch, seq_len, heads, head_dim) -> (batch, seq_len, heads * head_dim)
        hidden_states = rearrange(hidden_states, "b s h d -> b s (h d)")
        hidden_states = hidden_states.to(query.dtype)
