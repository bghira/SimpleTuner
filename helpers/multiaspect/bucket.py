...
class BucketManager:
    ...
    def split_buckets_between_processes(self):
        ...
        logger.debug(f"Count of items before split: {sum([len(bucket) for bucket in self.aspect_ratio_bucket_indices])}")
        ...
        logger.debug(f"Count of items after split: {sum([len(bucket) for bucket in self.aspect_ratio_bucket_indices])}")
