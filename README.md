...
- **External validation scripts** - Swap the built-in validation pipeline for your own executable via `--validation_method=external-script`, with placeholders like `{local_checkpoint_path}`, `{global_step}`, `{tracker_run_name}`, `{tracker_project_name}`, and `{model_family}`
