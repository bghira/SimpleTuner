...
class AssistantRegularisationTests(unittest.TestCase):
    ...
    def test_target_is_assistant_only_and_student_updates_only_concept(self):
        ...
        with torch.no_grad():
            base = self.component(**self.inputs)[0]
        self.component.enable_lora()
        self.restore()
        self.assertFalse(torch.allclose(expected, base))
    ...
    def test_parent_failure_restores_training_stack(self):
        def fail(prepared_batch):
            self.assertEqual(self.component.active_adapters(), ["assistant"])
