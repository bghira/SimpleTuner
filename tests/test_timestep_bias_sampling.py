"""Unit tests for per-dataset flow-matching ``timestep_bias`` sampling."""
...
class TestTimestepBias(unittest.TestCase):
    """``ModelFoundation._get_dataset_timestep_bias`` resolves the per-dataset bias."""
    ...
    def test_reads_per_dataset_bias(self, mock_get_config):
        mock_get_config.return_value = {"timestep_bias": -0.5}
        bias = ModelFoundation._get_dataset_timestep_bias(object(), {"data_backend_id": "ds1"})
    ...
    def test_missing_backend_id_is_unbiased(self, mock_get_config):
        mock_get_config.return_value = {}
        bias = ModelFoundation._get_dataset_timestep_bias(object(), {})
