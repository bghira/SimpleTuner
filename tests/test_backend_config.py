...
class TestImageBackendConfig(unittest.TestCase):
    ...
    def test_from_dict_full_config(self):
        ...
        self.assertEqual(config.timestep_bias, -0.5)
    ...
    def test_to_dict_full_output(self):
        ...
        self.assertEqual(output["config"]["timestep_bias"], 0.5)

    def test_timestep_bias_defaults_to_zero(self):
        """timestep_bias is optional and defaults to 0.0 (no sampling bias)"""
        backend_dict = {"id": "image_test", "type": "local"}

        config = ImageBackendConfig.from_dict(backend_dict, self.args)

        self.assertEqual(config.timestep_bias, 0.0)
        self.assertEqual(config.to_dict()["config"]["timestep_bias"], 0.0)
