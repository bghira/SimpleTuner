...
class SimpleTunerTestRunner:
    ...
    def draw_test_overlay(self, stdscr, height: int, width: int):
        ...
        self.test_viewport_start = max(0, min(self.test_viewport_start, 
                                              max(0, len(visible_tests) - viewport_height)))
    ...
    def run_ui(self, stdscr):
        ...
        with open(self.debug_log_path, 'a') as f:
            ...
            f.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Found {len(examples)} examples: {', '.join(examples)}\n")
...
def main():
    ...
    parser.add_argument("--parallel", type=int, default=1, help="Number of parallel tests (not implemented yet)")
