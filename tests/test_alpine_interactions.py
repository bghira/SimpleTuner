...
class TestAlpineJsComponentIsolation:
        ...
        if len(scopes) >= 2:
            ...
            for i, field in enumerate(fields_with_alpine[1:], 1):
                ...
                if other_scope and "value" in other_scope:
                    assert other_scope["value"] != "modified-value", f"Field {i} was affected by changes to field 0"
    ...
    def test_alpine_x_show_x_if_conditions(self, driver):
        ...
        for section in collapsible_sections:
            ...
            if parent:
                ...
                assert is_visible == alpine_state, "x-show visibility doesn't match Alpine state"
    ...
    def test_alpine_stores_isolation(self, driver):
        ...
        if stores:
            ...
            for store_name in stores:
                ...
                assert isinstance(store_data, (dict, type(None))), f"Store {store_name} has unexpected type"
            ...
            if parent_with_data:
                ...
                try:
                    ...
                    if "!" in click_handler:  # Toggle operation
                        assert before_data != after_data, f"Click handler didn't change state: {click_handler}"
                except Exception:
    ...
    def test_alpine_component_cleanup(self, driver):
        ...
        assert abs(final_count - initial_count) <= 1, f"Component count changed: {initial_count} -> {final_count}"
    ...
    def test_alpine_x_model_not_present(self, driver):
        ...
        for input_elem in inputs:
            ...
            assert x_model is None, f"Input {input_elem.get_attribute('id')} has x-model binding"
    ...
    def test_alpine_transitions(self, driver):
        ...
        if transition_elements:
            ...
            if parent:
                ...
                assert initial_visible != final_visible, "Transition didn't change element visibility"
...
class TestAlpineJsDataFlow:
    ...
    def test_alpine_data_binding_independence(self, driver):
        ...
        assert configs_component != model_component, "Form fields share the same Alpine component"
        ...
        assert model_data.get("value") != "test-value", "Data leaked between Alpine components"
        assert model_data.get("error") != "test-error", "Error state leaked between components"
