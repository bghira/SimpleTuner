...
from simpletuner.helpers.training.collate import collate_fn  # Adjust this import according to your project structure
from simpletuner.helpers.training.state_tracker import StateTracker  # Adjust imports as needed
...
class TestCollateFn(unittest.TestCase):
    def setUp(self):
        ...
        StateTracker.set_args(MagicMock(caption_dropout_probability=0.5, controlnet=False, flux=False))
    ...
    def test_collate_fn(self, mock_gather, mock_compute_embeds, mock_compute_latents):
        ...
        with patch("simpletuner.helpers.training.state_tracker.StateTracker.get_data_backend"):
            # Mock get_data_backend() to return a mock object with required attributes
            StateTracker.get_data_backend.return_value = MagicMock(compute_embeddings_for_legacy_prompts=MagicMock())
