...
# while in diffusers it split into scale, shift. Here we swap the linear
# projection weights in order to be able to use diffusers implementation
...
def convert_flux_transformer_checkpoint_to_diffusers(
    ...
    converted_state_dict["time_text_embed.text_embedder.linear_1.bias"] = original_state_dict.pop("vector_in.in_layer.bias")
    converted_state_dict["time_text_embed.text_embedder.linear_2.weight"] = original_state_dict.pop(
        "vector_in.out_layer.weight"
    )
    converted_state_dict["time_text_embed.text_embedder.linear_2.bias"] = original_state_dict.pop("vector_in.out_layer.bias")
    ...
    for i in range(num_layers):
        ...
        converted_state_dict[f"{block_prefix}ff.net.2.bias"] = original_state_dict.pop(f"double_blocks.{i}.img_mlp.2.bias")
    ...
    for i in range(num_single_layers):
        ...
        # output projections.
        converted_state_dict[f"{block_prefix}proj_out.weight"] = original_state_dict.pop(f"single_blocks.{i}.linear2.weight")
        converted_state_dict[f"{block_prefix}proj_out.bias"] = original_state_dict.pop(f"single_blocks.{i}.linear2.bias")
...
def main(args):
    ...
    if args.transformer:
        ...
        print(
            f"Saving Flux Transformer in Diffusers format. Variant: {
                'guidance-distilled' if has_guidance else 'timestep-distilled'}"
        )
